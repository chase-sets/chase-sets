...
- Milestone reviews (anti-ratchet): at most one comprehensive review per milestone per week, and each review must close or deliver something. Never amend new requirements into existing issues — a genuinely new gap becomes a new, fixed-scope issue. File it with the `slice` form so it lands classified (wave milestone + `priority:*` + `area:*` + `kind:*`), attach it as a **sub-issue of the owning epic**, and record any chain link as a **GitHub issue dependency** rather than only a `Blocked by #N` line. An unclassified, unparented issue is invisible to roll-up and undispatchable — see `docs/contributing/backlog-model.md`. This applies to issue and milestone comments too: review passes and evidence ledgers must not relocate there. The cadence is monitored by the weekly Review Cadence Digest workflow (advisory, never blocking).
