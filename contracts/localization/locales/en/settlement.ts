export const settlementEnglishTranslations = {
  "settlement.features.payoutReadiness.api.route.authentication.context.missing": "Authentication context missing.",
  "settlement.features.payoutReadiness.api.route.authentication.context.missing.2": "Authentication context missing.",
  "settlement.features.payoutReadiness.api.route.authentication.context.missing.3": "Authentication context missing.",
  "settlement.features.payoutReadiness.api.route.authentication.required": "Authentication required.",
  "settlement.features.payoutReadiness.api.route.forbidden": "Forbidden.",
  "settlement.features.payoutReadiness.api.route.request.failed": "Request failed.",
  "settlement.features.payoutReadiness.api.route.step.up.required":
    "Confirm it is you before managing payout account details.",
  "settlement.features.payoutReadiness.domain.setupProgress.a.payout.destination.is.ready":
    "A payout destination is ready.",
  "settlement.features.payoutReadiness.domain.setupProgress.add.or.confirm.the.payout.destination":
    "Add or confirm the payout destination.",
  "settlement.features.payoutReadiness.domain.setupProgress.continue.the.hosted.setup.flow":
    "Continue the payout setup page.",
  "settlement.features.payoutReadiness.domain.setupProgress.funds.can.move.from.the.platform":
    "Funds can move from the platform balance.",
  "settlement.features.payoutReadiness.domain.setupProgress.hosted.setup": "Payout setup",
  "settlement.features.payoutReadiness.domain.setupProgress.hosted.setup.is.complete": "Payout setup is complete.",
  "settlement.features.payoutReadiness.domain.setupProgress.identity.and.business.details":
    "Identity and business details",
  "settlement.features.payoutReadiness.domain.setupProgress.payout.creation.is.enabled": "Payout creation is enabled.",
  "settlement.features.payoutReadiness.domain.setupProgress.payout.destination": "Payout destination",
  "settlement.features.payoutReadiness.domain.setupProgress.payouts.enabled": "Payouts enabled",
  "settlement.features.payoutReadiness.domain.setupProgress.payouts.must.be.enabled.before.funds":
    "Payouts must be enabled before funds can be sent.",
  "settlement.features.payoutReadiness.domain.setupProgress.some.verification.details.need.attention":
    "Some verification details need attention.",
  "settlement.features.payoutReadiness.domain.setupProgress.transfers.enabled": "Transfers enabled",
  "settlement.features.payoutReadiness.domain.setupProgress.transfers.must.be.enabled.before.payouts":
    "Transfers must be enabled before payouts can be requested.",
  "settlement.features.payoutReadiness.domain.setupProgress.verification.details.are.being.tracked":
    "Verification details are being tracked.",
  "settlement.features.payoutReadiness.readModel.schema.create.table.if.not.exists.settlement":
    "\nCREATE TABLE IF NOT EXISTS settlement_payout_readiness_pages (\n  account_id text PRIMARY KEY,\n  status text NOT NULL,\n  missing_requirements jsonb NOT NULL DEFAULT '[]'::jsonb,\n  provider_reference text NULL,\n  onboarding_status text NOT NULL DEFAULT 'not-started',\n  transfer_capability_status text NOT NULL DEFAULT 'inactive',\n  payout_capability_status text NOT NULL DEFAULT 'inactive',\n  payout_destination_status text NOT NULL DEFAULT 'missing',\n  updated_at timestamptz NOT NULL,\n  last_stream_version bigint NOT NULL DEFAULT 0\n);\n\nCREATE INDEX IF NOT EXISTS settlement_payout_readiness_pages_status_idx\n  ON settlement_payout_readiness_pages (status, updated_at DESC, account_id DESC);\n\nCREATE INDEX IF NOT EXISTS settlement_payout_readiness_pages_provider_reference_idx\n  ON settlement_payout_readiness_pages (provider_reference)\n  WHERE provider_reference IS NOT NULL;\n\nALTER TABLE settlement_payout_readiness_pages\n  ADD COLUMN IF NOT EXISTS onboarding_status text NOT NULL DEFAULT 'not-started';\n\nALTER TABLE settlement_payout_readiness_pages\n  ADD COLUMN IF NOT EXISTS transfer_capability_status text NOT NULL DEFAULT 'inactive';\n\nALTER TABLE settlement_payout_readiness_pages\n  ADD COLUMN IF NOT EXISTS payout_capability_status text NOT NULL DEFAULT 'inactive';\n\nALTER TABLE settlement_payout_readiness_pages\n  ADD COLUMN IF NOT EXISTS payout_destination_status text NOT NULL DEFAULT 'missing';\n",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.account.agreement": "Account agreement",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.add.a.payout.destination": "Add a payout destination",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.add.a.payout.destination.description":
    "Open payout setup to add or confirm where eligible balances should be sent.",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.add.a.payout.account.using.the":
    "Add a payout account using the Chase Sets setup page before requesting payouts.",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.add.or.confirm.the.payout.destination":
    "Add or confirm the payout destination before requesting payouts.",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.business.profile": "Business profile",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.check.setup.status": "Check setup status",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.complete.requested.groups":
    "Open payout setup to complete: {groups}. Chase Sets does not store identity, tax, document, or bank details.",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.contact.support": "Contact support",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.continue.payout.setup": "Continue payout setup",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.continue.the.hosted.setup.flow.before":
    "Continue the Chase Sets setup page before requesting payouts.",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.eligible.seller.balances":
    "Eligible account balances can move to the connected payout account after fulfillment and settlement checks are complete.",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.future.requirements": "Upcoming payout requirements",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.future.requirements.description":
    "These details will be needed later, but they do not prevent payouts today.",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.finish.setup.to.receive.payouts":
    "Finish setup to receive payouts",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.fix.payout.setup": "Fix payout setup",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.identity.details": "Identity details",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.identity.and.business.details":
    "Identity and business details",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.in.progress": "In progress",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.in.review": "In review",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.last.checked": "Last checked: ",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.manage.payout.account": "Manage payout account",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.missing": "Missing",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.needs.attention": "Needs attention",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.needs.attention.2": "Needs attention",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.needs.attention.3": "Needs attention",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.not.checked.yet": "Not checked yet",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.not.started": "Not started",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.not.started.2": "Not started",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.not.started.3": "Not started",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.onboarding": "Onboarding: ",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.payout.account": "Payout account",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.payout.destination": "Payout destination: ",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.payout.setup.is.complete": "Payout setup is complete.",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.payout.setup.needs.attention":
    "Payout setup needs attention",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.payouts": "Payouts: ",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.payouts.are.ready": "Payouts are ready",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.platform.review": "Platform review",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.platform.review.description":
    "Contact support while Chase Sets reviews the payout provider configuration.",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.ready": "Ready",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.ready.2": "Ready",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.ready.3": "Ready",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.provider.review.in.progress":
    "Provider review is in progress",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.provider.review.in.progress.description":
    "No action is needed right now. Refresh setup status after the provider finishes review, or contact support if this state does not change.",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.requirement.group.count":
    "{count} detail(s) need attention.",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.requirement.group.needs.attention":
    "{group} needs attention",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.review.account.identity.or.business":
    "Review the requested account, identity, or business details in the secure setup form.",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.review.and.accept.the.required.account":
    "Review and accept the required account agreement in the secure setup form.",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.review.the.remaining.verification.details":
    "Review the remaining verification details in the secure setup form.",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.requirements.deadline": "Act by",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.restricted.past.due":
    "Required payout details are overdue. Complete them now to restore payouts.",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.restricted.paused":
    "The payout account is paused. Contact support to restore payouts.",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.restricted.rejected":
    "The payout provider cannot enable this account. Contact support for next steps.",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.restricted.review":
    "The payout provider is reviewing this account. No payout can be requested until the review finishes.",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.set.up.payouts": "Set up payouts",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.setup.can.continue": "Payout setup can continue",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.setup.can.continue.description":
    "Open the Chase Sets setup page to continue payout setup. If setup cannot load, retry from this page or contact support.",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.setup.details.need.attention":
    "Payout setup details need attention",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.setup.is.restricted": "Payout setup is restricted",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.setup.is.restricted.description":
    "Payouts cannot be requested until the provider restriction clears. Review payout setup in Chase Sets and contact support if the restriction remains.",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.some.payout.details.need.to.be":
    "Some payout details need to be corrected before payouts can be requested.",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.start.payout.setup": "Start payout setup",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.transfers": "Transfers: ",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.upcoming.requirements": "Upcoming requirements",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.verification.review": "Verification review",
  "settlement.features.payoutReadiness.ui.payoutReadinessPanel.what.needs.attention": "What needs attention",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.add.the.required.account.and":
    "Add the required account and payout destination details without leaving Chase Sets.",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.account.agreement": "Account agreement",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.back.to.payouts": "Back to payouts",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.complete.payout.setup": "Complete payout setup",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.connect.can.only.load.in.browser":
    "Secure payout setup can only load in the browser.",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.connect.failed.to.load":
    "Secure payout setup failed to load.",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.connect.loaded.without.initializer":
    "Secure payout setup loaded without an initializer.",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.current.status": "Current status",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.finish.payout.setup": "Finish payout setup",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.contact.support": "Contact support",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.identity.and.business.details":
    "Identity and business details",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.keep.your.payout.destination":
    "Keep payout destination and verification details current so eligible balances can move on time.",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.loading.secure.setup": "Loading secure payout setup",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.manage.details": "Manage details",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.manage.payout.details": "Manage payout details",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.in.progress": "In progress",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.needs.attention": "Needs attention",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.notification.banner.could.not.be.created":
    "Payout notifications could not be initialized.",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.notification.banner.aria.label": "Payout notifications",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.notification.banner.could.not.load":
    "Payout notifications could not load",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.notification.banner.could.not.load.detail":
    "The Stripe payout notification banner could not load.",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.notification.banner.loading": "Loading payout notifications",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.not.started": "Not started",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.payout.account": "Payout account",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.payout.notifications": "Payout notifications",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.platform.review": "Platform review — contact support",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.payout.setup": "Payout setup",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.payout.setup.is.complete.and.available":
    "Payout setup is complete and eligible balances can be requested after settlement checks.",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.payout.setup.is.ready": "Payout setup is ready.",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.payout.setup.needs.attention":
    "Payout setup needs attention.",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.payouts.are.ready": "Payouts are ready",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.provider.component.could.not.load":
    "The secure provider component could not load.",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.provider.setup.is.not.configured":
    "Secure provider setup is not configured for this environment.",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.refresh.setup.status": "Refresh setup status",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.ready": "Ready",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.request.payout": "Request payout",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.requirement.count": "{count} provider details available",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.requirement.group.count": "{count} detail(s) need attention",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.retry": "Retry",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.review.or.update.the.details":
    "Review or update payout details in the secure embedded provider form.",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.session.could.not.be.created":
    "A secure payout setup session could not be created.",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.session.was.missing.secret":
    "The secure payout setup session was missing its client secret.",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.settlement": "Settlement",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.setup.could.not.load": "Setup could not load",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.setup.load.failed.with.reason":
    "{reason} The setup session may have expired or the provider may be temporarily unavailable. Retry from Chase Sets; contact support if it still cannot load.",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.setup.session.may.have.expired":
    "The setup session may have expired or the provider may be temporarily unavailable. Retry from Chase Sets; contact support if it still cannot load.",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.setup.status.checked": "Setup status checked",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.setup.view": "Setup view",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.start.payout.setup": "Start payout setup",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.support.details": "Support details",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.verification.review": "Verification review",
  "settlement.features.payoutReadiness.ui.payoutSetupPage.you.can.request.payouts":
    "You can request payouts from eligible account balances.",
  "settlement.features.payouts.api.payoutBoundsPolicyRoute.authentication.context.missing":
    "Authentication context missing.",
  "settlement.features.payouts.api.payoutBoundsPolicyRoute.authentication.context.missing.2":
    "Authentication context missing.",
  "settlement.features.payouts.api.payoutBoundsPolicyRoute.authentication.required": "Authentication required.",
  "settlement.features.payouts.api.payoutBoundsPolicyRoute.document.not.found": "Policy document not found.",
  "settlement.features.payouts.api.payoutBoundsPolicyRoute.forbidden": "Forbidden.",
  "settlement.features.payouts.api.payoutBoundsPolicyRoute.request.failed": "Request failed.",
  "settlement.features.payouts.api.route.authentication.context.missing": "Authentication context missing.",
  "settlement.features.payouts.api.route.authentication.context.missing.2": "Authentication context missing.",
  "settlement.features.payouts.api.route.authentication.required": "Authentication required.",
  "settlement.features.payouts.api.route.forbidden": "Forbidden.",
  "settlement.features.payouts.api.route.job.not.found": "Payout reconciliation job not found.",
  "settlement.features.payouts.api.route.payout.not.found": "Payout not found.",
  "settlement.features.payouts.api.route.request.failed": "Request failed.",
  "settlement.features.payouts.api.route.step.up.required":
    "Confirm it is you before requesting a payout, or wait until the payout destination cooling period ends.",
  "settlement.features.payouts.api.runtime.payout.completed": "Payout completed",
  "settlement.features.payouts.api.runtime.payout.failed": "Payout failed",
  "settlement.features.payouts.api.runtime.payout.requested": "Payout requested",
  "settlement.features.payouts.api.runtime.provider.operation.submitted": "Provider operation submitted",
  "settlement.features.payouts.api.runtime.provider.payout.submission.failed": "Provider payout submission failed.",
  "settlement.features.payouts.api.runtime.provider.payout.submission.failed.2": "Provider payout submission failed.",
  "settlement.features.payouts.api.runtime.provider.payout.submission.failed.3": "Provider payout submission failed.",
  "settlement.features.payouts.api.runtime.provider.payout.submitted": "Provider payout submitted",
  "settlement.features.payouts.api.runtime.reconciliation.failed": "Reconciliation failed.",
  "settlement.features.payouts.api.runtime.wallet.credited": "Wallet credited",
  "settlement.features.payouts.api.runtime.wallet.debited": "Wallet debited",
  "settlement.features.payouts.readModel.queries.and.provider.payout.reference.is.null":
    "AND provider_payout_reference IS NULL",
  "settlement.features.payouts.readModel.queries.and.status.failed": "AND status = 'failed'",
  "settlement.features.payouts.readModel.queries.and.status.in.transit": "AND status = 'in-transit'",
  "settlement.features.payouts.readModel.queries.and.status.requested.and.updated.at":
    "AND status = 'requested' AND updated_at < NOW() - INTERVAL '15 minutes'",
  "settlement.features.payouts.readModel.queries.select.payout.id.account.id.amount":
    "\n  SELECT\n    payout_id,\n    account_id,\n    amount::text AS amount,\n    currency_code,\n    destination_reference,\n    note,\n    status,\n    provider_transfer_reference,\n    provider_payout_reference,\n    provider_status,\n    provider_failure_code,\n    provider_failure_message,\n    requested_at,\n    updated_at,\n    sent_at,\n    completed_at,\n    failed_at,\n    failure_reason,\n    last_provider_event_at,\n    last_reconciled_at,\n    retry_count,\n    next_retry_at,\n    retry_reason\n  FROM settlement_payout_pages\n",
  "settlement.features.payouts.readModel.schema.create.table.if.not.exists.settlement":
    "\nCREATE TABLE IF NOT EXISTS settlement_payout_pages (\n  payout_id text PRIMARY KEY,\n  account_id text NOT NULL,\n  amount numeric(12, 2) NOT NULL,\n  currency_code text NOT NULL,\n  destination_reference text NULL,\n  note text NULL,\n  status text NOT NULL,\n  provider_transfer_reference text NULL,\n  provider_payout_reference text NULL,\n  provider_status text NULL,\n  provider_failure_code text NULL,\n  provider_failure_message text NULL,\n  requested_at timestamptz NOT NULL,\n  updated_at timestamptz NOT NULL,\n  sent_at timestamptz NULL,\n  completed_at timestamptz NULL,\n  failed_at timestamptz NULL,\n  failure_reason text NULL,\n  last_provider_event_at timestamptz NULL,\n  last_reconciled_at timestamptz NULL,\n  retry_count integer NOT NULL DEFAULT 0,\n  next_retry_at timestamptz NULL,\n  retry_reason text NULL,\n  last_stream_version bigint NOT NULL DEFAULT 0\n);\n\nCREATE INDEX IF NOT EXISTS settlement_payout_pages_account_idx\n  ON settlement_payout_pages (account_id, updated_at DESC, payout_id DESC);\n\nCREATE INDEX IF NOT EXISTS settlement_payout_pages_provider_payout_idx\n  ON settlement_payout_pages (provider_payout_reference)\n  WHERE provider_payout_reference IS NOT NULL;\n\nCREATE TABLE IF NOT EXISTS settlement_money_movement_webhook_events (\n  provider_event_id text PRIMARY KEY,\n  provider_name text NOT NULL,\n  event_kind text NOT NULL,\n  provider_object_reference text NULL,\n  received_at timestamptz NOT NULL\n);\n\nCREATE INDEX IF NOT EXISTS settlement_money_movement_webhook_events_received_idx\n  ON settlement_money_movement_webhook_events (received_at DESC);\n\nCREATE TABLE IF NOT EXISTS settlement_provider_idempotency_keys (\n  operation_key text PRIMARY KEY,\n  provider_name text NOT NULL,\n  operation_kind text NOT NULL,\n  account_id text NULL,\n  payout_id text NULL,\n  provider_object_reference text NULL,\n  idempotency_key text NOT NULL,\n  created_at timestamptz NOT NULL\n);\n\nCREATE INDEX IF NOT EXISTS settlement_provider_idempotency_keys_account_idx\n  ON settlement_provider_idempotency_keys (account_id, created_at DESC)\n  WHERE account_id IS NOT NULL;\n\nCREATE TABLE IF NOT EXISTS settlement_reconciliation_runs (\n  reconciliation_run_id text PRIMARY KEY,\n  kind text NOT NULL,\n  checked_count integer NOT NULL,\n  reconciled_count integer NOT NULL,\n  ignored_count integer NOT NULL,\n  skipped_count integer NOT NULL,\n  error_count integer NOT NULL,\n  status text NOT NULL,\n  summary jsonb NOT NULL DEFAULT '{}'::jsonb,\n  started_at timestamptz NOT NULL,\n  completed_at timestamptz NOT NULL\n);\n\nCREATE INDEX IF NOT EXISTS settlement_reconciliation_runs_completed_idx\n  ON settlement_reconciliation_runs (completed_at DESC);\n\nALTER TABLE settlement_payout_pages\n  ADD COLUMN IF NOT EXISTS provider_transfer_reference text NULL;\n\nALTER TABLE settlement_payout_pages\n  ADD COLUMN IF NOT EXISTS provider_payout_reference text NULL;\n\nALTER TABLE settlement_payout_pages\n  ADD COLUMN IF NOT EXISTS provider_status text NULL;\n\nALTER TABLE settlement_payout_pages\n  ADD COLUMN IF NOT EXISTS provider_failure_code text NULL;\n\nALTER TABLE settlement_payout_pages\n  ADD COLUMN IF NOT EXISTS provider_failure_message text NULL;\n\nALTER TABLE settlement_payout_pages\n  ADD COLUMN IF NOT EXISTS last_provider_event_at timestamptz NULL;\n\nALTER TABLE settlement_payout_pages\n  ADD COLUMN IF NOT EXISTS last_reconciled_at timestamptz NULL;\n\nALTER TABLE settlement_payout_pages\n  ADD COLUMN IF NOT EXISTS retry_count integer NOT NULL DEFAULT 0;\n\nALTER TABLE settlement_payout_pages\n  ADD COLUMN IF NOT EXISTS next_retry_at timestamptz NULL;\n\nALTER TABLE settlement_payout_pages\n  ADD COLUMN IF NOT EXISTS retry_reason text NULL;\n\nALTER TABLE settlement_money_movement_webhook_events\n  ADD COLUMN IF NOT EXISTS event_kind text NULL;\n\nALTER TABLE settlement_money_movement_webhook_events\n  ADD COLUMN IF NOT EXISTS provider_object_reference text NULL;\n\nDO $$\nBEGIN\n  IF EXISTS (\n    SELECT 1\n    FROM information_schema.columns\n    WHERE table_name = 'settlement_payout_pages'\n      AND column_name = 'scheduled_at'\n  ) AND NOT EXISTS (\n    SELECT 1\n    FROM information_schema.columns\n    WHERE table_name = 'settlement_payout_pages'\n      AND column_name = 'requested_at'\n  ) THEN\n    ALTER TABLE settlement_payout_pages RENAME COLUMN scheduled_at TO requested_at;\n  END IF;\nEND $$;\n",
  "settlement.features.payouts.ui.moneyHealthPage.action": "Action",
  "settlement.features.payouts.ui.moneyHealthPage.adapter": "Adapter",
  "settlement.features.payouts.ui.moneyHealthPage.amount": "Amount",
  "settlement.features.payouts.ui.moneyHealthPage.checked.before.payout": "Checked before payout",
  "settlement.features.payouts.ui.moneyHealthPage.completed": "Completed",
  "settlement.features.payouts.ui.moneyHealthPage.connected.payouts": "Connected payouts",
  "settlement.features.payouts.ui.moneyHealthPage.counts": "Counts",
  "settlement.features.payouts.ui.moneyHealthPage.enabled.through.adapter": "Enabled through adapter",
  "settlement.features.payouts.ui.moneyHealthPage.forecast.after.pending.demand": "Forecast after pending demand: ",
  "settlement.features.payouts.ui.moneyHealthPage.local.only": "Local only",
  "settlement.features.payouts.ui.moneyHealthPage.local.test.adapter": "Local test adapter",
  "settlement.features.payouts.ui.moneyHealthPage.money.health": "Money Health",
  "settlement.features.payouts.ui.moneyHealthPage.account": "Account",
  "settlement.features.payouts.ui.moneyHealthPage.accounts.with.negative.wallet.balances.appear":
    "Accounts with negative wallet balances appear here.",
  "settlement.features.payouts.ui.moneyHealthPage.collections": "Collections",
  "settlement.features.payouts.ui.moneyHealthPage.negative.amount": "Negative amount",
  "settlement.features.payouts.ui.moneyHealthPage.negative.balance.accounts": "Negative Balance Accounts",
  "settlement.features.payouts.ui.moneyHealthPage.negative.since": "Negative since",
  "settlement.features.payouts.ui.moneyHealthPage.no.negative.balances": "No negative balances",
  "settlement.features.payouts.ui.moneyHealthPage.no.payout.issues": "No payout issues",
  "settlement.features.payouts.ui.moneyHealthPage.no.reconciliation.runs": "No reconciliation runs",
  "settlement.features.payouts.ui.moneyHealthPage.open": "Open",
  "settlement.features.payouts.ui.moneyHealthPage.operations": "Operations",
  "settlement.features.payouts.ui.moneyHealthPage.payout": "Payout",
  "settlement.features.payouts.ui.moneyHealthPage.payout.operations": "Payout operations",
  "settlement.features.payouts.ui.moneyHealthPage.payouts.needing.attention": "Payouts Needing Attention",
  "settlement.features.payouts.ui.moneyHealthPage.payouts.that.need.reconciliation.or.review":
    "Payouts that need reconciliation or review appear here.",
  "settlement.features.payouts.ui.moneyHealthPage.pending.payout.demand": "Pending payout demand: ",
  "settlement.features.payouts.ui.moneyHealthPage.platform.balance": "Platform balance",
  "settlement.features.payouts.ui.moneyHealthPage.platform.balance.forecast": "Platform Balance Forecast",
  "settlement.features.payouts.ui.moneyHealthPage.provider.balance": "Provider balance",
  "settlement.features.payouts.ui.moneyHealthPage.provider.diagnostics": "Provider Diagnostics",
  "settlement.features.payouts.ui.moneyHealthPage.recent.reconciliation": "Recent Reconciliation",
  "settlement.features.payouts.ui.moneyHealthPage.required": "Required",
  "settlement.features.payouts.ui.moneyHealthPage.review.payout.demand.reconciliation.history.and":
    "Review payout demand, reconciliation history, and provider-facing money movement signals.",
  "settlement.features.payouts.ui.moneyHealthPage.scheduled.and.manual.reconciliation.outcomes.appear":
    "Scheduled and manual reconciliation outcomes appear here.",
  "settlement.features.payouts.ui.moneyHealthPage.signal": "Signal",
  "settlement.features.payouts.ui.moneyHealthPage.status": "Status",
  "settlement.features.payouts.ui.moneyHealthPage.status.2": "Status",
  "settlement.features.payouts.ui.moneyHealthPage.unavailable": "Unavailable",
  "settlement.features.payouts.ui.moneyHealthPage.unavailable.2": "Unavailable",
  "settlement.features.payouts.ui.moneyHealthPage.updated": "Updated",
  "settlement.features.payouts.ui.moneyHealthPage.webhook.signatures": "Webhook signatures",
  "settlement.features.payouts.ui.moneyHealthPage.webhook.failure.classes": "Webhook failure classes",
  "settlement.features.payouts.ui.payoutDetailPage.account": "Account: ",
  "settlement.features.payouts.ui.payoutDetailPage.amount": "Amount: ",
  "settlement.features.payouts.ui.payoutDetailPage.back.to.payouts": "Back to payouts",
  "settlement.features.payouts.ui.payoutDetailPage.completed": "Completed: ",
  "settlement.features.payouts.ui.payoutDetailPage.contact.support": "Contact support",
  "settlement.features.payouts.ui.payoutDetailPage.failed": "Failed: ",
  "settlement.features.payouts.ui.payoutDetailPage.internal.payout": "Internal payout: ",
  "settlement.features.payouts.ui.payoutDetailPage.is.being.sent.to.your.saved":
    "is being sent to your saved payout account. Most payouts arrive in 1-3 business days after provider acceptance.",
  "settlement.features.payouts.ui.payoutDetailPage.last.provider.check": "Last provider check: ",
  "settlement.features.payouts.ui.payoutDetailPage.last.provider.event": "Last provider event: ",
  "settlement.features.payouts.ui.payoutDetailPage.last.reconciliation.check": "Last reconciliation check: ",
  "settlement.features.payouts.ui.payoutDetailPage.missing": "Missing",
  "settlement.features.payouts.ui.payoutDetailPage.missing.2": "Missing",
  "settlement.features.payouts.ui.payoutDetailPage.needs.attention": "Needs attention",
  "settlement.features.payouts.ui.payoutDetailPage.needs.attention.2": "Needs attention",
  "settlement.features.payouts.ui.payoutDetailPage.no.retry.scheduled": "No retry scheduled",
  "settlement.features.payouts.ui.payoutDetailPage.none": "None",
  "settlement.features.payouts.ui.payoutDetailPage.none.2": "None",
  "settlement.features.payouts.ui.payoutDetailPage.note": "Note: ",
  "settlement.features.payouts.ui.payoutDetailPage.on.the.way": "On the way",
  "settlement.features.payouts.ui.payoutDetailPage.paid": "Paid",
  "settlement.features.payouts.ui.payoutDetailPage.paid.2": "Paid",
  "settlement.features.payouts.ui.payoutDetailPage.payout.account.needs.review": "Payout account needs review",
  "settlement.features.payouts.ui.payoutDetailPage.payout.account.saved.payout.account":
    "Payout account: Saved payout account",
  "settlement.features.payouts.ui.payoutDetailPage.payout.requested": "Payout Requested",
  "settlement.features.payouts.ui.payoutDetailPage.payout.requested.2": "Payout requested",
  "settlement.features.payouts.ui.payoutDetailPage.payout.submitted": "Payout submitted",
  "settlement.features.payouts.ui.payoutDetailPage.provider.event.received": "Provider event received",
  "settlement.features.payouts.ui.payoutDetailPage.provider.failure.code": "Provider failure code: ",
  "settlement.features.payouts.ui.payoutDetailPage.provider.message": "Provider message: ",
  "settlement.features.payouts.ui.payoutDetailPage.provider.payout": "Provider payout: ",
  "settlement.features.payouts.ui.payoutDetailPage.provider.reported.a.payout.failure":
    "Provider reported a payout failure.",
  "settlement.features.payouts.ui.payoutDetailPage.provider.status": "Provider status: ",
  "settlement.features.payouts.ui.payoutDetailPage.provider.status.2": "Provider status: ",
  "settlement.features.payouts.ui.payoutDetailPage.provider.transfer": "Provider transfer: ",
  "settlement.features.payouts.ui.payoutDetailPage.provider.transfer.submitted": "Provider transfer submitted",
  "settlement.features.payouts.ui.payoutDetailPage.reason": "Reason: ",
  "settlement.features.payouts.ui.payoutDetailPage.requested": "Requested",
  "settlement.features.payouts.ui.payoutDetailPage.requested.2": "Requested: ",
  "settlement.features.payouts.ui.payoutDetailPage.requested.3": "Requested",
  "settlement.features.payouts.ui.payoutDetailPage.review.payout.details": "Review payout details",
  "settlement.features.payouts.ui.payoutDetailPage.review.payout.details.before.requesting":
    "The payout did not reach the saved payout account. Review payout details in Chase Sets before requesting another payout, and contact support if the same details fail again.",
  "settlement.features.payouts.ui.payoutDetailPage.retry.policy": "Retry policy: ",
  "settlement.features.payouts.ui.payoutDetailPage.retry.reason": "Retry reason: ",
  "settlement.features.payouts.ui.payoutDetailPage.sent": "Sent: ",
  "settlement.features.payouts.ui.payoutDetailPage.settlement": "Settlement",
  "settlement.features.payouts.ui.payoutDetailPage.status.timeline": "Status Timeline",
  "settlement.features.payouts.ui.payoutDetailPage.summary": "Summary",
  "settlement.features.payouts.ui.payoutDetailPage.support.details": "Support Details",
  "settlement.features.payouts.ui.payoutDetailPage.unknown": "Unknown",
  "settlement.features.payouts.ui.payoutDetailPage.usually.1.3.business.days.after":
    "Usually 1-3 business days after provider acceptance",
  "settlement.features.payouts.ui.payoutDetailPage.waiting.for.provider.event": "Waiting for provider event",
  "settlement.features.payouts.ui.payoutDetailPage.waiting.for.provider.payout.reference":
    "Waiting for provider payout reference",
  "settlement.features.payouts.ui.payoutDetailPage.waiting.for.provider.transfer.reference":
    "Waiting for provider transfer reference",
  "settlement.features.payouts.ui.payoutListPage.actions": "Actions",
  "settlement.features.payouts.ui.payoutListPage.after.provider.submission": "After provider submission",
  "settlement.features.payouts.ui.payoutListPage.amount": "Amount: ",
  "settlement.features.payouts.ui.payoutListPage.amount.2": "Amount",
  "settlement.features.payouts.ui.payoutListPage.amount.3": "Amount",
  "settlement.features.payouts.ui.payoutListPage.and": "and ",
  "settlement.features.payouts.ui.payoutListPage.arrival.is.usually.1.3.business":
    ". Arrival is usually 1-3 business days after the provider accepts the payout.",
  "settlement.features.payouts.ui.payoutListPage.available.after.request": "Available after request: ",
  "settlement.features.payouts.ui.payoutListPage.available.to.request": "Available to request: ",
  "settlement.features.payouts.ui.payoutListPage.back.to.edit": "Back to edit",
  "settlement.features.payouts.ui.payoutListPage.before.this.can.be.requested": "Before this can be requested",
  "settlement.features.payouts.ui.payoutListPage.confirm.payout": "Confirm payout",
  "settlement.features.payouts.ui.payoutListPage.confirm.payout.request": "Confirm payout request",
  "settlement.features.payouts.ui.payoutListPage.estimated.arrival": "Estimated arrival",
  "settlement.features.payouts.ui.payoutListPage.estimated.arrival.usually.1.3.business":
    "Estimated arrival: Usually 1-3 business days after provider acceptance",
  "settlement.features.payouts.ui.payoutListPage.full.available": "Full available ",
  "settlement.features.payouts.ui.payoutListPage.if.the.provider.cannot.complete.the":
    "If the provider cannot complete the transfer or payout, the wallet is credited back automatically.",
  "settlement.features.payouts.ui.payoutListPage.minimum": "Minimum ",
  "settlement.features.payouts.ui.payoutListPage.needs.attention": "Needs attention",
  "settlement.features.payouts.ui.payoutListPage.no.payouts.yet": "No payouts yet",
  "settlement.features.payouts.ui.payoutListPage.not.sent": "Not sent",
  "settlement.features.payouts.ui.payoutListPage.note": "Note: ",
  "settlement.features.payouts.ui.payoutListPage.note.2": "Note",
  "settlement.features.payouts.ui.payoutListPage.note.summary": "Note: {note}",
  "settlement.features.payouts.ui.payoutListPage.on.the.way": "On the way",
  "settlement.features.payouts.ui.payoutListPage.n.payouts.need.attention": "{count} payouts need attention",
  "settlement.features.payouts.ui.payoutListPage.open": "Open",
  "settlement.features.payouts.ui.payoutListPage.optional.memo": "Optional memo",
  "settlement.features.payouts.ui.payoutListPage.paid": "Paid",
  "settlement.features.payouts.ui.payoutListPage.payout.account": "Payout account",
  "settlement.features.payouts.ui.payoutListPage.payout.account.saved.payout.account":
    "Payout account: Saved payout account",
  "settlement.features.payouts.ui.payoutListPage.payout.history": "Payout History",
  "settlement.features.payouts.ui.payoutListPage.payouts": "Payouts",
  "settlement.features.payouts.ui.payoutListPage.payouts.must.be.between": "Payouts must be between ",
  "settlement.features.payouts.ui.payoutListPage.preview.payout": "Preview payout",
  "settlement.features.payouts.ui.payoutListPage.request.a.payout.from.your.available":
    "Request a payout from your available balance to get started.",
  "settlement.features.payouts.ui.payoutListPage.request.payout": "Request Payout",
  "settlement.features.payouts.ui.payoutListPage.request.payouts.from.your.available.wallet":
    "Request payouts from your available wallet balance to your saved payout account.",
  "settlement.features.payouts.ui.payoutListPage.requested": "Requested",
  "settlement.features.payouts.ui.payoutListPage.requested.2": "Requested",
  "settlement.features.payouts.ui.payoutListPage.saved.payout.account": "Saved payout account",
  "settlement.features.payouts.ui.payoutListPage.settlement": "Settlement",
  "settlement.features.payouts.ui.payoutListPage.setup.checked": "Setup checked",
  "settlement.features.payouts.ui.payoutListPage.status": "Status",
  "settlement.features.payouts.ui.payoutListPage.typically.1.3.business.days": "Typically 1-3 business days",
  "settlement.features.payouts.ui.payoutListPage.view.wallet": "View wallet",
  "settlement.features.payouts.ui.payoutListPage.why.payouts.are.unavailable": "Why payouts are unavailable",
  "settlement.features.payouts.ui.payoutOperationsPage.actions": "Actions",
  "settlement.features.payouts.ui.payoutOperationsPage.all": "All",
  "settlement.features.payouts.ui.payoutOperationsPage.amount": "Amount",
  "settlement.features.payouts.ui.payoutOperationsPage.account.cannot.request.payouts.until":
    "Account payouts stay blocked until setup is ready",
  "settlement.features.payouts.ui.payoutOperationsPage.checked": "Checked ",
  "settlement.features.payouts.ui.payoutOperationsPage.confirmed": "Confirmed",
  "settlement.features.payouts.ui.payoutOperationsPage.count": "Count",
  "settlement.features.payouts.ui.payoutOperationsPage.created": "Created",
  "settlement.features.payouts.ui.payoutOperationsPage.current.payout.setup.status":
    "Current setup state used by payout request checks",
  "settlement.features.payouts.ui.payoutOperationsPage.destination.status.from.provider":
    "Support-safe payout destination status from the provider",
  "settlement.features.payouts.ui.payoutOperationsPage.errors": "Errors: ",
  "settlement.features.payouts.ui.payoutOperationsPage.external.status": "External status",
  "settlement.features.payouts.ui.payoutOperationsPage.failed": "Failed",
  "settlement.features.payouts.ui.payoutOperationsPage.failed.payouts": "Failed payouts",
  "settlement.features.payouts.ui.payoutOperationsPage.idempotency.key": "Retry protection",
  "settlement.features.payouts.ui.payoutOperationsPage.ignored": "; ignored ",
  "settlement.features.payouts.ui.payoutOperationsPage.in.transit": "In transit",
  "settlement.features.payouts.ui.payoutOperationsPage.last.checked": "Last checked: ",
  "settlement.features.payouts.ui.payoutOperationsPage.last.checked.2": "Last checked",
  "settlement.features.payouts.ui.payoutOperationsPage.meaning": "Meaning",
  "settlement.features.payouts.ui.payoutOperationsPage.missing": "Missing",
  "settlement.features.payouts.ui.payoutOperationsPage.missing.provider.references": "Missing provider references",
  "settlement.features.payouts.ui.payoutOperationsPage.missing.reference": "Missing reference",
  "settlement.features.payouts.ui.payoutOperationsPage.money.health": "Money health",
  "settlement.features.payouts.ui.payoutOperationsPage.monitor.payouts.that.may.need.provider":
    "Monitor payouts that may need provider reconciliation or support review.",
  "settlement.features.payouts.ui.payoutOperationsPage.needs.attention": "Needs Attention",
  "settlement.features.payouts.ui.payoutOperationsPage.needs.provider.reference": "Needs provider reference",
  "settlement.features.payouts.ui.payoutOperationsPage.no.payouts.need.attention": "No payouts need attention",
  "settlement.features.payouts.ui.payoutOperationsPage.no.provider.attempts": "No provider attempts",
  "settlement.features.payouts.ui.payoutOperationsPage.none": "None",
  "settlement.features.payouts.ui.payoutOperationsPage.not.checked": "Not checked",
  "settlement.features.payouts.ui.payoutOperationsPage.not.checked.in.this.session": "Not checked in this session",
  "settlement.features.payouts.ui.payoutOperationsPage.older.than.15.minutes": "Older than 15 minutes",
  "settlement.features.payouts.ui.payoutOperationsPage.open": "Open",
  "settlement.features.payouts.ui.payoutOperationsPage.open.setup.requirements": "Open setup requirements",
  "settlement.features.payouts.ui.payoutOperationsPage.operation": "Operation",
  "settlement.features.payouts.ui.payoutOperationsPage.payout": "Payout",
  "settlement.features.payouts.ui.payoutOperationsPage.payout.confirmation": "Payout confirmation",
  "settlement.features.payouts.ui.payoutOperationsPage.payout.destination": "Payout destination",
  "settlement.features.payouts.ui.payoutOperationsPage.payout.operations": "Payout Operations",
  "settlement.features.payouts.ui.payoutOperationsPage.payout.setup.blocked": "Payout setup blocked",
  "settlement.features.payouts.ui.payoutOperationsPage.payout.setup.status.needs.refresh":
    "Ready setup older than 30 days should be refreshed before release promotion",
  "settlement.features.payouts.ui.payoutOperationsPage.pending": "Pending",
  "settlement.features.payouts.ui.payoutOperationsPage.pending.confirmation": "Pending confirmation",
  "settlement.features.payouts.ui.payoutOperationsPage.provider.payout": "Provider payout",
  "settlement.features.payouts.ui.payoutOperationsPage.provider.payout.reconciliation.is.clear":
    "Provider payout reconciliation is clear.",
  "settlement.features.payouts.ui.payoutOperationsPage.provider.readiness.snapshot":
    "Last support-safe provider readiness snapshot",
  "settlement.features.payouts.ui.payoutOperationsPage.provider.reference": "Provider reference",
  "settlement.features.payouts.ui.payoutOperationsPage.provider.requirement.groups.remain.open":
    "Provider requirement groups remain open",
  "settlement.features.payouts.ui.payoutOperationsPage.provider.reported.failure.or.internal.submission":
    "A payout needs review before it can continue",
  "settlement.features.payouts.ui.payoutOperationsPage.provider.status": "Provider status",
  "settlement.features.payouts.ui.payoutOperationsPage.provider.status.has.not.been.checked":
    "Status has not been checked recently",
  "settlement.features.payouts.ui.payoutOperationsPage.provider.transfer.and.payout.submissions.appear":
    "Provider transfer and payout submissions appear here.",
  "settlement.features.payouts.ui.payoutOperationsPage.protected.retry.key": "Protected retry key",
  "settlement.features.payouts.ui.payoutOperationsPage.ready.to.reconcile": "Ready to reconcile",
  "settlement.features.payouts.ui.payoutOperationsPage.recent.provider.attempts": "Recent Provider Attempts",
  "settlement.features.payouts.ui.payoutOperationsPage.reconciled": "; reconciled ",
  "settlement.features.payouts.ui.payoutOperationsPage.reconciliation": "Reconciliation",
  "settlement.features.payouts.ui.payoutOperationsPage.retry.policy": "Retry policy",
  "settlement.features.payouts.ui.payoutOperationsPage.retry.policy.has.a.next.provider":
    "The next retry is already scheduled",
  "settlement.features.payouts.ui.payoutOperationsPage.retry.queued": "Retry queued",
  "settlement.features.payouts.ui.payoutOperationsPage.retry.queued.2": "Retry queued",
  "settlement.features.payouts.ui.payoutOperationsPage.review": "Review",
  "settlement.features.payouts.ui.payoutOperationsPage.run.reconciliation": "Run reconciliation",
  "settlement.features.payouts.ui.payoutOperationsPage.settlement": "Settlement",
  "settlement.features.payouts.ui.payoutOperationsPage.setup.last.updated": "Setup last updated",
  "settlement.features.payouts.ui.payoutOperationsPage.setup.status": "Setup status",
  "settlement.features.payouts.ui.payoutOperationsPage.signal": "Signal",
  "settlement.features.payouts.ui.payoutOperationsPage.skipped": "; skipped ",
  "settlement.features.payouts.ui.payoutOperationsPage.stale.payout.setup.status": "Stale payout setup status",
  "settlement.features.payouts.ui.payoutOperationsPage.stale.reconciliation": "Stale reconciliation",
  "settlement.features.payouts.ui.payoutOperationsPage.status": "Status",
  "settlement.features.payouts.ui.payoutOperationsPage.transfer.or.payout.reference.has.not":
    "A payout confirmation is still pending",
  "settlement.features.payouts.ui.payoutOperationsPage.unknown": "Unknown",
  "settlement.features.payouts.ui.payoutOperationsPage.updated": "Updated",
  "settlement.features.wallets.api.clearancePolicyRoute.authentication.context.missing":
    "Authentication context missing.",
  "settlement.features.wallets.api.clearancePolicyRoute.authentication.context.missing.2":
    "Authentication context missing.",
  "settlement.features.wallets.api.clearancePolicyRoute.authentication.required": "Authentication required.",
  "settlement.features.wallets.api.clearancePolicyRoute.document.not.found": "Policy document not found.",
  "settlement.features.wallets.api.clearancePolicyRoute.forbidden": "Forbidden.",
  "settlement.features.wallets.api.clearancePolicyRoute.request.failed": "Request failed.",
  "settlement.features.wallets.api.route.adjustment.failed": "Adjustment failed.",
  "settlement.features.wallets.api.route.authentication.context.missing": "Authentication context missing.",
  "settlement.features.wallets.api.route.authentication.context.missing.2": "Authentication context missing.",
  "settlement.features.wallets.api.route.authentication.context.missing.3": "Authentication context missing.",
  "settlement.features.wallets.api.route.authentication.context.missing.4": "Authentication context missing.",
  "settlement.features.wallets.api.route.authentication.required": "Authentication required.",
  "settlement.features.wallets.api.route.dispute.hold": "Dispute hold",
  "settlement.features.wallets.api.route.dispute.hold.failed": "Dispute hold failed.",
  "settlement.features.wallets.api.route.dispute.hold.released": "Dispute hold released",
  "settlement.features.wallets.api.route.dispute.release.failed": "Dispute release failed.",
  "settlement.features.wallets.api.route.forbidden": "Forbidden.",
  "settlement.features.wallets.api.route.refund.debit.failed": "Refund debit failed.",
  "settlement.features.wallets.api.route.seller.refund.adjustment": "Seller refund adjustment",
  "settlement.features.wallets.api.wallet-adjustment-route.account.not-found": "Wallet account not found.",
  "settlement.features.wallets.api.wallet-adjustment-route.adjustment.not-found": "Wallet adjustment not found.",
  "settlement.features.wallets.api.wallet-adjustment-route.authentication.context.missing":
    "Authentication context missing.",
  "settlement.features.wallets.api.wallet-adjustment-route.authentication.required": "Authentication required.",
  "settlement.features.wallets.api.wallet-adjustment-route.command.failed": "Wallet adjustment command failed.",
  "settlement.features.wallets.api.wallet-adjustment-route.forbidden": "Forbidden.",
  "settlement.features.wallets.api.wallet-adjustment-route.step.up.required":
    "Confirm it is you before posting, approving, or reversing this Wallet Adjustment.",
  "settlement.features.wallets.integrations.paymentSource.paymentSourceSchema.create.table.if.not.exists.settlement":
    "\nCREATE TABLE IF NOT EXISTS settlement_payment_sources (\n  payment_id text PRIMARY KEY,\n  buyer_account_id text NOT NULL,\n  order_ids jsonb NOT NULL DEFAULT '[]'::jsonb,\n  amount numeric(12,2) NOT NULL,\n  balance_credit_amount numeric(12,2) NOT NULL DEFAULT 0,\n  processor_amount numeric(12,2) NOT NULL DEFAULT 0,\n  currency_code text NOT NULL,\n  processor_name text NOT NULL,\n  processor_payment_reference text NOT NULL,\n  processor_status text NOT NULL,\n  status text NOT NULL,\n  failure_code text NULL,\n  failure_message text NULL,\n  created_at timestamptz NOT NULL,\n  updated_at timestamptz NOT NULL,\n  captured_at timestamptz NULL,\n  failed_at timestamptz NULL,\n  cancelled_at timestamptz NULL,\n  last_stream_version integer NOT NULL DEFAULT 0\n);\n\nCREATE INDEX IF NOT EXISTS settlement_payment_sources_status_idx\n  ON settlement_payment_sources (status, updated_at DESC, payment_id DESC);\n\nALTER TABLE settlement_payment_sources\n  ADD COLUMN IF NOT EXISTS balance_credit_amount numeric(12,2) NOT NULL DEFAULT 0;\n\nALTER TABLE settlement_payment_sources\n  ADD COLUMN IF NOT EXISTS processor_amount numeric(12,2) NOT NULL DEFAULT 0;\n\nCREATE TABLE IF NOT EXISTS settlement_refund_sources (\n  refund_id text PRIMARY KEY,\n  payment_id text NOT NULL,\n  order_ids jsonb NOT NULL DEFAULT '[]'::jsonb,\n  amount numeric(12,2) NOT NULL,\n  currency_code text NOT NULL,\n  reason text NOT NULL,\n  processor_name text NOT NULL,\n  processor_status text NOT NULL,\n  processor_refund_reference text NULL,\n  status text NOT NULL,\n  failure_code text NULL,\n  failure_message text NULL,\n  requested_at timestamptz NOT NULL,\n  updated_at timestamptz NOT NULL,\n  issued_at timestamptz NULL,\n  failed_at timestamptz NULL,\n  last_stream_version integer NOT NULL DEFAULT 0\n);\n\nCREATE INDEX IF NOT EXISTS settlement_refund_sources_payment_idx\n  ON settlement_refund_sources (payment_id, updated_at DESC, refund_id DESC);\n\nCREATE INDEX IF NOT EXISTS settlement_refund_sources_status_idx\n  ON settlement_refund_sources (status, updated_at DESC, refund_id DESC);\n",
  "settlement.features.wallets.readModel.queries.select.account.id.currency.code.pending":
    "\n  SELECT\n    account_id,\n    currency_code,\n    pending_balance_amount::text AS pending_balance_amount,\n    available_balance_amount::text AS available_balance_amount,\n    total_credited_amount::text AS total_credited_amount,\n    total_debited_amount::text AS total_debited_amount,\n    opened_at,\n    updated_at\n  FROM settlement_wallet_pages\n",
  "settlement.features.wallets.readModel.schema.create.table.if.not.exists.settlement":
    "\nCREATE TABLE IF NOT EXISTS settlement_wallet_pages (\n  account_id text PRIMARY KEY,\n  currency_code text NOT NULL,\n  pending_balance_amount numeric(12, 2) NOT NULL,\n  available_balance_amount numeric(12, 2) NOT NULL,\n  total_credited_amount numeric(12, 2) NOT NULL,\n  total_debited_amount numeric(12, 2) NOT NULL,\n  opened_at timestamptz NULL,\n  updated_at timestamptz NULL\n);\n\nCREATE TABLE IF NOT EXISTS settlement_ledger_entry_pages (\n  ledger_entry_id text PRIMARY KEY,\n  account_id text NOT NULL REFERENCES settlement_wallet_pages (account_id) ON DELETE CASCADE,\n  kind text NOT NULL,\n  direction text NOT NULL,\n  amount numeric(12, 2) NOT NULL,\n  currency_code text NOT NULL,\n  funds_status text NOT NULL,\n  order_id text NULL,\n  payment_id text NULL,\n  payout_id text NULL,\n  description text NULL,\n  posted_at timestamptz NOT NULL,\n  available_at timestamptz NULL,\n  updated_at timestamptz NOT NULL\n);\n\nCREATE INDEX IF NOT EXISTS settlement_ledger_entry_pages_account_idx\n  ON settlement_ledger_entry_pages (account_id, posted_at DESC, ledger_entry_id DESC);\n\nCREATE INDEX IF NOT EXISTS settlement_ledger_entry_pages_payout_idx\n  ON settlement_ledger_entry_pages (payout_id)\n  WHERE payout_id IS NOT NULL;\n",
  "settlement.features.wallets.ui.walletPage.add.or.verify.a.payout.destination": "Add or verify a payout destination",
  "settlement.features.wallets.ui.walletPage.add.payout.destination": "Add payout destination",
  "settlement.features.wallets.ui.walletPage.amount": "Amount",
  "settlement.features.wallets.ui.walletPage.available": "Available",
  "settlement.features.wallets.ui.walletPage.available.balance.can.be.used.for":
    "Available balance can be used for marketplace purchases or requested as a payout after setup is ready. Payout requests move money into a payout-in-progress ledger entry until the provider confirms the result.",
  "settlement.features.wallets.ui.walletPage.available.funds.appear.here.after.sales":
    "Available funds appear here after sales settle.",
  "settlement.features.wallets.ui.walletPage.available.funds.appear.here.after.sales.2":
    "Available funds appear here after sales settle",
  "settlement.features.wallets.ui.walletPage.available.funds.can.be.requested.on":
    "Available funds can be requested on demand after setup is ready. Pending funds remain protected until settlement makes them available.",
  "settlement.features.wallets.ui.walletPage.available.now": "Available now",
  "settlement.features.wallets.ui.walletPage.balance": "Balance",
  "settlement.features.wallets.ui.walletPage.create.the.hosted.payout.setup.session": "Open the payout setup page",
  "settlement.features.wallets.ui.walletPage.days.before.they.become.available": "days before they become available.",
  "settlement.features.wallets.ui.walletPage.destination.is.ready": "Destination is ready",
  "settlement.features.wallets.ui.walletPage.direction": "Direction",
  "settlement.features.wallets.ui.walletPage.done": "Done",
  "settlement.features.wallets.ui.walletPage.earn.available.funds": "Earn available funds",
  "settlement.features.wallets.ui.walletPage.finish.account.verification": "Finish account verification",
  "settlement.features.wallets.ui.walletPage.finish.payout.setup.before.requesting.available":
    "Finish payout setup before requesting available funds.",
  "settlement.features.wallets.ui.walletPage.funds.are.still.pending": "Funds are still pending",
  "settlement.features.wallets.ui.walletPage.funds.ready.for.platform.purchases.and":
    "Funds ready for platform purchases and, after payout setup, payouts.",
  "settlement.features.wallets.ui.walletPage.kind": "Kind",
  "settlement.features.wallets.ui.walletPage.ledger": "Ledger",
  "settlement.features.wallets.ui.walletPage.ledger.entries.appear.here.once.sales":
    "Ledger entries appear here once sales, fees, and payouts are processed.",
  "settlement.features.wallets.ui.walletPage.lifetime.credits": "Lifetime credits: ",
  "settlement.features.wallets.ui.walletPage.lifetime.debits": "Lifetime debits: ",
  "settlement.features.wallets.ui.walletPage.next": "Next",
  "settlement.features.wallets.ui.walletPage.no.ledger.activity": "No ledger activity",
  "settlement.features.wallets.ui.walletPage.no.payoutable.funds": "No payoutable funds",
  "settlement.features.wallets.ui.walletPage.payout.availability": "Payout Availability",
  "settlement.features.wallets.ui.walletPage.payout.available.now": "Payout available now",
  "settlement.features.wallets.ui.walletPage.payout.setup": "Payout Setup",
  "settlement.features.wallets.ui.walletPage.pending": "Pending",
  "settlement.features.wallets.ui.walletPage.pending.2": "Pending",
  "settlement.features.wallets.ui.walletPage.pending.review": "Pending review",
  "settlement.features.wallets.ui.walletPage.posted": "Posted",
  "settlement.features.wallets.ui.walletPage.provider.account.created": "Provider account created",
  "settlement.features.wallets.ui.walletPage.sale.funds.are.held.for": "Sale funds are held for ",
  "settlement.features.wallets.ui.walletPage.seller.setup.checklist": "Account Setup Checklist",
  "settlement.features.wallets.ui.walletPage.settlement": "Settlement",
  "settlement.features.wallets.ui.walletPage.setup.has.not.started": "Setup has not started",
  "settlement.features.wallets.ui.walletPage.setup.is.the.blocker": "Setup is the blocker",
  "settlement.features.wallets.ui.walletPage.start.payout.setup": "Start payout setup",
  "settlement.features.wallets.ui.walletPage.status": "Status",
  "settlement.features.wallets.ui.walletPage.track.your.pending.and.available.balances":
    "Track your pending and available balances, and review all ledger activity.",
  "settlement.features.wallets.ui.walletPage.view.payouts": "View payouts",
  "settlement.features.wallets.ui.walletPage.wallet": "Wallet",
  "settlement.features.wallets.ui.walletAdjustmentCopy.credit": "Credit",
  "settlement.features.wallets.ui.walletAdjustmentCopy.debit": "Debit",
  "settlement.features.wallets.ui.walletAdjustmentCopy.transaction.correction": "Transaction correction",
  "settlement.features.wallets.ui.walletAdjustmentCopy.refund.correction": "Refund correction",
  "settlement.features.wallets.ui.walletAdjustmentCopy.fee.correction": "Fee correction",
  "settlement.features.wallets.ui.walletAdjustmentCopy.dispute.resolution": "Dispute resolution",
  "settlement.features.wallets.ui.walletAdjustmentCopy.fraud.recovery": "Fraud recovery",
  "settlement.features.wallets.ui.walletAdjustmentCopy.support.resolution": "Support resolution",
  "settlement.features.wallets.ui.walletAdjustmentCopy.legal.obligation": "Legal obligation",
  "settlement.features.wallets.ui.walletAdjustmentCopy.goodwill.cash.credit": "Goodwill cash credit",
  "settlement.features.wallets.ui.walletAdjustmentCopy.operational.error": "Operational error",
  "settlement.features.wallets.ui.walletAdjustmentCopy.other.requires.explanation": "Other (requires explanation)",
  "settlement.features.wallets.ui.walletAdjustmentCopy.awaiting.approval": "Awaiting approval",
  "settlement.features.wallets.ui.walletAdjustmentCopy.approved.posting": "Approved, posting",
  "settlement.features.wallets.ui.walletAdjustmentCopy.rejected": "Rejected",
  "settlement.features.wallets.ui.walletAdjustmentCopy.posted": "Posted",
  "settlement.features.wallets.ui.walletAdjustmentCopy.reversed": "Reversed",
  "settlement.features.wallets.ui.walletAdjustmentCopy.high.value.amount": "High-value amount",
  "settlement.features.wallets.ui.walletAdjustmentCopy.creates.or.deepens.a.negative.balance":
    "Creates or deepens a negative balance",
  "settlement.features.wallets.ui.walletAdjustmentCopy.reversal.of.settled.funds": "Reversal of settled funds",
  "settlement.features.wallets.ui.walletAdjustmentCopy.check.the.adjustment.details": "Check the adjustment details",
  "settlement.features.wallets.ui.walletAdjustmentCopy.balance.changed.since.preview": "Balance changed since preview",
  "settlement.features.wallets.ui.walletAdjustmentCopy.reauthentication.required": "Reauthentication required",
  "settlement.features.wallets.ui.walletAdjustmentCopy.second.approval.required": "Second approval required",
  "settlement.features.wallets.ui.walletAdjustmentCopy.self.benefiting.adjustment.blocked":
    "Self-benefiting adjustment blocked",
  "settlement.features.wallets.ui.walletAdjustmentCopy.adjustment.not.found": "Adjustment not found",
  "settlement.features.wallets.ui.walletAdjustmentCopy.not.authorized": "Not authorized",
  "settlement.features.wallets.ui.walletAdjustmentCopy.sign.in.required": "Sign-in required",
  "settlement.features.wallets.ui.walletAdjustmentCopy.action.could.not.be.completed": "Action could not be completed",
  "settlement.features.wallets.ui.walletAdjustmentCopy.something.went.wrong": "Something went wrong",
  "settlement.features.wallets.ui.walletAdjustmentReauthenticationLink.sign.in.again.in.new.tab":
    "Sign in again in a new tab",
  "settlement.features.wallets.ui.walletAdjustmentReceipt.account": "Account",
  "settlement.features.wallets.ui.walletAdjustmentReceipt.direction": "Direction",
  "settlement.features.wallets.ui.walletAdjustmentReceipt.amount": "Amount",
  "settlement.features.wallets.ui.walletAdjustmentReceipt.reason": "Reason",
  "settlement.features.wallets.ui.walletAdjustmentReceipt.explanation": "Explanation",
  "settlement.features.wallets.ui.walletAdjustmentReceipt.references": "References",
  "settlement.features.wallets.ui.walletAdjustmentReceipt.requested": "Requested",
  "settlement.features.wallets.ui.walletAdjustmentReceipt.approved": "Approved",
  "settlement.features.wallets.ui.walletAdjustmentReceipt.posted": "Posted",
  "settlement.features.wallets.ui.walletAdjustmentReceipt.balance.impact": "Balance impact",
  "settlement.features.wallets.ui.walletAdjustmentReceipt.rejected": "Rejected",
  "settlement.features.wallets.ui.walletAdjustmentReceipt.reversed": "Reversed",
  "settlement.features.wallets.ui.walletAdjustmentReceipt.receipt": "Wallet adjustment receipt",
  "settlement.features.wallets.ui.walletAdjustmentReceipt.balance.before.after": "{before} → {after}",
  "settlement.features.wallets.ui.walletAdjustmentReceipt.rejected.on.with.reason": "{date} — {reason}",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.step.details": "Details",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.step.details.description":
    "Select credit or debit and enter amount, reason, and evidence.",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.step.review": "Review preview",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.step.review.description":
    "Confirm the balance impact and any approval requirement.",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.step.confirm": "Confirm & submit",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.step.confirm.description":
    "Explicitly confirm the exact account and signed amount.",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.fix.the.highlighted.fields":
    "Fix the highlighted fields and request a new preview.",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.credit.or.debit": "Credit or debit",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.amount": "Amount",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.reason": "Reason",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.explanation.required": "Explanation (required)",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.explanation.optional": "Explanation (optional)",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.explanation.description":
    'Required detail for "Other"; recommended for every other reason.',
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.related.references": "Related references",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.related.references.description":
    "Order, payment, payout, or support references this adjustment relates to.",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.request.preview": "Request preview",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.preview.title": "Preview",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.target.account": "Target account",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.available.balance.before": "Available balance before",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.available.balance.after": "Available balance after",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.spendable": "Spendable",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.payoutable": "Payoutable",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.yes": "Yes",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.no": "No",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.negative.balance.title":
    "Creates or deepens a negative balance",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.negative.balance.description":
    "This debit will take the account balance below zero. Collections handling may apply.",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.second.approval.title":
    "A second, elevated approval will be required",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.self.benefiting.title":
    "Self-benefiting adjustment blocked",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.self.benefiting.description":
    "This adjustment targets your own account and can never be approved. Ask another platform admin to request it.",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.confirm.title": "Confirm this wallet adjustment",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.confirm.description":
    "This will {direction} {amount} for {account}. The available balance will become {after}.",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.confirm.submit": "Submit adjustment",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.cancel": "Cancel",
  "settlement.features.wallets.ui.walletAdjustmentGuidedFlow.confirm.and.submit": "Confirm & submit",
  "settlement.features.wallets.ui.walletAdjustmentApprovalActions.you.requested.this.adjustment":
    "You requested this adjustment",
  "settlement.features.wallets.ui.walletAdjustmentApprovalActions.another.platform.admin.must.approve":
    "Another platform admin must approve or reject it.",
  "settlement.features.wallets.ui.walletAdjustmentApprovalActions.approve.title": "Approve this wallet adjustment",
  "settlement.features.wallets.ui.walletAdjustmentApprovalActions.approve.description":
    "This will approve and post {amount} for {account}.",
  "settlement.features.wallets.ui.walletAdjustmentApprovalActions.approve.confirm": "Approve & post",
  "settlement.features.wallets.ui.walletAdjustmentApprovalActions.cancel": "Cancel",
  "settlement.features.wallets.ui.walletAdjustmentApprovalActions.approve": "Approve",
  "settlement.features.wallets.ui.walletAdjustmentApprovalActions.reject.title": "Reject this wallet adjustment",
  "settlement.features.wallets.ui.walletAdjustmentApprovalActions.reject.description":
    "The requester will be able to see the rejection reason.",
  "settlement.features.wallets.ui.walletAdjustmentApprovalActions.reject.confirm": "Reject adjustment",
  "settlement.features.wallets.ui.walletAdjustmentApprovalActions.reject": "Reject",
  "settlement.features.wallets.ui.walletAdjustmentApprovalActions.elevation.explanation":
    "Policy requires a second, elevated approval for this adjustment. Enter the platform admin user id of a colleague who independently verified this action.",
  "settlement.features.wallets.ui.walletAdjustmentApprovalActions.elevation.approver": "Elevated approver user id",
  "settlement.features.wallets.ui.walletAdjustmentApprovalActions.rejection.reason": "Rejection reason (optional)",
  "settlement.features.wallets.ui.walletAdjustmentReversalActions.reversal.explanation":
    "Reversal always requires a primary approver and a second, elevated approver distinct from the requester. Enter the platform admin user ids of the colleagues who verified this reversal.",
  "settlement.features.wallets.ui.walletAdjustmentReversalActions.approver": "Approver user id",
  "settlement.features.wallets.ui.walletAdjustmentReversalActions.elevated.approver": "Elevated approver user id",
  "settlement.features.wallets.ui.walletAdjustmentReversalActions.reversal.reason": "Reversal reason (optional)",
  "settlement.features.wallets.ui.walletAdjustmentReversalActions.confirm.title": "Reverse this wallet adjustment",
  "settlement.features.wallets.ui.walletAdjustmentReversalActions.confirm.description":
    "This posts an opposite-direction adjustment of {amount} for {account}, reversing the original entry.",
  "settlement.features.wallets.ui.walletAdjustmentReversalActions.confirm.reverse": "Reverse adjustment",
  "settlement.features.wallets.ui.walletAdjustmentReversalActions.cancel": "Cancel",
  "settlement.features.wallets.ui.walletAdjustmentReversalActions.reverse": "Reverse",
  "settlement.routes.admin.moneyHealth.money.health.settlement.admin": "Money Health | Settlement Admin",
  "settlement.routes.admin.payoutOperations.payout.operations.settlement.admin": "Payout Operations | Settlement Admin",
  "settlement.routes.marketplace.accountPayout.payout.marketplace": "Payout | Marketplace",
  "settlement.routes.marketplace.accountPayout.payout.not.found": "Payout not found.",
  "settlement.routes.marketplace.accountPayout.payout.preparing": "Preparing payout",
  "settlement.routes.marketplace.accountPayout.payout.preparing.description":
    "We're preparing your payout details. Try again in a moment.",
  "settlement.routes.marketplace.accountPayoutSetup.account.access": "Account access",
  "settlement.routes.marketplace.accountPayoutSetup.account.access.required.description":
    "This signed-in account can browse the marketplace, but it is not authorized to complete payout setup.",
  "settlement.routes.marketplace.accountPayoutSetup.account.access.required.title":
    "Use a payout-enabled account for setup",
  "settlement.routes.marketplace.accountPayoutSetup.next.step": "Next step",
  "settlement.routes.marketplace.accountPayoutSetup.payout.setup.marketplace": "Payout Setup | Marketplace",
  "settlement.routes.marketplace.accountPayoutSetup.payout.setup.status.could.not":
    "Payout setup status could not be refreshed.",
  "settlement.routes.marketplace.accountPayoutSetup.payout.setup.status.was.refreshed":
    "Payout setup status was refreshed.",
  "settlement.routes.marketplace.accountPayoutSetup.use.a.different.account": "Use a different account",
  "settlement.routes.marketplace.accountPayoutSetup.use.an.account.with.payout.setup.access":
    "Choose an account with payout setup access to continue. If this is the wrong account, use a different sign-in and return to this page.",
  "settlement.routes.marketplace.accountPayoutSetup.view.account": "View account",
  "settlement.routes.marketplace.accountPayouts.account.access": "Account access",
  "settlement.routes.marketplace.accountPayouts.account.access.required.description":
    "This signed-in account can browse the marketplace, but it is not authorized to view or request payouts.",
  "settlement.routes.marketplace.accountPayouts.account.access.required.title": "Use a payout-enabled account",
  "settlement.routes.marketplace.accountPayouts.next.step": "Next step",
  "settlement.routes.marketplace.accountPayouts.payout.setup.status.was.refreshed":
    "Payout setup status was refreshed.",
  "settlement.routes.marketplace.accountPayouts.payouts.marketplace": "Payouts | Marketplace",
  "settlement.routes.marketplace.accountPayouts.use.a.different.account": "Use a different account",
  "settlement.routes.marketplace.accountPayouts.use.an.account.with.payout.access":
    "Choose an account with payout access to continue. If this is the wrong account, use a different sign-in and return to this page.",
  "settlement.routes.marketplace.accountPayouts.view.account": "View account",
  "settlement.routes.marketplace.accountPayouts.you.do.not.have.permission.to":
    "You do not have permission to request payouts.",
  "settlement.routes.marketplace.accountPayouts.you.do.not.have.permission.to.2":
    "You do not have permission to request payouts.",
  "settlement.routes.marketplace.accountPayouts.you.do.not.have.permission.to.3":
    "You do not have permission to request payouts.",
  "settlement.routes.marketplace.accountPayouts.you.do.not.have.permission.to.4":
    "You do not have permission to update payout setup.",
  "settlement.routes.marketplace.accountPayouts.you.do.not.have.permission.to.5":
    "You do not have permission to update payout setup.",
  "settlement.routes.marketplace.accountPayouts.you.do.not.have.permission.to.6":
    "You do not have permission to update payout setup.",
  "settlement.features.payouts.ui.moneyHealthPage.amount.available": "{amount} available",
  "settlement.features.payouts.ui.moneyHealthPage.reconciliation.counts":
    "Checked {checked}; reconciled {reconciled}; errors {errors}",
  "settlement.features.payouts.ui.payoutDetailPage.attempt.plural": "attempts",
  "settlement.features.payouts.ui.payoutDetailPage.attempt.singular": "attempt",
  "settlement.features.payouts.ui.payoutDetailPage.payout.title": "Payout {payoutReference}",
  "settlement.features.payouts.ui.payoutDetailPage.retry.next": "{count} {attemptLabel}; next {nextAt}",
  "settlement.features.payouts.ui.payoutDetailPage.retry.none.pending":
    "{count} {attemptLabel}; no automatic retry pending",
  "settlement.features.payouts.ui.payoutListPage.no.available.balance.detail":
    "{reason} Pending funds become available after settlement review.",
  "settlement.features.payouts.ui.payoutListPage.paid.on.date": "Paid {date}",
  "settlement.features.wallets.api.route.ledger.description.with.reason": "{description} ({reason})",
  "settlement.features.wallets.ui.walletPage.amount.available": "{amount} available",
  "settlement.features.wallets.ui.walletPage.amount.can.be.requested.for.payout":
    "{amount} can be requested for payout.",
  "settlement.features.wallets.ui.walletPage.amount.is.pending":
    "{amount} is pending and appears here when it becomes available.",
  "settlement.features.wallets.ui.walletPage.available.on.date": "Available {date}",
  "settlement.features.wallets.ui.walletPage.expected.on.date": "Expected {date}",
  "settlement.features.wallets.ui.walletPage.setup.status": "Setup is {status}",
  "settlement.features.payouts.ui.payoutDetailPage.amount.is.being.sent.to.your.saved":
    "{amount} is being sent to your saved payout destination.",
  "settlement.features.wallets.ui.walletPage.hold.days.before.they.become.available":
    "{count} days before they become available",
  "settlement.routes.marketplace.accountSettlement.wallet.marketplace": "Wallet | Marketplace",
  "settlement.routes.admin.walletWorkbench.wallet.workbench.settlement.admin": "Wallet Workbench | Settlement Admin",
  "settlement.routes.admin.walletWorkbench.action.failed": "The wallet-adjustment action could not be completed.",
  "settlement.features.wallets.ui.walletWorkbenchPage.settlement": "Settlement",
  "settlement.features.wallets.ui.walletWorkbenchPage.wallet.workbench": "Wallet Workbench",
  "settlement.features.wallets.ui.walletWorkbenchPage.wallet.for.account": "Wallet for {name}",
  "settlement.features.wallets.ui.walletWorkbenchPage.account.id.reference": "Account: {accountId}",
  "settlement.features.wallets.ui.walletWorkbenchPage.account.not.found.title": "Account wallet not found",
  "settlement.features.wallets.ui.walletWorkbenchPage.account.not.found.description":
    "No wallet has ever been opened for account {accountId}. Confirm the account id is correct.",
  "settlement.features.wallets.ui.walletWorkbenchPage.permission.denied.title": "Permission required",
  "settlement.features.wallets.ui.walletWorkbenchPage.permission.denied.description":
    "You do not have permission to view this account's wallet. Ask a platform admin to grant wallet-adjustments access.",
  "settlement.features.wallets.ui.walletWorkbenchPage.unavailable.title": "Wallet workbench unavailable",
  "settlement.features.wallets.ui.walletWorkbenchPage.unavailable.description":
    "The wallet could not be loaded right now. Try again shortly.",
  "settlement.features.wallets.ui.walletWorkbenchPage.balance": "Balance",
  "settlement.features.wallets.ui.walletWorkbenchPage.available": "Available",
  "settlement.features.wallets.ui.walletWorkbenchPage.pending": "Pending",
  "settlement.features.wallets.ui.walletWorkbenchPage.lifetime.credits": "Lifetime credits",
  "settlement.features.wallets.ui.walletWorkbenchPage.lifetime.debits": "Lifetime debits",
  "settlement.features.wallets.ui.walletWorkbenchPage.standing.good": "In good standing",
  "settlement.features.wallets.ui.walletWorkbenchPage.standing.negative": "Negative balance",
  "settlement.features.wallets.ui.walletWorkbenchPage.standing.collections": "Collections",
  "settlement.features.wallets.ui.walletWorkbenchPage.last.updated": "Last updated {date}",
  "settlement.features.wallets.ui.walletWorkbenchPage.never.updated": "No wallet activity yet",
  "settlement.features.wallets.ui.walletWorkbenchPage.payout.restricted.title": "Payouts restricted",
  "settlement.features.wallets.ui.walletWorkbenchPage.payout.restricted.negative.description":
    "This account carries a negative balance. Payouts are restricted until the balance is resolved.",
  "settlement.features.wallets.ui.walletWorkbenchPage.payout.restricted.collections.description":
    "This account is in collections. Payouts are restricted and the negative balance may be escalated for recovery.",
  "settlement.features.wallets.ui.walletWorkbenchPage.request.adjustment": "Request adjustment",
  "settlement.features.wallets.ui.walletWorkbenchPage.direction": "Direction",
  "settlement.features.wallets.ui.walletWorkbenchPage.amount": "Amount",
  "settlement.features.wallets.ui.walletWorkbenchPage.reason": "Reason",
  "settlement.features.wallets.ui.walletWorkbenchPage.adjustment.history": "Adjustment history",
  "settlement.features.wallets.ui.walletWorkbenchPage.status": "Status",
  "settlement.features.wallets.ui.walletWorkbenchPage.actors": "Actors",
  "settlement.features.wallets.ui.walletWorkbenchPage.requested.by": "Requested by {userId}",
  "settlement.features.wallets.ui.walletWorkbenchPage.approved.by": "Approved by {userId}",
  "settlement.features.wallets.ui.walletWorkbenchPage.rejected.by": "Rejected by {userId}",
  "settlement.features.wallets.ui.walletWorkbenchPage.resulting.balance": "Resulting balance",
  "settlement.features.wallets.ui.walletWorkbenchPage.reversal": "Reversal",
  "settlement.features.wallets.ui.walletWorkbenchPage.reverses": "Reverses {id}",
  "settlement.features.wallets.ui.walletWorkbenchPage.reversed.by": "Reversed by {id}",
  "settlement.features.wallets.ui.walletWorkbenchPage.timeline": "Timeline",
  "settlement.features.wallets.ui.walletWorkbenchPage.posted.at": "Posted {date}",
  "settlement.features.wallets.ui.walletWorkbenchPage.actions": "Actions",
  "settlement.features.wallets.ui.walletWorkbenchPage.filter.all": "All",
  "settlement.features.wallets.ui.walletWorkbenchPage.no.adjustments.title": "No adjustments",
  "settlement.features.wallets.ui.walletWorkbenchPage.no.adjustments.description":
    "Wallet adjustments requested, approved, or reversed for this account appear here.",
  "settlement.features.wallets.ui.walletWorkbenchPage.previous": "Previous",
  "settlement.features.wallets.ui.walletWorkbenchPage.next": "Next",
  "settlement.features.wallets.ui.walletWorkbenchPage.projection.catching.up.title": "Projection catching up",
  "settlement.features.wallets.ui.walletWorkbenchPage.projection.catching.up.description":
    "A change was just submitted and is shown below from the command response; the table may take a moment to reflect it too.",
  "settlement.features.wallets.ui.walletWorkbenchPage.preview.could.not.be.generated":
    "Preview could not be generated.",
  "settlement.features.wallets.ui.walletWorkbenchPage.adjustment.could.not.be.submitted":
    "Adjustment could not be submitted.",
  "settlement.features.wallets.ui.walletWorkbenchPage.approval.could.not.be.completed":
    "Approval could not be completed.",
  "settlement.features.wallets.ui.walletWorkbenchPage.rejection.could.not.be.completed":
    "Rejection could not be completed.",
  "settlement.features.wallets.ui.walletWorkbenchPage.reversal.could.not.be.completed":
    "Reversal could not be completed.",
  "settlement.features.wallets.ui.walletWorkbenchPage.ledger": "Ledger",
  "settlement.features.wallets.ui.walletWorkbenchPage.kind": "Kind",
  "settlement.features.wallets.ui.walletWorkbenchPage.posted": "Posted",
  "settlement.features.wallets.ui.walletWorkbenchPage.related": "Related",
  "settlement.features.wallets.ui.walletWorkbenchPage.related.payout": "View payout",
  "settlement.features.wallets.ui.walletWorkbenchPage.no.ledger.title": "No ledger activity",
  "settlement.features.wallets.ui.walletWorkbenchPage.no.ledger.description":
    "Ledger entries for this account appear here once posted.",

  "settlement.features.wallets.api.route.wallet.adjustment.not.found": "Wallet adjustment not found.",

  "settlement.features.wallets.integrations.transactionalNotifications.walletAdjustmentNotificationIntents.reason.transaction.correction":
    "General transaction correction",
  "settlement.features.wallets.integrations.transactionalNotifications.walletAdjustmentNotificationIntents.reason.refund.correction":
    "Refund correction",
  "settlement.features.wallets.integrations.transactionalNotifications.walletAdjustmentNotificationIntents.reason.fee.correction":
    "Fee correction",
  "settlement.features.wallets.integrations.transactionalNotifications.walletAdjustmentNotificationIntents.reason.dispute.resolution":
    "Dispute resolution",
  "settlement.features.wallets.integrations.transactionalNotifications.walletAdjustmentNotificationIntents.reason.fraud.recovery":
    "Fraud recovery",
  "settlement.features.wallets.integrations.transactionalNotifications.walletAdjustmentNotificationIntents.reason.support.resolution":
    "Support case resolution",
  "settlement.features.wallets.integrations.transactionalNotifications.walletAdjustmentNotificationIntents.reason.legal.obligation":
    "Legal correction",
  "settlement.features.wallets.integrations.transactionalNotifications.walletAdjustmentNotificationIntents.reason.goodwill.cash.credit":
    "Goodwill credit",
  "settlement.features.wallets.integrations.transactionalNotifications.walletAdjustmentNotificationIntents.reason.operational.error":
    "Operational correction",
  "settlement.features.wallets.integrations.transactionalNotifications.walletAdjustmentNotificationIntents.reason.other":
    "Other correction",
  "settlement.features.wallets.integrations.transactionalNotifications.walletAdjustmentNotificationIntents.posted.credit.title":
    "You received a {amount} wallet credit",
  "settlement.features.wallets.integrations.transactionalNotifications.walletAdjustmentNotificationIntents.posted.debit.title":
    "A {amount} wallet debit was applied to your account",
  "settlement.features.wallets.integrations.transactionalNotifications.walletAdjustmentNotificationIntents.posted.body":
    "{reasonCategory}. Your resulting wallet balance is {resultingBalance}. Reference: {reference}.",
  "settlement.features.wallets.integrations.transactionalNotifications.walletAdjustmentNotificationIntents.reversed.title":
    "Wallet adjustment {reference} was reversed",
  "settlement.features.wallets.integrations.transactionalNotifications.walletAdjustmentNotificationIntents.reversed.body":
    "The adjustment {originalReference} was reversed by a new correcting entry, {reversalReference}. Your resulting wallet balance is {resultingBalance}.",

  "settlement.features.wallets.ui.walletAdjustmentAccountCopy.status.under.review": "Under review",
  "settlement.features.wallets.ui.walletAdjustmentAccountCopy.status.not.applied": "Not applied",
  "settlement.features.wallets.ui.walletAdjustmentAccountCopy.status.posted": "Posted",
  "settlement.features.wallets.ui.walletAdjustmentAccountCopy.status.reversed": "Reversed",
  "settlement.features.wallets.ui.walletAdjustmentAccountCopy.direction.credit": "Credit",
  "settlement.features.wallets.ui.walletAdjustmentAccountCopy.direction.debit": "Debit",

  "settlement.features.wallets.ui.walletAdjustmentAccountDetailPage.wallet": "Wallet",
  "settlement.features.wallets.ui.walletAdjustmentAccountDetailPage.title": "Wallet adjustment {reference}",
  "settlement.features.wallets.ui.walletAdjustmentAccountDetailPage.description":
    "Why this adjustment happened, its effect on your balance, and how to reach support if something looks wrong.",
  "settlement.features.wallets.ui.walletAdjustmentAccountDetailPage.view.wallet": "View wallet",
  "settlement.features.wallets.ui.walletAdjustmentAccountDetailPage.details": "Details",
  "settlement.features.wallets.ui.walletAdjustmentAccountDetailPage.direction": "Direction",
  "settlement.features.wallets.ui.walletAdjustmentAccountDetailPage.amount": "Amount",
  "settlement.features.wallets.ui.walletAdjustmentAccountDetailPage.reason": "Reason",
  "settlement.features.wallets.ui.walletAdjustmentAccountDetailPage.requested": "Requested",
  "settlement.features.wallets.ui.walletAdjustmentAccountDetailPage.posted": "Posted",
  "settlement.features.wallets.ui.walletAdjustmentAccountDetailPage.resulting.balance": "Resulting balance",
  "settlement.features.wallets.ui.walletAdjustmentAccountDetailPage.about.this.adjustment": "About this adjustment",
  "settlement.features.wallets.ui.walletAdjustmentAccountDetailPage.cash.equivalent.title":
    "This is cash-equivalent, not promotional credit",
  "settlement.features.wallets.ui.walletAdjustmentAccountDetailPage.cash.equivalent.description":
    "A wallet adjustment is part of your available balance -- spendable and eligible for payout the same as any other funds. It is reviewed corrections, not a promotional balance.",
  "settlement.features.wallets.ui.walletAdjustmentAccountDetailPage.reversal.section": "Reversal status",
  "settlement.features.wallets.ui.walletAdjustmentAccountDetailPage.corrects.title":
    "This entry corrects an earlier adjustment",
  "settlement.features.wallets.ui.walletAdjustmentAccountDetailPage.corrects.description":
    "This entry corrects an earlier adjustment, {reference}.",
  "settlement.features.wallets.ui.walletAdjustmentAccountDetailPage.view.original.entry": "View original entry",
  "settlement.features.wallets.ui.walletAdjustmentAccountDetailPage.reversed.title": "This adjustment was reversed",
  "settlement.features.wallets.ui.walletAdjustmentAccountDetailPage.reversed.description":
    "This adjustment was reversed on {date}. See the correcting entry, {reference}.",
  "settlement.features.wallets.ui.walletAdjustmentAccountDetailPage.view.correcting.entry": "View correcting entry",
  "settlement.features.wallets.ui.walletAdjustmentAccountDetailPage.provisional.note":
    "Wallet adjustments are reviewed corrections and can be reversed if a mistake is later found. If this entry is reversed, a linked correcting entry will appear here and you will be notified again.",
  "settlement.features.wallets.ui.walletAdjustmentAccountDetailPage.support.title": "Questions about this adjustment?",
  "settlement.features.wallets.ui.walletAdjustmentAccountDetailPage.support.description":
    "Contact support and reference {reference} so we can locate this adjustment right away.",
  "settlement.features.wallets.ui.walletAdjustmentAccountDetailPage.contact.support": "Contact support",

  "settlement.features.wallets.ui.walletPage.kind.adjustment": "Wallet adjustment",
  "settlement.features.wallets.ui.walletPage.adjustment.reversed": "Reversed",
  "settlement.features.wallets.ui.walletPage.view.adjustment.details": "View details",

  "settlement.routes.marketplace.accountWalletAdjustment.adjustment.not.found": "Wallet adjustment not found.",
  "settlement.routes.marketplace.accountWalletAdjustment.wallet.adjustment": "Wallet Adjustment",

  "settlement.features.moneyDashboard.ui.actionFailed": "We could not complete that action",
  "settlement.features.moneyDashboard.ui.actions": "Actions",
  "settlement.features.moneyDashboard.ui.activity": "Activity",
  "settlement.features.moneyDashboard.ui.amount": "Amount",
  "settlement.features.moneyDashboard.ui.atAGlance": "Money at a glance",
  "settlement.features.moneyDashboard.ui.availableAfter": "{amount} available after this payout",
  "settlement.features.moneyDashboard.ui.availableToRequest": "{amount} available to request",
  "settlement.features.moneyDashboard.ui.awaitingConfirmation": "Awaiting payout confirmation",
  "settlement.features.moneyDashboard.ui.blocked": "Blocked payouts",
  "settlement.features.moneyDashboard.ui.blockedDetail": "Failed or overdue payouts that need a next step",
  "settlement.features.moneyDashboard.ui.checkStatus": "Check status",
  "settlement.features.moneyDashboard.ui.completed": "Completed",
  "settlement.features.moneyDashboard.ui.confirmPayout": "Confirm payout",
  "settlement.features.moneyDashboard.ui.contactSupport": "Contact support",
  "settlement.features.moneyDashboard.ui.description":
    "See your balance, payouts, and anything blocking your money in one place.",
  "settlement.features.moneyDashboard.ui.direction": "Direction",
  "settlement.features.moneyDashboard.ui.editPayout": "Edit payout",
  "settlement.features.moneyDashboard.ui.estimatedBy": "Estimated by {date}",
  "settlement.features.moneyDashboard.ui.failedPayout": "Failed payout",
  "settlement.features.moneyDashboard.ui.finishPayoutSetup": "Finish payout setup",
  "settlement.features.moneyDashboard.ui.fullAvailable": "Full available · {amount}",
  "settlement.features.moneyDashboard.ui.managePayoutAccount": "Manage payout account",
  "settlement.features.moneyDashboard.ui.marketplaceTitle": "Money | Chase Sets",
  "settlement.features.moneyDashboard.ui.manyNeedAttention": "{count} payouts need attention",
  "settlement.features.moneyDashboard.ui.minimumAmount": "Minimum · {amount}",
  "settlement.features.moneyDashboard.ui.needsAttention": "Needs attention",
  "settlement.features.moneyDashboard.ui.nextAction": "Next action",
  "settlement.features.moneyDashboard.ui.nextPayout": "Next payout",
  "settlement.features.moneyDashboard.ui.noBlockedPayouts": "No payouts need attention",
  "settlement.features.moneyDashboard.ui.noBlockedPayoutsDetail": "Failed and overdue payout checks will appear here.",
  "settlement.features.moneyDashboard.ui.noneScheduled": "None scheduled",
  "settlement.features.moneyDashboard.ui.noWalletActivity": "No wallet activity yet",
  "settlement.features.moneyDashboard.ui.noWalletActivityDetail": "Sales, payouts, and adjustments will appear here.",
  "settlement.features.moneyDashboard.ui.onTheWay": "On the way",
  "settlement.features.moneyDashboard.ui.oneNeedsAttention": "1 payout needs attention",
  "settlement.features.moneyDashboard.ui.openPayout": "Open payout",
  "settlement.features.moneyDashboard.ui.optionalNote": "Optional note",
  "settlement.features.moneyDashboard.ui.optionalNoteSummary": "Add a note for your records",
  "settlement.features.moneyDashboard.ui.paid": "Paid",
  "settlement.features.moneyDashboard.ui.payout": "Payout",
  "settlement.features.moneyDashboard.ui.payoutAmount": "Payout amount",
  "settlement.features.moneyDashboard.ui.payoutAttention": "Payouts needing attention",
  "settlement.features.moneyDashboard.ui.payoutBlocked": "Why this payout is unavailable",
  "settlement.features.moneyDashboard.ui.payoutBreakdown": "{reference} breakdown",
  "settlement.features.moneyDashboard.ui.payoutTimeline": "Payout timeline",
  "settlement.features.moneyDashboard.ui.pendingBalance": "{amount} pending",
  "settlement.features.moneyDashboard.ui.posted": "Posted",
  "settlement.features.moneyDashboard.ui.previewPayout": "Preview payout",
  "settlement.features.moneyDashboard.ui.providerAcceptedPayout": "Submitted to your saved payout account.",
  "settlement.features.moneyDashboard.ui.providerAcceptedTransfer": "The payout provider accepted the transfer.",
  "settlement.features.moneyDashboard.ui.reason": "Why",
  "settlement.features.moneyDashboard.ui.requested": "Requested",
  "settlement.features.moneyDashboard.ui.requestPayout": "Request a payout",
  "settlement.features.moneyDashboard.ui.resultingBalance": "Resulting available balance",
  "settlement.features.moneyDashboard.ui.requestPermissionRequired": "You do not have permission to request payouts.",
  "settlement.features.moneyDashboard.ui.requestWhenReady": "Request a payout when funds are available",
  "settlement.features.moneyDashboard.ui.reconcilePermissionRequired":
    "You do not have permission to retry or reconcile payouts.",
  "settlement.features.moneyDashboard.ui.retryPayout": "Retry payout",
  "settlement.features.moneyDashboard.ui.sellerDesk": "Seller Desk",
  "settlement.features.moneyDashboard.ui.sent": "Sent",
  "settlement.features.moneyDashboard.ui.status": "Status",
  "settlement.features.moneyDashboard.ui.statusCheckOverdue": "Status check overdue",
  "settlement.features.moneyDashboard.ui.statusCheckQueued": "Payout status check queued",
  "settlement.features.moneyDashboard.ui.title": "Money",
  "settlement.features.moneyDashboard.ui.viewBreakdown": "View breakdown",
  "settlement.features.moneyDashboard.ui.walletActivity": "Wallet activity",
  "settlement.features.moneyDashboard.ui.walletAdjustment": "Wallet adjustment",
  "settlement.features.moneyDashboard.ui.walletAdjustmentDescription":
    "See this correction's account-safe balance effect and reason.",
  "settlement.features.moneyDashboard.ui.walletAdjustmentSafeDescription":
    "Wallet adjustments are reviewed cash-equivalent corrections. Internal operator notes and provider details are not shown here.",
  "settlement.features.moneyDashboard.ui.walletAdjustmentSafeTitle": "About this adjustment",
  "settlement.features.moneyDashboard.ui.walletAdjustmentTitle": "Wallet adjustment {reference}",
  "settlement.features.moneyDashboard.ui.walletBalance": "Available wallet balance",
} as const;
