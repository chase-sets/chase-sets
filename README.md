...
- `customer-feedback` ([README](bounded-contexts/customer-feedback/README.md), `@chase-sets/customer-feedback`): owned nouns `customer-satisfaction`, `csat-survey`, `survey-version`, `csat-invitation`, `csat-response`, `workflow-outcome-code`, `outcome-fact`, `sampling-policy`, `response-rate`, `csat-analytics-fact`, `legacy-experience-rating`.
