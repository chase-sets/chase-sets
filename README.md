...
- `pricing` ([README](bounded-contexts/pricing/README.md), `@chase-sets/pricing`): owned nouns `market-price-snapshot`, `price-recommendation`, `price-signal`, `trades-tape`, `daily-product-rollup`, `market-state-snapshot`, `repricing-policy`, `repricing-policy-assignment`.
