...
- `platform-operations` ([README](bounded-contexts/platform-operations/README.md), `@chase-sets/platform-operations`): owned nouns `projection-operation`, `platform-operation-attention`, `sales-performance-kpi`, `fulfillment-latency-kpi`, `conversion-order-kpi`, `dashboard-read-model`, `ops-dashboard`, `gmv-reconciliation-run`, `offer-economics-summary`, `platform-feedback`, `platform-feedback-prompt`, `rate-limit-policy`, `reported-content`, `risk-alert`, `support-request`.
