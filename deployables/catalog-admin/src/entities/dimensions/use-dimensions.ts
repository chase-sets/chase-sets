...
import type { Dimension, DimensionDetail, ListResponse, CommandResponse } from "../../api/types";
import { useFetch } from "../../shared/use-fetch";

export function useDimensionList() {
  return useFetch(() => api.get<ListResponse<Dimension>>("/dimensions"));
}
...
export function createDimension(body: { dimensionId: string; key: string; name: string }) {
  return api.post<CommandResponse>("/dimensions", body);
}

export function reviseDimension(id: string, body: { key: string; name: string }) {
...
export function addChoice(dimensionId: string, body: { choiceId: string; code: string; numericValue?: number }) {
  return api.post<CommandResponse>(`/dimensions/${dimensionId}/choices`, body);
}

export function reviseChoice(dimensionId: string, choiceId: string, body: { code: string; numericValue?: number }) {
