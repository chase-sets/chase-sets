 ...
 *
 * Timing inputs come from the sole shared-seed diagnostic run 36039215604,
 * attempt 1, artifact 10826652356 (shared-seed-bootstrap-measurement-36039215604-1),
 * SHA256 7cc04dadad1e5512100e2d00738b1fd5c49391c2a18484bf63655a6009150bec.
 * Its measured three-unit layout projects to 292803/197178/190361ms, at least
 * its observed 292795.551/189905.115/187641.930ms; modeled aggregate 732000ms
 * is at least its owning job's 722000ms wall clock. The shipped two-unit layout
 * projects the same 55 case observations to 292803/375350ms, aggregate 719811ms.
 *
...
export const bootstrapDbScheduleModel = Object.freeze({
  referenceRunId: 36039215604,
  referenceJobId: 107766880827,
  referenceJobName: "Diagnostic API Bootstrap Measurement Only",
  referenceHeadSha: "439268f8460cf09d5def8a5ad14b6e6e1e7755ed",
  referenceEvent: "push",
  maxWorkersPerExecutionUnit: 3,
  testFileFixedCostMs: 1693,
  executionUnitFixedCostMs: 12189,
  jobOverheadMs: 51658,
...
const scheduleModelFieldNames = Object.freeze(Object.keys(scheduleModelFieldConstraints));
const scheduleModelFieldAllowlist = new Set(scheduleModelFieldNames);
...
function validateScheduleModelShape(model) {
  ...
  try {
    ...
    return {
      model: null,
      violations: ["the execution-unit schedule model prototype could not be inspected"],
    };
  }
  ...
  let ownKeys;
  try {
    ownKeys = Reflect.ownKeys(model);
  } catch {
    return {
      model: null,
      violations: ["the execution-unit schedule model own properties could not be inspected"],
    };
  }

  const validatedValues = Object.create(null);
  for (const key of ownKeys) {
    if (typeof key === "symbol") {
      violations.push(`the execution-unit schedule model must not declare symbol field '${String(key)}'`);
      continue;
    }
    ...
    try {
      ...
      violations.push(`the execution-unit schedule model own property '${key}' could not be inspected`);
      continue;
    }
    if (!descriptor) {
      violations.push(`the execution-unit schedule model own property '${key}' has no descriptor`);
      continue;
    }
    if (!descriptor.enumerable) {
      violations.push(`the execution-unit schedule model own property '${key}' must be enumerable`);
    }
    if (!("value" in descriptor)) {
      violations.push(`the execution-unit schedule model own property '${key}' must be a data property`);
      continue;
    }
    if (!scheduleModelFieldAllowlist.has(key)) {
      violations.push(`the execution-unit schedule model declares unknown field '${key}'`);
      continue;
    }
    validatedValues[key] = descriptor.value;
  }
  for (const fieldName of scheduleModelFieldNames) {
    const constraint = scheduleModelFieldConstraints[fieldName];
    if (!Object.hasOwn(validatedValues, fieldName)) {
      violations.push(`the execution-unit schedule model must declare ${fieldName}`);
      continue;
    }
    if (!scheduleModelFieldSatisfies(validatedValues[fieldName], constraint)) {
      violations.push(
        `the execution-unit schedule model field ${fieldName} must be ${describeScheduleModelConstraint(constraint)}`,
      );
    }
  }
  if (violations.length > 0) return { model: null, violations };

  for (const relationship of scheduleModelRelationships) {
    if (!relationship.holds(validatedValues)) {
      violations.push(`the execution-unit schedule model is inconsistent: ${relationship.describe(validatedValues)}`);
    }
  }
  return { model: violations.length === 0 ? validatedValues : null, violations };
}
...
export function checkBootstrapDbEnrollment({
  ...
  const scheduleModelValidation = validateScheduleModelShape(scheduleModel);
  const validatedScheduleModel = scheduleModelValidation.model;
  const scheduleModelIsUsable = validatedScheduleModel !== null;
  violations.push(...scheduleModelValidation.violations);
  if (validatedScheduleModel) {
    violations.push(...validateManifestShape(manifest, executionUnitBootBearingCaseCeilings, validatedScheduleModel));
  }
  ...
  const schedule = scheduleModelIsUsable
    ? projectExecutionUnitSchedule({
        ...
        observedUnitNames,
        executionUnitBootBearingCaseCeilings,
        scheduleModel: validatedScheduleModel,
      })
