...
Browser routes that redirect to a read-model-backed page after a write carry the source-context receipt as a short-lived `afterWrite` query token. Server-side route fetches must use `@chase-sets/platform-runtime/http` forwarding helpers so that token is sent as `Chase-Sets-Read-After-Write`; request clients on shared API mounts must also pass `readTargetContextName` so `Chase-Sets-Read-Target-Context` is set. Stale auth-only forwarding helpers are retired and should not be reintroduced. Mounted APIs wait for relevant projection checkpoints before loading the read model. Route authors should declare `readFreshnessRoutes` on the owning API mount for post-write read-model routes so the gate waits on exact projection dependencies instead of every projection in the target context. Detail loaders may still use bounded `404` retries as a compatibility fallback. Clients must not treat every `404` as retryable; only a valid, unexpired fresh-write token means the resource may still be waiting for projections.

Routes should use `classifyFreshWriteReadError` from `@chase-sets/http/responses` before deciding whether read-model `404` or `projection_freshness_timeout` is temporary. Missing, malformed, future-dated, or expired `afterWrite` values are permanent for recovery purposes. Checkout maps valid fresh-write `404` and `503 projection_freshness_timeout` responses to a temporary checkout preparation state and keeps manual or stale checkout URLs as true not-found responses. Non-Checkout detail routes should use the same contract with context-owned copy: for example, Marketplace listing detail can show a temporary listing preparation state immediately after publication, while an old listing URL without a valid receipt remains a normal not-found or access response.
