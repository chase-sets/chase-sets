...
- The customer-facing surfaces expose too much marketplace machinery. Seller allocation, fulfillment preview revisions, Smart Match tuning, fallback listing publication, fee fingerprints, payout readiness, and detailed package rules are correct domain concepts but should not sit inside the main checkout form.
...
| Checkout start and identity gateway | `buy-checkout-readiness` at `/checkout/buy/readiness` | `bounded-contexts/checkout/routes/checkout-start.tsx`; guest checkout request support | `checkout-routes.test.ts`; cookie-backed continuation doc | Checkout owns start session intent; Auth owns guest continuation and exit |
| Buy checkout session | `buy-checkout-session` at `/checkout/buy/session/:sessionId` | `bounded-contexts/checkout/routes/checkout-session.tsx`; `bounded-contexts/checkout/features/sessions/ui/checkout-page.tsx`; session API, domain, projection, and read model | `checkout-page.test.tsx`; `sessions/api/route.test.ts`; `sessions/domain/domain.test.ts`; `checkout-confirmation-support.test.ts` | Checkout owns review; Ordering creates purchases; Payments creates payment; Fulfillment/Tax/Identity provide quoted inputs |
...
| Sell List review | `account-sell-list` at `/account/sell-list` | `bounded-contexts/checkout/routes/account-sell-list.tsx`; `bounded-contexts/checkout/features/sell-list/ui/sell-list-page.tsx`; Sell List API, domain, projection, and read-model files | `sell-list-page.test.tsx`; `sell-list/api/route.test.ts`; `sell-list/domain/domain.test.ts`; `checkout-routes.test.ts` | Checkout owns seller intent and confirmation readiness; Marketplace owns offers/listings; Settlement owns payout readiness |
| Guest Sell List | `account-sell-list` guest branch | `account-sell-list.tsx`; guest Sell List API/runtime support | `checkout-routes.test.ts`; `sell-list/api/route.test.ts` | Checkout lets guests collect Sell List intent but confirmation requires a signed-in account |
...
| Sell List | Sell List combines selected offers, product Smart Match offers, fallback listing creation, inventory selection, payout readiness, seller net, future listing gross, and confirmation decisions in one surface. | Split list editing from confirmation readiness. The Sell List review should show item, condition, quantity, expected payout, and primary action. Offer acceptance, fallback listing creation, and inventory sourcing belong behind progressive disclosure or the conditional readiness step. | #1108, #1111, #1121, #1135 |
| Guest Sell Checkout | Guests can collect Sell List intent, but confirmation requires sign-in and payout readiness. | Keep guest Sell List as list-building only until the user signs in. If guest sell checkout is supported later, it must first route through account creation and readiness; it should not run as an anonymous payout flow. | #1109, #1111, #1121 |
| Signed-in Sell Checkout | Signed-in confirmation can accept offers, publish fallback listings, record handoff evidence, and show payout readiness inside the Sell List route. | Make account, payout, and ship-from readiness explicit before confirmation. The final sell checkout should look like a simple review/confirm surface, not an operations console. | #1110, #1111, #1121, #1128, #1135 |
...
| Session schema | Keep `checkout_session_pages` final columns in the base table with no add-column compatibility guards. | Product is pre-launch, so local, staging, and proof databases should reset/bootstrap to the fresh schema instead of carrying old session payload convergence. | #1132, #1100 |
...
| Sell List confirmations | Use `checkout_sell_list_confirmation_pages` as the support-safe seller confirmation handoff record and delete execution/receipt fallback reads. | Execution and receipt pages are compatibility-shaped pre-launch staging surfaces, not the launch confirmation contract. | #1111, #1132, #1135 |
...
| Dense Sell List UI | Simplify review and confirmation; move fallback listing and offer handoff detail behind progressive disclosure or readiness. | Sell List checkout should not read like an operations console. | #1108, #1110, #1111 |
...
- fallback listing handoff details
...
- Identity: guest buy checkout may remain supported; guest sell confirmation should require account readiness.
