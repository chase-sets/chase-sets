...
The environment root, `staging.chasesets.com`, is the launch-facing staging marketplace entry point. It is delegated from the parent `chasesets.com` zone into its own DigitalOcean DNS zone and routed to the DOKS ingress load balancer.

When an environment root also receives mail through Google Workspace, it must not be a CNAME. Delegate the environment root as a child DNS zone, then use an apex A record for the DOKS load balancer so platform routing and certificates can coexist with exact-name Gmail MX/TXT records.

The Gmail-compatible environment-root record shape is:

- Parent zone: `NS staging` delegation to DigitalOcean nameservers.
- Child zone apex: environment-DNS Terraform-managed A routing for `staging.chasesets.com` to the DOKS load balancer.
...
- Child zone nested DOKS hosts: environment-DNS Terraform-managed A records for `www`, `marketplace`, and `admin`.
