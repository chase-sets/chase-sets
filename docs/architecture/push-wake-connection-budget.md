...
This is the per-environment DigitalOcean managed Postgres connection ledger for the push-first projection wake topology. `scripts/push-wake-capacity-evidence.mjs` computes the runtime profile; `scripts/staging-connection-envelope.test.mjs` enforces the staging transient phases in the script-test verification gate. Terraform enforces listener topology parity and least privilege. This ledger answers three questions before any push-wake phase is enabled:
...
- **In-pod seeds are short-lived direct clients with a deploy-time peak.** The API pod receives the same bootstrap Secret keys for representative commerce and admin-QA commands. Each command caps its query pools at one connection per unique URL and holds the schema-bootstrap advisory lock on a separate one-connection direct pool, independent of the runtime API's `DATABASE_POOL_MAX=6`. With the public profile's 20 context URLs, one control URL (shared with the work-signal URL), four separate waiter URLs, and the lock pool, one command configures **up to 26 direct backends** until it closes its pools; pools connect lazily, so unused waiter pools need not hold a backend. The deployment invokes the commands separately; manually overlapping two commands can double this demand. They add no *persistent* demand after exit, but their transient demand must be counted during deployments.
...
| Bootstrap (separate pre-upgrade phase) | 21 direct context/control URLs × Job-local pool max 1; worker relay quiesced | 21 |
| **Steady-state total** | 40 + 7 + 8 | **55 ≤ 94** (headroom 39) |
| **Deploy overlap** | 40 + (2 relays × 7) + (4 waiter contexts × 2 API generations × 2 replicas) | **70 ≤ 94** (headroom 24; 5 below the 75 backend rolling-deploy tier trigger) |

The bootstrap hook and advisory scenario-seed Job inherit Job-local `DATABASE_POOL_MAX=1` from canonical Helm runtime values. Both quiesce the worker relay before bootstrap; the hook is `pre-upgrade`, and the advisory, staging deploy, standalone representative, and admin-QA jobs share `platform-deploy-staging` job concurrency. The advisory dispatch occurs inside the deploy job but cannot start its Job until that deploy job releases the group. Manual and rerun launches use the same group. The advisory Job restores directly scaled workers after success or failure and resumes KEDA; bootstrap finishes before restoration.

The enforced phases are: pre-upgrade bootstrap or advisory `40 pooled + 8 API waiters + 21 direct bootstrap = 69/94`; rolling deployment `40 + 14 relays + 16 API waiters = 70/94` (below the existing 75 rolling-deploy trigger); representative or standalone seed command after rollout `40 + 7 + 8 + 26 = 81/94`. The command's 26 configured maxima include its separate direct schema-lock pool. These phases do not overlap through the named launch paths. The 80% trigger remains the **rolling-deploy** tier-upgrade policy, not a new hard ceiling for short-lived seed commands; their 81-backend phase is checked against the 94 budget and 97 non-reserved cluster slots. At #8171's revised head, concurrent advisory and in-pod seed maxima without this repair would be `40 + 7 + 8 + (21 × 4) + 26 = 165`, above 97; at the original `1f756f8` base the focused red case is 164 with 25 seed maxima. A one-per-URL advisory cap without ordering or quiescing would still yield `102`; a rolling overlap plus seed would yield `96` without the former four-backend reservation. The script-test gate rejects pool, URL-inventory, phase-order, worker-quiesce, or budget drift; `scripts/staging-connection-envelope.mjs` emits the computed numbers without credentials. Manually starting in-pod commands outside these workflow launch paths can overlap and is not admitted by this envelope.
...
| Bootstrap/maintenance (separate pre-upgrade phase) | 21 direct URLs × Job-local max 1; relay quiesced | 21 |
| **Steady-state total** | 33 + 7 + 4 | **44 ≤ 94** (headroom 50) |
| **Deploy overlap** | 33 + 2 × 7 + 2 × 4 (old/new listener generations; PgBouncer allocation does not grow with clients) | **55 ≤ 94** (headroom 39) |
...
The production pre-upgrade bootstrap phase is `33 pooled + 4 API waiters + 21 direct bootstrap = 58/94`; it is separate from rollout. The rolling-deploy overlap envelope (55) is deliberately pessimistic (both deployment generations' direct relay and waiter listeners held at once), but PgBouncer's server-side allocation is client-count-independent. On the current `db-s-2vcpu-4gb` production tier the tier-upgrade trigger is `75` backends (`floor(94 × 0.80)`), leaving 20 backends before the trigger. Each additional direct-listened source context costs 2 overlap backends.
...
- The live staging envelope is 55/94 steady and 70/94 during a pessimistic DOKS rollout (headroom 39 and 24 respectively; 5 before the 75 rolling trigger).
- Production overlap demand is 55/94 (headroom 39 to the hard limit, 20 to the 75 trigger). The current wave-2 direct-listener proposal adds 6 overlap backends to 61/94 and fits under both the hard tier limit and trigger.
...
- **Violation is a static evidence failure.** `staging-connection-envelope.test.mjs` exercises the actual Helm Job cap, advisory and standalone workflow ordering, quiesce, and aggregate phase budget with a negative control; the hosted script-test gate fails on drift. `push-wake-capacity-evidence.mjs` fails when a proposed listener expansion cannot fit any budgeted tier, and its regression tests pin current steady, overlap, and trigger values. The fix is to reduce direct demand or scale `database_size`, update the tier map, and update this ledger in the same change.
