...
- **In-pod seeds are short-lived direct clients.** The API pod receives the same bootstrap Secret keys for representative commerce and admin-QA commands; those commands open direct pools only for their runs. This adds no steady-state direct backend demand to the API connection budget.
