...
The representative-commerce-state command runs after the Helm upgrade, potentially alongside the advisory scenario-seed bootstrap Job. At the ledger's steady listener footprint and its existing four-backend bootstrap allowance, add **26 configured maxima** for the in-pod command: `40 pooled + 7 relay + 8 API waiters + 4 advisory bootstrap + 26 in-pod seed = 85/94` budgeted backends (9 headroom). Before the seed cap the same worst-case per-URL command demand was `25 × 6 = 150`, for `209/94` against that ledger. The admin-QA command has the same bound when run separately. The bootstrap allowance is a planning reservation, **not** an aggregate cap enforced on the advisory Job: its own 21 direct context/control URLs can theoretically open up to `21 × 4 = 84` connections, so concurrent worst-case pool maxima would be `40 + 7 + 8 + 84 + 26 = 165`, above the cluster's 97 non-superuser slots. Similarly, the pessimistic rolling listener overlap plus a command exceeds 94 (`74 + 26 = 100`). Do not treat 85 as an enforced cluster-wide bound; the scenario seed and rollout overlap need separate capacity enforcement if observed to coincide at their maxima. The seed cap removes the in-pod command's unbounded multiplier without changing the scenario bootstrap or runtime pools; aggregate advisory-Job enforcement is tracked by [#8172](https://github.com/chase-sets/chase-sets/issues/8172).
