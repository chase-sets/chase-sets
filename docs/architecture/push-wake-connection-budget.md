...
- **App Platform query traffic is pooled in every environment (#4655).** Terraform creates one managed PgBouncer transaction pool per active context database. App Platform API, worker, and interim bootstrap-job clients connect to PgBouncer; the cluster backends those clients can occupy are capped by the server-side pool `size` values in `context_database_connection_pool_sizes`, not by app pool maxima. The sum of those sizes is the worst-case backend footprint of all pooled traffic. #4655 added production query pools with their own budget entries (`production_context_database_connection_pool_size_overrides`), converging production onto the staging pooled shape and closing the last App Platform database-topology asymmetry; production no longer attaches App Platform database bindings.
...
| Demand | Math | Backends |
| --- | --- | --- |
| PgBouncer server-side allocation | Existing 17 context pools; client count does not increase this cap | 36 |
...
| **Steady-state total** | 36 + 7 + 8 + 4 | **55 ≤ 94** (headroom 39) |
| **Deploy overlap** | 36 + (2 relays × 7) + (3 API replicas × 4 waiters) + 4 | **66 ≤ 94** (headroom 28; 9 below the 75 backend tier-upgrade trigger) |
...
During the dual-estate cutover, App Platform and DOKS share the same 36-backend PgBouncer allocation instead of double-dipping query backends. The combined steady envelope is **59/94**: 36 pooled backends + 7 active-relay listeners + 12 waiter listeners (one App Platform API plus two DOKS APIs, four each) + 4 bootstrap/maintenance. With App Platform steady and DOKS rolling, the combined peak is **70/94**: 36 + 14 overlapping relay listeners + 16 waiter listeners (one App Platform API plus three DOKS APIs) + 4 bootstrap/maintenance. That leaves 24 hard-limit slots and 5 slots below the 75 tier-upgrade trigger. Simultaneously rolling both orchestration lanes, especially both bootstrap jobs, is still outside this envelope and requires a separately recorded migration budget.
...
- The dual-estate staging envelope is 59/94 steady and 70/94 with DOKS rolling (headroom 35 and 24 respectively; 5 overlap slots remain before the 75 upgrade trigger).
