...
- **In-pod seeds are short-lived direct clients with a deploy-time peak.** The API pod receives the same bootstrap Secret keys for representative commerce and admin-QA commands. Each command caps its own pools at one connection per unique URL, independent of the runtime API's `DATABASE_POOL_MAX=6`. With the public profile's 20 context URLs, one control URL (shared with the work-signal URL), and four separate waiter URLs, one command can hold **25 direct backends** until it closes its pools. The deployment invokes the commands separately; manually overlapping two commands can double this demand. They add no *persistent* demand after exit, but their transient demand must be counted during deployments.
...
The enforced phases are: pre-upgrade bootstrap or advisory `40 pooled + 8 API waiters + 21 direct bootstrap = 69/94`; rolling deployment `40 + 14 relays + 16 API waiters = 70/94` (below the existing 75 rolling-deploy trigger); representative or standalone seed command after rollout `40 + 7 + 8 + 25 = 80/94`. These phases do not overlap through the named launch paths. The 80% trigger remains the **rolling-deploy** tier-upgrade policy, not a new hard ceiling for short-lived seed commands; their 80-backend phase is checked against the 94 budget and 97 non-reserved cluster slots. Before this repair, the concurrent advisory and in-pod seed maxima were `40 + 7 + 8 + (21 × 4) + 25 = 164`, above 97. A one-per-URL advisory cap without ordering or quiescing would still yield `101`; a rolling overlap plus seed would yield `95` without the former four-backend reservation. The script-test gate rejects pool, URL-inventory, phase-order, worker-quiesce, or budget drift; `scripts/staging-connection-envelope.mjs` emits the computed numbers without credentials. Manually starting in-pod commands outside these workflow launch paths can overlap and is not admitted by this envelope.
