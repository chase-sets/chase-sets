...
| Marketplace | Marketplace | Listing create or price/quantity update | Listing detail/list read models | `fresh-read` detail and list recovery | Keep existing exact waits and detail `404` recovery. Stale unchanged `200` must be prevented by the existing read freshness dependency; add semantic `resource-updated` only if a concrete route proves a stale `200` can bypass that wait. | Critical | Watch; no semantic migration now |
...
| Marketplace listings | `bounded-contexts/marketplace/routes/account-listing.tsx` and `account-listings.tsx` use existing fresh-read redirects and exact Marketplace projection waits; stale detail `404` and projection freshness timeout are already temporary under `afterWrite`. Tests include listing route recovery and write consistency metadata coverage. |
