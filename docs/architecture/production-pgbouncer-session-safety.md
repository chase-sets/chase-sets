...
- `infrastructure/digitalocean/platform/main.tf` keeps projection wake listener URLs on dedicated wake-listener users and checks that they do not regress to owning context users, and provisions per-context production query pools (`production_context_database_connection_pool_size_overrides`).
