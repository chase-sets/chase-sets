...
- The only session-scoped path is the schema bootstrap (`infrastructure/bounded-context-runtime/schema.ts`), which holds a session advisory lock plus `SET lock_timeout` across multiple statements. It is a deploy-time migration path, not steady-state query traffic, and it stays on direct/session URLs: production runtime bootstrap is DOKS-owned and consumes the direct cluster URLs exported by `.github/workflows/platform-production.yml`; the interim App Platform bootstrap job rides the same pooled URLs staging has proven (single instance, worker quiesced, idempotent DDL).
...
| Schema bootstrap, migrations, grants, and production maintenance | DOKS-exported direct cluster URLs, Terraform/admin direct URLs, or the interim App Platform bootstrap job | Direct-only for the session advisory lock; the interim App Platform bootstrap job rides the staging-proven pooled URLs. |
