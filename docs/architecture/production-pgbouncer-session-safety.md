...
- The worker startup Settlement schema bootstrap selects `BOOTSTRAP_DATABASE_URL_SETTLEMENT` into a short-lived direct pool and closes it before activating marketplace label postage. Ordinary worker Settlement queries remain on `DATABASE_URL_SETTLEMENT`; standalone local startup may use its direct query URL when no bootstrap URL is exported.
