...
`LISTEN` requires direct or session-compatible connections. Terraform provides the active relay with per-context direct cluster URLs for the wave-1 source contexts (`checkout`, `marketplace`, `ordering`, `payments`). Staging and production build those URLs from the same expression using dedicated least-privilege `cs_<env>_<context>_wake_listener` database users (#1243). Preview environments intentionally omit listener URLs because push rollout never targets previews. Only the active relay holds listener connections; ordinary DOKS workers and API processes keep pooled query URLs. The per-environment connection ledger and the plan-time Terraform checks that keep this topology within DigitalOcean backend limits are documented in the [push-wake connection budget](./push-wake-connection-budget.md).
...
- **Operator/debug surface:** the only wake-store debug surface is the DOKS platform worker's internal `GET /internal/workers/status`; it returns aggregate intent counts, control flags, relay supervisor state, and lease/runner metadata only — never intent metadata, payloads, or error bodies.
