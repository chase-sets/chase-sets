...
`LISTEN` requires direct or session-compatible connections. App Platform context query URLs go through PgBouncer transaction pools in staging, which cannot carry `LISTEN`, so Terraform provides the active relay with per-context direct cluster URLs for the wave-1 source contexts (`checkout`, `marketplace`, `ordering`, `payments`). Staging and production build those URLs from the same Terraform expression using dedicated least-privilege `cs_<env>_<context>_wake_listener` database users (#1243) — production no longer reuses the full-DML App Platform database bindings for listeners, so the listener wiring path is structurally identical in both environments. Preview environments intentionally omit listener URLs because push rollout never targets previews. Only the active relay holds listener connections; ordinary App Platform staging workers and API processes keep pooled query URLs. Current DOKS staging is the exception: its Kubernetes Secret exporter writes ordinary query URLs directly from cluster host/context users, so DOKS staging pool changes are budgeted as direct until that exporter uses PgBouncer pool resources. The per-environment connection ledger and the plan-time Terraform checks that keep this topology within DigitalOcean backend limits are documented in the [push-wake connection budget](./push-wake-connection-budget.md).
...
- **Listener connections (staging and production):** Terraform builds `WORKER_LISTENER_DATABASE_URL_<CONTEXT>` from a dedicated per-context wake-listener database user (`digitalocean_database_user.wake_listeners[...]`, named `cs_<env>_<context>_wake_listener`) against the cluster's direct port, because `LISTEN` cannot ride the PgBouncer transaction pools. The credential is least-privilege (#1243): `CONNECT` on the context database — the only privilege the listener connection actually needs, since it issues `LISTEN`/`UNLISTEN` exclusively — plus `USAGE` on the schema and a **best-effort** `SELECT` on the event-store tables (`event_store_events`, `event_store_streams`) as defense-in-depth; the table grant is skipped with a warning when the cluster admin lacks grant authority on context-owned tables (the apply never fails on it, and re-granting after ownership changes is a `terraform taint terraform_data.wake_listener_database_grants` away). No DML and no access to domain tables. Grants are applied by `terraform_data.wake_listener_database_grants` inside the same `terraform apply` that creates the users, and the App Platform spec depends on that resource, so listener users are always granted before workers restart with the URLs. The plan-time check `wake_listener_least_privilege` fails any configuration whose listener URLs regress to the owning context users or App Platform bindings. Production relay enablement remains killed by default (`worker_projection_wake_relay_enabled = "false"`).
