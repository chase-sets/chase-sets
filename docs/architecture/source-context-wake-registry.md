...
The #1364 tier decision is now explicit: on the current `db-s-2vcpu-4gb` production tier, the rolling-deploy overlap budget can absorb one additional direct listener context, while wave 2 needs four (`catalog`, `fulfillment`, `identity`, `inventory`). Run `pnpm run ops push-wake:capacity-evidence -- --out artifacts/release-health/push-wake-capacity-evidence.json` for the checked-in no-secret evidence record.
