...
| `wave-4-deferred-or-not-eligible` | `auth`, `authenticity`, `collections`, `customer-feedback`, `notifications`, `pricing` | Keep deferred or not-yet-eligible source contexts explicitly classified while their relay fan-out remains disabled. Collections and Pricing inventory the Saved List valuation projection, and Collections registers its Saved List shared-page projection; both remain non-emitting until rollout approval and capacity evidence are recorded. |
