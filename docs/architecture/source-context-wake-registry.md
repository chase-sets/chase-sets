...
The #1364 tier decision is now explicit: on the current `db-s-2vcpu-4gb` production tier, the rolling-deploy overlap budget is fully allocated after adding Inventory as a direct listener, while the remaining wave-2 listener expansion needs three more direct contexts (`catalog`, `fulfillment`, `identity`). Run `pnpm run ops push-wake:capacity-evidence -- --out artifacts/release-health/push-wake-capacity-evidence.json` for the checked-in no-secret evidence record.
