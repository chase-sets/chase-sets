...
The tier decision is now explicit: on the current `db-s-2vcpu-4gb` production tier, the rolling-deploy overlap budget is 72/94 and the 80% tier-upgrade trigger is 75. The remaining wave-2 listener expansion needs two more direct contexts (`catalog`, `fulfillment`) and fits at 76/94, above the trigger, so it now requires `db-s-4vcpu-8gb` or production transaction pools first (#3342). Adding a 3rd `platform-api` instance or a 2nd `platform-worker` instance also crosses the trigger and needs the same capacity work. Run `pnpm run ops push-wake:capacity-evidence -- --out artifacts/release-health/push-wake-capacity-evidence.json` for the checked-in no-secret evidence record.
