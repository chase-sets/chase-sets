...
| Staging push loop | Source-context wake registry (wave-1 `checkout`/`marketplace`/`ordering`/`payments`, wave-2 `catalog`, and wave-3 `settlement` = `staging-enabled`, emission + relay fan-out on) | `checkout` live since 2026-06-10 ~21:00 UTC; staging canary latencies above are push-accelerated checkout evidence. Wave-1 remainder enabled 2026-06-11 (with the #1224 migration inventory). `catalog` is staging-enabled for Source Observation import/review freshness; `settlement` is staging-enabled for payout-readiness projection freshness that gates seller checkout readiness. |
