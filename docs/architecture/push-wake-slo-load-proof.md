...
- CI-safe connection budget evidence for #1363/#1364 now exists (`push-wake-capacity-evidence/v1`), but it is not a production-like volume load proof. It records that remaining wave-2 direct listener expansion fits the current production overlap envelope after deployable consolidation (`58/94` if `catalog` and `fulfillment` also get direct LISTEN URLs), so the remaining gate is route/worker rollout plus notification-latency evidence rather than a default database tier bump.
