...
- CI-safe connection budget evidence for #1363/#1364 now exists (`push-wake-capacity-evidence/v1`), but it is not a production-like volume load proof. It records that remaining wave-2 direct listener expansion fits the hard current production overlap envelope (`76/94` if `catalog` and `fulfillment` also get direct LISTEN URLs) but crosses the 80% trigger, so capacity work is required before that expansion.
