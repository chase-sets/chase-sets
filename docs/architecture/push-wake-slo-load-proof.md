...
- CI-safe connection budget evidence for #1363/#1364 now exists (`push-wake-capacity-evidence/v1`), but it is not a production-like volume load proof. It records a current production overlap envelope of `59/94`; the wave-2 direct-listener proposal raises it to `65/94`, below both the hard limit and the `75/94` tier trigger.
