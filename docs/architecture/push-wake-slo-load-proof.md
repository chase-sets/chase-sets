...
| Connection/capacity budgets | [Push-Wake Connection Budget](./push-wake-connection-budget.md) + Terraform plan-time checks `wake_connection_budget`, `wake_connection_budget_tier_upgrade_trigger`, `wake_listener_topology_parity` + CI-safe `pnpm run ops push-wake:capacity-evidence` | Staging steady-state 50/94, deploy overlap 60/94; production steady-state 37/94, deploy overlap 70/94. Production has current-tier and 80% trigger headroom for the remaining wave-2 direct LISTEN expansion, but adding a 3rd `platform-api` instance or 2nd `platform-worker` instance requires `db-s-4vcpu-8gb` or production transaction pools first. |
