...
| Connection/capacity budgets | [Push-Wake Connection Budget](./push-wake-connection-budget.md), current Terraform pool/listener topology, and canonical Helm runtime values | Review both steady and rolling-overlap demand before changing replicas, runners, pools, waiters, or listener coverage. |
...
- Checked-in configuration evidence is not a production-like volume load proof. Any listener expansion still requires a fresh calculation from the Terraform and Helm sources plus live load evidence.
