...
| Staging push loop | Source-context wake registry (wave-1 `checkout`/`marketplace`/`ordering`/`payments`, wave-2 `catalog`/`inventory`, and wave-3 `settlement` = `staging-enabled`, emission + relay fan-out on) | `checkout` live since 2026-06-10 ~21:00 UTC; staging canary latencies above are push-accelerated checkout evidence. Wave-1 remainder enabled 2026-06-11 (with the #1224 migration inventory). `catalog` is staging-enabled for Source Observation import/review freshness; `inventory` is staging-enabled for reservation outcome and supply freshness; `settlement` is staging-enabled for payout-readiness projection freshness that gates seller checkout readiness. |
...
| Connection/capacity budgets | [Push-Wake Connection Budget](./push-wake-connection-budget.md) + Terraform plan-time checks `wake_connection_budget`, `wake_listener_topology_parity` + CI-safe `pnpm run ops push-wake:capacity-evidence` | Staging steady-state 44/94, deploy overlap 48/94; production steady-state 48/94, deploy overlap 92/94. Production has one current-tier direct listener slot; wave-2 direct LISTEN expansion needs a tier/overlap decision before enablement. |
...
- CI-safe connection budget evidence for #1363/#1364 now exists (`push-wake-capacity-evidence/v1`), but it is not a production-like volume load proof. It records that wave-2 direct listener expansion does not fit the current production overlap envelope (`100/94` if all four wave-2 contexts get direct LISTEN URLs) and recommends `db-s-4vcpu-8gb` unless an evidence-backed overlap relaxation or pool/instance reduction is chosen.
