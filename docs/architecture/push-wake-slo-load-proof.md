...
| Staging push loop | Source-context wake registry (`checkout` = `staging-enabled`, emission + relay fan-out on) | Live since 2026-06-10 ~21:00 UTC; staging canary latencies above are push-accelerated evidence |
...
| Interest-index lookup time/staleness | partially instrumented | Index version visible per cursor + worker; no lookup-latency metric — accepted gap (in-memory map) |
