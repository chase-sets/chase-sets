...
| R02 retired legacy paths | Hard-coded retired artifacts, including old deployables, old generated deployable scripts, `contracts/dev-seeds`, `contracts/sellable-units`, and the catalog authoring package, must not exist. | Prevents deleted architecture from creeping back in under familiar names. | Delete the artifact or move the behavior to the current manifest-driven surface. |
...
| R67 API deployable names | `apiDeployables`, `runtimeDeployables`, and `sourceRuntimeDeployables` must use the known deployable names for their category. | Mount and runtime ownership must align with real hosts. | Use `platform-api`, `admin-support-api`, `platform-worker`, or `admin-support-worker` as appropriate. |
...
| R70 projection group schema | `projectionGroups` need unique non-empty names, source context names, owned tables, valid reset strategy, and optional boolean bootstrap flag. | Read-model ownership and rebuild behavior must be explicit. | Declare the projection group and table owners; use one of the supported reset strategies. |
