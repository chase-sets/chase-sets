...
| R41 context root index | Context root `index.ts` may normally export only `contextManifest` and `module`, and must not re-export secondary public surfaces. The Channels root additionally exposes its exact closed Channel Publication contract, including its canonical closed delist validator; the structure gate rejects missing or additional members. | The root is the deployable plug-in contract, not an aggregate barrel. Channels also owns one ratified same-context publication port whose closed member table is enforced directly. | Remove `client`, `server`, `web`, and `seed-support` re-exports from root index. For Channels, preserve the exact publication-port member table and its one canonical delist validator without exporting the production descriptor table. |
