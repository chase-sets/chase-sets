...
Use OpenTelemetry in `platform-api` and an open-source LGTM stack for local/self-hosted observability. The application emits OTLP traces and metrics, keeps logs as JSON stdout, and propagates W3C `traceparent` / `tracestate` instead of custom correlation headers.
...
The implementation lives in `infrastructure/observability` so deployables remain thin composition roots and bounded contexts stay focused on domain behavior.
