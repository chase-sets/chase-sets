...
- Production marketplace launch additionally requires `PRODUCTION_MARKETPLACE_PUBLIC_ENABLED=true`, approved Tax readiness evidence, Stripe live-mode keys, production Connect return and refresh URLs on `https://marketplace.chasesets.com`, EasyPost production mode, and complete Amazon SES transactional email configuration. Keep the switch off while production remains landing/admin-support only.
