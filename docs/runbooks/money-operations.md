...
```bash
pnpm run ops settlement:payout-account-migration-report --database-url "$SETTLEMENT_DATABASE_URL" --environment staging --checked-at "$(date -u +%Y-%m-%dT%H:%M:%S.000Z)"
```
...
- Keep existing payout-ready legacy hosted-dashboard accounts from losing payout access; do not mutate dashboard access, responsibilities, or connected-account ids as a rollback shortcut.
