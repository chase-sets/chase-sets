...
- Checkout uses explicit fee-category selection for launch (`card`, `us_bank_account`, or platform credit) so Marketplace Checkout Fee quotes match the provider request. Card-category sessions dynamically offer card-rail methods (cards, Apple Pay, Google Pay, and Link); bank-category sessions remain `us_bank_account` only. Do not enable methods with different fee or settlement semantics without adding a matching category and quote policy first.
