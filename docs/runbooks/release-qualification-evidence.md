...
Exact identity handoff: Platform PR captures the digest emitted by the actual `docker push`, verifies the immutable registry reference, and uploads a run-attempt-bound candidate record as a non-enforcing artifact. GitHub leaves `actions/runs/<id>.pull_requests` empty for real `merge_group` runs, and `commits/<merge-group SHA>/pulls` only materializes after the PR merges (live merge-group run 29888165410), so neither can source identity at producer time. Instead, the producing merge_group run snapshots the GraphQL merge queue (`repository.mergeQueue(branch).entries`) while its own candidate is still queued and requires exactly one still-queued entry whose immutable `headCommit.oid` is the run's merge-group SHA, in a pre-merge state, with `baseCommit.oid` equal to the merge-group event base; that entry's `pullRequest` number/headRefOid is the candidate PR identity. The temporal guard is structural: GitHub removes the entry at merge or dequeue, so a post-merge replay finds no entry and the producer refuses; the run metadata must also still be in flight. The raw snapshot and run metadata are retained beside `candidate.json` in the run-attempt artifact. That record binds the Platform PR workflow ID/path, run ID/attempt, queue base SHA, and the queue-entry PR/head set. Release discovery independently re-reads the post-merge commit association (`commits/<merge-group SHA>/pulls`, populated only after merge, requiring `merge_commit_sha`, base SHA, and head SHA agreement), exhaustively paginates merge-group runs and exact-attempt artifacts, and requires a byte-equivalent candidate identity before persisting a closed release-linkage artifact. Production consumes the dispatch artifact and records its artifact ID/name, workflow identity, run attempt, candidate SHA/tree/digest, and distinct final merge SHA/tree. It never identifies lineage from a queue branch name or from tree equality. The advisory workflow never rereads a mutable tree tag. Its gate requires the captured sha256 digest. Candidate-level `passed` or `failed` evidence requires exact built/gate digests plus candidate SHA/tree echoes; failures before immutable resolution are `infrastructure_error` with identity explicitly unavailable.
