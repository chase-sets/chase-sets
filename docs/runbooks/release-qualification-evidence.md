...
Exact identity handoff: Platform PR captures the digest emitted by the actual `docker push`, verifies the immutable registry reference, and uploads a run-attempt-bound candidate record as a non-enforcing artifact. The advisory workflow never rereads a mutable tree tag. Its gate requires the captured sha256 digest. Candidate-level `passed` or `failed` evidence requires exact built/gate digests plus candidate SHA/tree echoes; failures before immutable resolution are `infrastructure_error` with identity explicitly unavailable.
...
The plan persists the original policy decision before the gate. Publishers and observers consume that run-attempt record; they never re-evaluate a later policy to classify an older attempt. `.github/workflows/platform-merge-qualification-terminalizer.yml` observes the advisory workflow, paginates jobs/artifacts and verifies API totals, binds repository/workflow path/workflow ID/run/attempt, and checks the exact attempt artifact name. Cancellation is evaluated before any disabled classification. A skipped publisher counts as disabled only with a valid bound disabled decision. Every emitted event carries a deterministic idempotency key, so redelivery is a byte-identical duplicate instead of a contradictory terminal.
