...
The advisory qualification chain in `.github/workflows/platform-pr.yml` (jobs `merge-qualification-plan`, `merge-qualification-gate`, `merge-qualification-advisory`) plus the independent `.github/workflows/platform-merge-qualification-terminalizer.yml` observer ships behind the checked-in default-off policy at `scripts/merge-qualification-policy.json` (`merge-qualification-policy/v1`, validated by `scripts/merge-qualification-advisory.mjs`). The policy validates fail-closed with a recursively closed schema: an absent, disabled, expired, or malformed policy — unknown top-level or nested `ceiling` fields, date-only or timezone-less instants, `expiresAt` not strictly after `enabledAt`, or `expiresAt` more than 30 days after `enabledAt` (`enabledAt < expiresAt <= enabledAt + 30 days`) — behaves as disabled with a visible advisory summary note, no provider mutation, no qualification records, and no advisory check assertion. Enablement values (owner, ceiling, `enabledAt`, `expiresAt`) are recorded by #5881 at enablement; the expiry clock starts at enablement, and advisory automation stops automatically at expiry.

Exact-digest identity: the Docker Image Build job exposes the immutable digest it pushed as a job output, the reusable-gate call receives it as `image_digest` (the gate refuses to deploy any other digest), and the publisher compares candidate SHA, tree SHA, and digest end to end — the gate's resolved identity must echo the plan's candidate and this run's built digest, and a `passed` event without the exact valid digest is rejected. A retagged `tree-<tree>` reference after the build can therefore never substitute a different image into a passed record.

Red-run posture (enabled policy only): reusable-call jobs cannot use `continue-on-error`, so a failing enabled gate marks the merge-group workflow RUN conclusion failed (red) while remaining non-required — `PR Required` has no advisory dependency and the merge queue enforces only configured required checks. The #5881 soak observes and rules on this before any required-gate adoption. The runnable plan/publisher jobs do carry `continue-on-error`.
...
| `passed` | isolated candidate; reused merge-gate workflow succeeded against the exact built digest with matching SHA/tree/digest echoes | gate namespace (torn down by the gate's always() finalizers) | classifies the next candidate fresh |
...
| `cancelled_evicted` | cancellation/eviction at ANY stage — plan, Docker image, gate, publisher, or whole-run/force cancellation — with a stage-distinct reason code | possibly (finalizers + scheduled gate sweep backstop) | classifies the next candidate fresh |
| `infrastructure_error` | plan crashed, classifier output missing, candidate image unavailable, gate not invoked, gate identity/digest mismatch, or advisory result missing after a completed run | never | classifies the next candidate fresh |

Exactly-once terminalization: the in-run publisher runs `if: always()`, but GitHub only re-evaluates cancellation conditions for queued/running jobs and force cancellation bypasses `always()` entirely. The `workflow_run` terminalizer observer closes that gap: after every completed merge-group run it re-evaluates the policy fail-closed, no-ops when the run already carries a `merge-qualification-events-*` artifact or the publisher was skipped by a definitive disabled decision, and otherwise emits the missing terminal event (`cancelled_evicted` with the earliest cancelled stage, or `infrastructure_error`/`advisory_result_missing`). The event is built from run-derived data only, so re-runs rebuild a byte-identical record that the canonical reader deduplicates. A candidate that still ends up with no valid event (for example the terminalizer itself failed) surfaces as an orphan in delivery health.

Disabled steady state (this leaf's shipped state, and the day-after path in both directions): the plan job posts the disabled note and the publisher job is SKIPPED on a definitive disabled decision, so a disabled policy creates no green `Merge Qualification (advisory)` check at all — no provider mutation, no records, no assertion. The publisher still executes (and re-evaluates the policy itself, fail-closed) only when the plan crashed or was cancelled and its outputs are unavailable. Re-enabling requires a reviewed policy change (registered release-machinery surface in the classifier registry), and an overnight expiry lands the next run back in the disabled note with `policy_expired`.

Canonical delivery health (scheduled production path): `scripts/release-health-delivery-health.mjs` collects `merge-qualification-events-*`/`merge-qualification-terminal-*` artifacts from merge-group Platform PR runs and terminalizer runs, builds the candidate inventory from merge-group head SHAs, validates and deduplicates every event to one authoritative attempt per candidate (retries select the latest attempt; same-attempt contradictions are exposed conflicts), resolves each staged release's tree SHA from its commit object, executes the tree/digest/time-safe join in-process, and records provider headroom threaded from the gate preflight (`headroom_runs`). Every collection or validation failure degrades `completeness` and suppresses alerting. The join requires the release to complete after the candidate's qualification, exact digest agreement when both sides pin one (a disagreement is an explicit `identity_mismatch` orphan, never a catch), and selects deterministically among multiple qualifying releases (exact digest match, then earliest completion, then lowest main SHA) — never branch names. Terraform/provider-topology staging failures are classifier-routing evidence; application/contract failures after a passed qualification are staging catches. Persistent staging remains required for every merged deployable candidate regardless of policy state.
