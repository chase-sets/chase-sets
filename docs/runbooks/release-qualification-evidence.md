...
Exact identity handoff: Platform PR captures the digest emitted by the actual `docker push`, verifies the immutable registry reference, and uploads a run-attempt-bound candidate record as a non-enforcing artifact. That record also binds the Platform PR workflow ID/path, run ID/attempt, queue base SHA, and the API-observed PR/head set. The release-candidate dispatcher exhaustively paginates merge-group runs and exact-attempt artifacts, downloads that record, and persists a closed release-linkage artifact before dispatch. Production consumes the dispatch artifact and records its artifact ID/name, workflow identity, run attempt, candidate SHA/tree/digest, and distinct final merge SHA/tree. It never identifies lineage from a queue branch name or from tree equality. The advisory workflow never rereads a mutable tree tag. Its gate requires the captured sha256 digest. Candidate-level `passed` or `failed` evidence requires exact built/gate digests plus candidate SHA/tree echoes; failures before immutable resolution are `infrastructure_error` with identity explicitly unavailable.
