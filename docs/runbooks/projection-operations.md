...
- Platform-worker `/internal/workers/status` and worker heartbeat metadata include `databasePoolPressure`. Use `waitingClients > 0`, `waitingPoolCount > 0`, or saturated pools to attribute freshness lag to DB-pool pressure instead of hot-lane queueing or projection-group lease contention.
