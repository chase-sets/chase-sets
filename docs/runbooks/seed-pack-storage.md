...
1. ordered accepted Observation Pack ids and versions;
...
The first compatible restore downloads the immutable set into `artifacts/representative-snapshot-cache`; later restores validate and reuse that cache. After restoring all dumps and assets, the command runs the normal bootstrap with only `critical-bootstrap`, `catalog-integration-bootstrap`, and `representative-catalog`, then recomputes the post-replay verifier digest and requires exact equality with the publish manifest.
