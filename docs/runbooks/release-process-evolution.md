...
Normalized scope excludes lockfile-only churn, generated registries/metadata under a `generated/`/`__generated__/` path, snapshots (`.snap`/`__snapshots__/`), and vendored/binary files (GitHub reports zero additions and zero deletions with no patch — the only diff-metadata-provable binary signal, so an oversized text diff with an omitted patch is never mistaken for binary and its raw size is never hidden). There is no formatting-only exclusion: decision #5971 (Option A) removed it entirely rather than narrowing its proof rule, after a sorted-content comparison was shown to admit an arbitrary within-file statement permutation as "formatting." Reindentation, whitespace-only diffs, and every other content permutation count fully toward normalized scope for the duration of the 14-day advisory calibration; a separately reviewed, narrower proof rule may reintroduce a bounded formatting exclusion only after that calibration data supports one. Tests, migrations, workflows, and handwritten generator sources always count. Thresholds are exact and inclusive of the boundary itself: advisory at normalized lines > 1,000 or files > 20; large at normalized lines > 2,500 or files > 35.
