...
The deployment source has a v2 default everywhere. The exact external value that selects v1 is the GitHub `vars.STRIPE_CONNECT_ACCOUNTS_API` value set to the literal `v1`; the DOKS workflows pass that value directly into the runtime environment. If the GitHub variable is unset, the workflow passes `v2`.

- **Preview:** the PR workflow uses `vars.STRIPE_CONNECT_ACCOUNTS_API || 'v2'` and passes it as `STRIPE_CONNECT_ACCOUNTS_API` to the preview Kubernetes deployment. With the variable unset, preview runs v2. The canonical Helm runtime values also default to v2.
- **Staging:** the normal DOKS deployment maps `vars.STRIPE_CONNECT_ACCOUNTS_API || 'v2'` directly into the Kubernetes runtime configuration. Staging selects v1 only while the environment variable remains explicitly set to `v1`.
- **Production:** the production DOKS deployment uses the same v2 fallback. Production selects v1 only when an active proof/public environment explicitly sets the GitHub variable to `v1`; the repository default and landing-only path select v2.
- **Runtime injection:** `platform-kubernetes-deployment.mjs` passes the selector to the Helm release, and both platform API and platform worker read the resulting `STRIPE_CONNECT_ACCOUNTS_API` value at startup.
