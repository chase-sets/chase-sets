...
Production sets `SMOKE_REQUIRE_MARKETPLACE=true` only when `PRODUCTION_MARKETPLACE_PUBLIC_ENABLED=true` after launch, Marketplace Checkout Fee, Support operations, Fulfillment postage, and Tax approval evidence has passed validation. The current production posture should not include a marketplace URL in smoke output.
