...
3. Validates required production secrets and variables. When `PRODUCTION_MARKETPLACE_PUBLIC_ENABLED=true`, validation also requires approved Tax readiness evidence, Stripe live-mode keys, Stripe webhook secret, EasyPost production configuration, production Connect return/refresh URLs, and complete Amazon SES transactional email configuration.
