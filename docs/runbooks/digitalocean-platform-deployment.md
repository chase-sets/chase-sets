...
App Platform owns production CNAMEs while its live domains are attached. Terraform imports those records only for `prepare-doks` TTL lowering, then releases their state identities before the forward plan. The provider retains computed domains for an empty desired list, so DOKS mode replaces apex, `www`, `admin`, and conditional marketplace domains with `app-platform.chasesets.com`; every warm fallback route is explicitly authority-qualified to that parking attachment. DOKS A creates depend on the completed app update. In reverse, the DOKS A resources and apex are deleted before the app update restores the live attachments, allowing App Platform to recreate its records without a competing Terraform CNAME create.
