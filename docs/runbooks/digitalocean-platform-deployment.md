...
This runbook covers the current DigitalOcean platform deployments. PR previews run as per-PR Kubernetes namespaces on the shared DOKS cluster with disposable in-cluster Postgres; staging and production runtime compute also serve from DOKS. The retained App Platform Terraform root owns the warm production rollback target and transitional DNS/domain machinery until the post-soak decommission in #4055. [ADR 0018](../adr/0018-doks-compute-runtime.md) records the accepted pre-launch decision to migrate compute/runtime orchestration to DOKS after #101 deploy stabilization and #97 beta-clock start.

## Architecture

- Regions: DOKS runtime compute, managed Postgres, and Spaces run in `nyc3`; the retained App Platform rollback target runs in `nyc` during the production soak.
...
- Deploy orchestration: GitHub Actions is the canonical deploy owner. Label-gated PR previews and staging build one platform container image in GitHub Actions with bounded Docker Buildx cache, push it to DigitalOcean Container Registry, and record the digest. PR previews deploy that image through Helm into `chase-sets-pr-<number>` namespaces; staging and production deploy the immutable image through Helm to their DOKS clusters. Production verifies and promotes the staging-built commit image instead of rebuilding a second artifact. A change-scope classifier gates CI and CD work so documentation-only, workflow-only, and non-deployable changes do not build images or deploy runtime infrastructure.
...
- Production branch: `production` is a smoke-verified deployed release marker. The production workflow fast-forwards it only after the DOKS Helm deployment and production smoke pass. It also creates an annotated `release-<yyyymmddHHMMSS>-<sha>` Git tag and a matching DOCR image tag for audit and rollback.
...
- Image groups are intentionally deferred. The platform deploys one shared image across its DOKS workloads and the retained App Platform rollback components because splitting deployables into separate image groups would add Docker, registry, Helm/Terraform, promotion, rollback, and smoke-test complexity before there is enough deployment data to justify it.
...
Use the delegated child-zone primary-domain mode for root environment hosts that must support both runtime routing and Google Workspace mail. The DOKS staging deploy lane waits on HTTPS ingress readiness for the landing, admin, marketplace, and root marketplace hosts (the 4 hosts DOKS actually serves) before smoke checks run; legacy dash-based redirect hosts are excluded from this gate (#5564) since App Platform's redirect routes go empty once DOKS owns serving. The staging reset workflow still owns the temporary App Platform attachment repair path until App Platform decommission; a root-domain attachment that stays in `CONFIGURING` blocks reset smoke checks because App Platform routing and certificate validation are not healthy.
...
Run the `Platform Staging Rollback Drill` workflow (`.github/workflows/platform-staging-rollback-drill.yml`) quarterly, and before closing #3334 for the first time. It is staging-only and uses the shared `platform-deploy-staging` concurrency group so it cannot race a normal staging deploy. Manual dispatch requires the exact confirmation phrase `run staging rollback drill`, a prior known-good Helm revision, and a `rollback_reference` link showing how the revision was selected.

The workflow configures the staging DOKS context from `doks/staging.tfstate`, captures Helm history for release `chase-sets-platform` in namespace `chase-sets-platform`, and refuses a missing or already-current target revision. It records the current revision as the mandatory roll-forward target, uses `platform-kubernetes-deployment.mjs rollback` with the staging rollout workload map to serve the prior revision, runs staging smoke, restores the captured revision even when rollback smoke fails, and smokes again. It must never run with `DEPLOYMENT_ENVIRONMENT=production`, read production DOKS state, or mutate production DNS or serving variables.

The evidence artifact is `artifacts/staging-rollback-drill/staging-rollback-drill.json` with 30-day retention. Success requires `phases.rollback`, `phases.rollbackSmoke`, `phases.rollForward`, and `phases.rollForwardSmoke` to be `success`. The artifact is support-safe: it records the Helm release, namespace, original and target revisions, phase outcomes, workflow identity, and non-secret errors only; it must not contain kubeconfig content, credentials, connection strings, event payloads, or customer data. #3334 should remain open until one live successful workflow run is linked with evidence showing the prior revision served, health checks green on it, and restoration completed.
...
Before the production Helm deploy, the production workflow resolves the smoke-verified `production` marker commit, finds the matching `release-*` tag, verifies the rollback image still exists in DOCR, captures the DOKS release/namespace/workload target, and writes:
...
If post-deploy production smoke, Stage 1 canary, proof-mode Buy Now, or settlement provider-health verification fails after readiness succeeded, the workflow runs `platform-kubernetes-deployment.mjs rollback` for Helm release `chase-sets-platform` in namespace `chase-sets-platform`, waits for every Deployment/Rollout to become healthy, and retains support-safe diagnostics. The production marker is not advanced for the failed revision. The release-health record reports recovery mode `rollback`, the rollback target commit, and the workflow-generated rollback reference.
...
The preview, staging, and production Kubernetes workloads share the same runtime image and differ only by run command, environment, scaling, health checks, and ingress routing. Staging and production DOKS workloads retain the existing managed Postgres paths; the compute cutover did not move the database topology.
...
14. Waits for landing, admin, marketplace, and staging root marketplace domains — the 4 hosts DOKS actually serves (see `buildDoksIngressHosts()` in `scripts/render-platform-helm-values.mjs`). Legacy dash-based redirect hostnames are intentionally excluded from this gate (#5564): their App Platform routes go empty once DOKS owns serving, so probing them was a stable, deterministic 404 that blocked every deploy; legacy redirect behavior is instead exercised by the staging admin-smoke/rollback-drill/bootstrap-hook-drill workflows. Staging root is deployment-gated because it is the launch-facing marketplace root and App Platform routing/certificate validation depends on child-zone records while Gmail MX/TXT records coexist at the same owner name.
...
Production serving changes are deliberately two-invocation operations. Never change `PRODUCTION_APP_SERVING` and begin a preparation phase in the same invocation. The workflow reconciles exactly one A/CNAME owner per affected live name, rejects duplicates or an unknown owner, and stores the applied TTL-preparation phase, timestamp, and observed previous maximum TTL in platform Terraform state. The forward preparation also attaches the zone-backed `app-platform.chasesets.com` alias while every live domain remains attached; a second state marker is committed only after that hostname resolves, has trusted TLS, and serves the old deployment with exact HTTP 200. A later swap is refused until both markers are present, every affected live record is at 300 seconds or less, the stored previous TTL has elapsed, and a fresh parking probe passes.

App Platform owns production CNAMEs while its live domains are attached. Terraform imports those records only for `prepare-doks` TTL lowering, then releases their state identities before the forward plan. The prepared app has apex, `www`, `admin`, conditional marketplace, and the parking alias attached together; every parking route is explicitly authority-qualified. DOKS mode removes only the live attachments and promotes parking to primary, so the already-certified old deployment remains reachable throughout the detach. DOKS A creates depend on the completed app update and parking preparation. In reverse, the DOKS A resources and apex are deleted before the app update restores the live attachments, while the parking hostname remains attached as the warm target.

Forward cutover:

1. Keep `PRODUCTION_APP_SERVING=app-platform`. Set `PRODUCTION_SERVING_DNS_PHASE=prepare-doks` and run Platform Deploy. This separate managed change lowers the live App Platform CNAME TTLs to at most 300 seconds, records apply time plus the prior TTL, attaches the parking alias without removing any live domain, and polls until parking returns exact HTTPS 200 with valid TLS. It must not replace a serving record or detach a live domain.
2. Wait until the recorded prior TTL has elapsed. Keep `PRODUCTION_SERVING_DNS_PHASE=prepare-doks`, set `PRODUCTION_APP_SERVING=doks`, and run Platform Deploy again. The workflow also requires the DOKS certificate preflight, validates the retained parking marker/attachment/CNAME, runs a fresh exact-200 TLS probe, and refuses with a named parking reason before any state release if one check fails. Only then may it release the prepared CNAME state identities and plan the exact live App Platform domain detaches.
3. After DOKS smoke is green, set `PRODUCTION_SERVING_DNS_PHASE=steady`. If a routine deploy arrives first with the completed `prepare-doks` value still configured, the workflow treats that phase as completed, plans with steady inputs, and rejects any serving-topology mutation. DOKS records remain at 300 seconds or less through the rollback-confidence window.
...
2. Wait for that recorded TTL to expire. Keep `PRODUCTION_SERVING_DNS_PHASE=prepare-app-platform`, set `PRODUCTION_APP_SERVING=app-platform`, and run Platform Deploy. The guarded graph removes the DOKS apex and leaf A records before restoring the live domains; the verified parking hostname remains attached as an alias throughout the rollback and App Platform then recreates its live CNAME and apex records.
3. After rollback smoke and resolver convergence are green, set `PRODUCTION_SERVING_DNS_PHASE=steady`. A delayed cleanup is safe: the completed `prepare-app-platform` value is normalized to steady for routine deploys, while only `prepare-doks` can begin another forward transition. App Platform remains the DNS owner and the workflow continues to round-trip the warm parking marker; do not import live CNAMEs outside a future `prepare-doks` phase.

The production destructive-plan approval remains an independent boundary: review the exact plan fingerprint and enumerated addresses before either swap invocation. The inspector represents nested app removals as `digitalocean_app.platform.domain["<hostname>"]`, so a forward approval must name every released live attachment; rollback names only the deleted DOKS records because parking is retained. Do not bypass either preparation gate with a manual Terraform apply or direct DNS edit.
