...
3. After rollback smoke and resolver convergence are green, set `PRODUCTION_SERVING_DNS_PHASE=steady`. App Platform remains the DNS owner and the workflow continues to round-trip the warm parking marker; do not import live CNAMEs outside a future `prepare-doks` phase.
