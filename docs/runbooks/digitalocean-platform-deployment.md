...
The staging environment root, `staging.chasesets.com`, is delegated from `chasesets.com` into its own DigitalOcean DNS zone and attached to App Platform as the staging app's primary domain in that child zone. This mirrors production apex ownership by making the staging root a real apex: platform A/AAAA routing records coexist with exact-name Google Workspace MX/TXT records at the same owner name. A managed App Platform subdomain alias or self-managed alias is not correct for this host because those modes expect CNAME ownership and conflict with exact-name mail.

When Google Workspace mail is enabled for an environment root, configure that root like the production apex. For staging, the parent zone owns only the `NS staging` delegation; the child zone owns Gmail MX/SPF, optional Google DKIM, SES bounce/DKIM/DMARC, the asset CDN CNAME, platform A/AAAA records at the apex, and CNAME records for App Platform subdomain aliases. No CNAME exists at the environment root.
