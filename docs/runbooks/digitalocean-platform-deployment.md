...
The cleanup workflow runs with the trusted base workflow definition, checks out the PR base commit, initializes Terraform with `platform/previews/pr-<number>.tfstate`, waits for any active App Platform deployment to finish, disables the platform root's stateful `prevent_destroy` guard in the ephemeral checkout, and runs `terraform destroy -auto-approve`. That bypass is limited to the approved preview cleanup workflow so ordinary plans and ad hoc targeted destroys still fail closed.
