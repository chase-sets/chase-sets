...
14. While `PRODUCTION_MARKETPLACE_PROOF_ENABLED=true`, signs in with the same proof operator credential pair and calls the authenticated Settlement provider-health endpoint. It writes `artifacts/release-health/production-settlement-provider-health-probe.json`, emits the `provider-health-checked` settlement operation telemetry used to anchor payout-error Prometheus canary evidence, waits for OTLP export and Prometheus scraping (`PRODUCTION_SETTLEMENT_CANARY_SCRAPE_WAIT_SECONDS`, default 90 seconds), and blocks the production marker when it fails.
