...
- `PRODUCTION_PROOF_CANARY_EMAIL` / `PRODUCTION_PROOF_CANARY_PASSWORD` (secrets): operator account with access to the permission-gated proof marketplace host. Required while proof mode is enabled; the canary step fails closed without them.
...
13. While `PRODUCTION_MARKETPLACE_PROOF_ENABLED=true`, runs the proof-mode [Buy Now Freshness Canary](./guest-buy-now-freshness-canary.md) (authenticated flow with `PRODUCTION_PROOF_CANARY_EMAIL`/`PASSWORD` against the permission-gated proof marketplace host, same readiness budget and negative probe as staging). It writes `artifacts/release-health/production-proof-buy-now-freshness-canary.json`, records the result in the job summary, and blocks the production marker when it fails.
14. Adds a matching `release-<yyyymmddHHMMSS>-<sha>` DOCR tag to the promoted image, creates the annotated Git release tag, and fast-forwards the protected `production` branch to the smoke-verified deployed release commit.
15. Writes a `release-health/v1` artifact with queue timing, exposure-posture categories, staging and production results, the canary result (Stage 1 plus proof-mode Buy Now canary when it runs), release-lock state, release attempt phase/reason, and recovery metadata. Staging failures and stale staging skips upload `staging-release-health`; production attempts upload `production-release-health` including the proof-mode canary evidence.
