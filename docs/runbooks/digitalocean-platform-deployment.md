...
Before production Terraform apply or App Platform deployment can trigger `PRE_DEPLOY` bootstrap migrations, the production workflow creates a DigitalOcean managed-database fork from the current production Postgres cluster. DigitalOcean exposes this as `doctl databases fork <name> --restore-from-cluster-id <cluster-id> --wait`; the forked cluster contains source data from the original cluster at fork creation time. The workflow resolves the source cluster from the saved `postgres_cluster_id` output, falling back to the existing `digitalocean_database_cluster.postgres` state resource while that output is first rolling out. The workflow fails closed if the fork cannot be created or if Terraform state does not expose either source.
