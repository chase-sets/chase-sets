...
Production rejects filesystem-backed Catalog asset storage. The `infrastructure/digitalocean/catalog-assets` Terraform root owns bucket permissions, CDN policy, managed certificates, and CDN custom domains. The Spaces bucket ACL stays private; the Catalog S3-compatible upload adapter grants `public-read` only to the individual immutable objects it publishes through the CDN. The staging CDN DNS record is also declared in `infrastructure/digitalocean/environment-dns` because `assets.staging.chasesets.com` lives inside the delegated `staging.chasesets.com` child zone. Catalog owns only the provider import decision and the public URL it stores.

The Catalog asset buckets are stateful Terraform resources with `prevent_destroy = true`. A deliberate bucket replacement or deletion requires an audited PR that names the exact resource address, records migration or takedown evidence, temporarily removes the lifecycle guard only for that approved operation, and restores the guard immediately after the stateful change is complete. Do not bypass the guard from an operator shell or apply Terraform destroy against this root.
