...
2. **Query paths.** Compare the Kubernetes runtime Secret key sets between environments: every context has `DATABASE_URL_<CONTEXT>` plus `PLATFORM_CONTROL_DATABASE_URL`, with identical names in staging and production. Query URLs use managed PgBouncer transaction pools; listener, waiter, and bootstrap URLs remain direct as documented in the connection budget.
