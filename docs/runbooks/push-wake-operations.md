...
| Admin console > Projection Operations > **Settings and reference** (`/platform/projections/reference`) | Grafana/runbook handoff plus current wake attention items. Use the Projection repair queue for blocked-stream and poison-event evidence, and entity drawers for retry, rebuild, cancel, and durable operation detail. | Admin web, `projection-operations.view` for reads; `projection-operations.operate` for refresh/retry/cancel; `projection-operations.rebuild` for rebuilds |
...
2. **Query paths.** Compare the rendered Helm values and deployed Kubernetes Secret key sets between environments: every context has `DATABASE_URL_<CONTEXT>` plus `PLATFORM_CONTROL_DATABASE_URL`, with identical names in both environments. Both staging and production query values point at managed PgBouncer transaction pools; direct waiter, listener, and bootstrap keys are separate.
...
1. An intent retired after `WORKER_WAKE_MAX_ATTEMPTS` (default 10) means the underlying projection run kept failing — go to the projection console's Projection repair queue for the blocked stream, poison event, or degraded group and repair per [Projection Operations](./projection-operations.md).
