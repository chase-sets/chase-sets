...
`staging.chasesets.com` is also the staging marketplace root. If Google Workspace receives mail for that exact environment root, do not use a CNAME there. Delegate `staging.chasesets.com` as its own DigitalOcean DNS zone. The environment DNS Terraform root owns the child-zone Google Workspace MX/SPF records, SES bounce/DKIM/DMARC records, the catalog asset CDN CNAME, and the live DOKS A records for the apex, `www`, `marketplace`, and `admin`. Google Workspace DKIM for staging should be generated in the Admin Console and provided to the `staging` GitHub Environment as `GOOGLE_WORKSPACE_DKIM_TXT_VALUE`, which publishes `google._domainkey.staging.chasesets.com`.
