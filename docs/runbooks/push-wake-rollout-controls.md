...
| `WORKER_WAKE_HOT_LANE_RUNNER_COUNT` / `WORKER_WAKE_STANDARD_LANE_RUNNER_COUNT` / `WORKER_WAKE_BULK_LANE_RUNNER_COUNT` | One priority lane on the platform-worker | Non-negative integer, default `1`; `0` disables the lane (empty/invalid values keep the default, they do not zero the lane) | Env var | A zero-count lane gets no scheduler runners; intents in that lane stay queued until TTL expiry | Other lanes, polling, intent enqueueing |
