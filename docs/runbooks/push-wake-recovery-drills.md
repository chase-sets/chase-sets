...
When the workflow variable `STAGING_PLATFORM_WORKER_STATUS_URL` is configured, the drill also captures sanitized worker-status snapshots before the drill, after load, and after the final convergence check. These snapshots keep only bounded operational fields used by `projection:hot-lag-evidence`: worker state/counts, loop concurrency, wake lane counts, wake-intent aggregates, DB pool pressure counts, and projection-group lease-busy attribution. Do not put credentials, query strings with secrets, raw worker payloads, database URLs, provider details, or customer/account identifiers in that URL or in public evidence.
