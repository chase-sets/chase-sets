...
8. If `platform-error-page-detected` or `negative-probe-platform-error`, check whether a customer-facing checkout recovery is returning a 5xx document. Edge infrastructure may replace 5xx documents with a generic error wrapper, so temporary checkout recovery should render with a non-5xx document status while internal API freshness timeouts can remain 503 JSON.
