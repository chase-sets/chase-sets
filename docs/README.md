...
- [Post-Write Consistency Policy](./architecture/post-write-consistency.md): product-wide strategy taxonomy for `fresh-read`, `optimistic-with-correction`, `snapshot-return`, and `realtime-correction`, including SSE as a bounded correction channel.
...
- [Semantic Post-Write Handoffs](./architecture/semantic-post-write-handoffs.md): lightweight `postWriteHandoff` query metadata for `fresh-read` routes where stale `200` empty, stale resource, or `404` responses can hide a successful command.
