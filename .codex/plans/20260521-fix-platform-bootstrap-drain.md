...
Long-lived environment bootstrap should remain bounded to schema setup, seed reconciliation, and bootstrap-required projection synchronization. It should not fully drain every context subscription, projector, outbox, workflow, or projection group as part of DigitalOcean pre-deploy.
...
- When `scenario-seed` is not enabled, synchronize only bootstrap-required projection groups after seed work and skip open-ended full runtime drains.
