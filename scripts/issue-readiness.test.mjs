...
describe("issue-readiness/v1 receipt and rule contract", () => {
  ...
  it("keeps Terms: none. accepted for glossary impact", async () => {
    ...
    expect(result.receipt.status).toBe("ready");
    expect(result.receipt.checkedRules).toContainEqual({ id: "ready-07-terms-declared", status: "pass", reasonCodes: [] });
  });
  ...
  it("shared explicit-none forms pass both checkers", () => {
    ...
    expect(result.checkedRules).toContainEqual({ id: "ready-08-authority-probe-timed", status: "pass", reasonCodes: [] });
  });
