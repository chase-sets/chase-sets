...
export const mutationEvidenceCases = [
  mutationCase(
...
export async function executeConsentAuthorizationMutationEvidence({ receiptDir = null } = {}) {
  ...
  assertOfflineEvidence(new Set(caseIds).size === enumeration.reconciliation.total, "executed case identifiers repeat");
  assertOfflineEvidence(
...
function commandLineOptions(argv) {
  const options = { execute: false, receiptDir: null };
