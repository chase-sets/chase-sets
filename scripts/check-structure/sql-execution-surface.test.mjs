...
describe("repository-wide SQL execution partition", () => {
  ...
  it("measures the exact five-module and enforcement-neutral partition transition", () => {
    ...
    expect(legacyModules).toHaveLength(2348);
    expect(governedModules).toHaveLength(2343);
    ...
    expect(legacyPartition.sqlExecuting).toHaveLength(393);
    expect(legacyPartition.unprovableForm).toHaveLength(3);
    expect(legacyPartition.notSql).toHaveLength(1952);
    expect(legacyPartition.unresolvedMemberRoots.count).toBe(265);
    ...
    expect(partition.notSql).toHaveLength(1947);
