...
describe("repository-wide SQL execution partition", () => {
  ...
  it("measures the exact five-module and enforcement-neutral partition transition", () => {
    ...
    expect(legacyModules).toHaveLength(2482);
    expect(governedModules).toHaveLength(2477);
    ...
    expect(legacyPartition.sqlExecuting).toHaveLength(427);
    expect(legacyPartition.unprovableForm).toHaveLength(3);
    expect(legacyPartition.notSql).toHaveLength(2052);
    expect(legacyPartition.unresolvedMemberRoots.count).toBe(274);
    ...
    expect(partition.notSql).toHaveLength(2047);
