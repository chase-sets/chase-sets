...
function computedConstructorProperty(node) {
  ...
  if (ts.isElementAccessExpression(node.parent) && node.parent.argumentExpression === node) {
    // A key decided at its own written literal is not a runtime key shape, so
    // the generic residual arm leaves the written element-access arm standing;
    // only an exact coverage identity displaces it. The seam decides which
    // census arm counts this reference and nothing else about it.
    const written = { constructor: node.text, axis: "key", arm: "computed-property:element-access" };
    const identity = deriveConsentAuthorizationKeyCoverageIdentity(
      "element-access",
      node.kind,
      consentAuthorizationConstantKeyOutcome(node),
    );
    return identity === consentAuthorizationKeyRuntimeUnknownArm ? written : { ...written, arm: identity };
  }
  if (
...
/* -- Census coverage identity for key positions --------------------------- */
 ...
 */
export const consentAuthorizationConstantKeyOutcomes = Object.freeze([
  "written-literal",
  "shadowed-lexical-constant",
  "lexical-constant",
  "runtime",
]);

const noConstantBindings = new Map();

export function consentAuthorizationConstantKeyOutcome(node, bindings = noConstantBindings) {
...
/**
 * The census coverage identities for key positions, at the granularity one
 * committed coverage row is written at. Each member is one branch of a total
 * dispatch over the (syntax role, expression kind, constant-key outcome) triple
 * the analyzer has already observed.
 *
 * This table is consumed only by the census arm authority. It decides which
 * published census arm a key position is counted under; it never decides
 * whether a reference is classified, which constructor a reference names, or
 * which disposition a committed row carries. A triple no member names routes by
 * construction to the generic residual arm below, so a compiler or runtime key
 * shape nobody has met yet is admitted and counted rather than disappearing.
 */
export const consentAuthorizationKeyCoverageIdentities = Object.freeze(
  [
    {
      syntaxRole: "element-access",
      expressionKind: "StringLiteral",
      constantKeyOutcome: "written-literal",
      identity: "key:element-access:written-string-literal",
    },
    {
      syntaxRole: "element-access",
      expressionKind: "Identifier",
      constantKeyOutcome: "shadowed-lexical-constant",
      identity: "key:element-access:shadowed-lexical-constant:identifier",
    },
    {
      syntaxRole: "element-access",
      expressionKind: "TemplateExpression",
      constantKeyOutcome: "shadowed-lexical-constant",
      identity: "key:element-access:shadowed-lexical-constant:template-expression",
    },
    {
      syntaxRole: "binding-element",
      expressionKind: "Identifier",
      constantKeyOutcome: "lexical-constant",
      identity: "key:binding-element:lexical-constant:identifier",
    },
    {
      syntaxRole: "binding-element",
      expressionKind: "TemplateExpression",
      constantKeyOutcome: "lexical-constant",
      identity: "key:binding-element:lexical-constant:template-expression",
    },
  ].map((entry) => Object.freeze(entry)),
);

/**
 * The generic residual key arm. Every runtime key expression that is not one of
 * the exact coverage identities above reaches this one arm by construction and
 * is counted in the published census, so a new compiler or runtime key shape
 * cannot disappear and cannot classify itself.
 */
export const consentAuthorizationKeyRuntimeUnknownArm = "key:runtime-unknown";

/**
 * The coverage-only seam. It answers exactly one question -- which published
 * census arm a key position is counted under -- from three facts the analyzer
 * has already observed. It receives no source text, no node, and no bindings
 * map, so it cannot resolve a name, reach a constructor, fold a key, or decide
 * a reference's class.
 *
 * `expressionKind` is the compiler's numeric syntax kind, resolved through the
 * forward mapping for the same reason the specifier shape table is: several
 * kinds share a numeric value with a boundary alias, so the reverse mapping
 * answers `FirstTemplateToken` where a table names `NoSubstitutionTemplateLiteral`.
 */
export function deriveConsentAuthorizationKeyCoverageIdentity(syntaxRole, expressionKind, constantKeyOutcome) {
  const entry = consentAuthorizationKeyCoverageIdentities.find(
    (candidate) =>
      candidate.syntaxRole === syntaxRole &&
      candidate.constantKeyOutcome === constantKeyOutcome &&
      ts.SyntaxKind[candidate.expressionKind] === expressionKind,
  );
  return entry ? entry.identity : consentAuthorizationKeyRuntimeUnknownArm;
}

/**
 * The identity table must name compiler syntax kinds, roles and outcomes this
 * analyzer decides, one identity per triple, and no identity may collide with
 * the residual arm -- otherwise a key shape could be admitted under an identity
 * that is really the open default, or two committed rows could share one
 * identity again.
 */
export function assertConsentAuthorizationKeyCoverageIdentities(entries = consentAuthorizationKeyCoverageIdentities) {
  const identities = entries.map(({ identity }) => identity);
  const triples = entries.map(({ syntaxRole, expressionKind, constantKeyOutcome }) =>
    [syntaxRole, expressionKind, constantKeyOutcome].join("\0"),
  );
  const invalid = {
    unresolvedKinds: entries
      .filter(({ expressionKind }) => typeof ts.SyntaxKind[expressionKind] !== "number")
      .map(({ expressionKind }) => expressionKind),
    unnamedRoles: entries
      .filter(({ syntaxRole }) => !consentAuthorizationKeySyntaxRoles.includes(syntaxRole))
      .map(({ syntaxRole }) => syntaxRole),
    unnamedOutcomes: entries
      .filter(({ constantKeyOutcome }) => !consentAuthorizationConstantKeyOutcomes.includes(constantKeyOutcome))
      .map(({ constantKeyOutcome }) => constantKeyOutcome),
    duplicatedIdentities: [...new Set(identities.filter((value, index) => identities.indexOf(value) !== index))],
    duplicatedTriples: [...new Set(triples.filter((value, index) => triples.indexOf(value) !== index))],
    residualCollisions: identities.filter((identity) => identity === consentAuthorizationKeyRuntimeUnknownArm),
  };
  if (entries.length === 0 || Object.values(invalid).some((entry) => entry.length > 0)) {
    throw guardFailure(
      "consent-authorization-key-coverage-identity-invalid",
      "coverage.key-coverage-identity",
      "the committed key coverage identity table does not name one distinct identity per distinct role, compiler syntax kind and constant-key outcome",
      invalid,
    );
  }
  return entries;
...
export function scanConsentAuthorizationSource(relativeFile, source, artifact) {
  ...
  const visit = (node) => {
    ...
    if (
      ...
      computedBindingKeys.push(node.propertyName.expression);
    }
 ...
 * already decided at the literal itself -- one arm per committed key coverage
 * identity, and the generic residual arm every other runtime key routes to. The
 * specifier axis is the canonical named import,
...
export function deriveConsentAuthorizationCensusArms({ analyzerSource } = {}) {
  ...
  const keyCoverageIdentities = (
    declaredArrayLiteral(sourceFile, "consentAuthorizationKeyCoverageIdentities")?.elements ?? []
  )
    .map((element) => objectLiteralStringProperty(element, "identity"))
    .filter((value) => value !== null);
  const keyRuntimeUnknownArm = declaredStringConstant(sourceFile, "consentAuthorizationKeyRuntimeUnknownArm");
  ...
  requireDerived(keyCoverageIdentities, "the committed key coverage identities");
  requireDerived(keyRuntimeUnknownArm === null ? [] : [keyRuntimeUnknownArm], "the key runtime-unknown residual arm");
  ...
  return {
    key: unique([
      ...
      ...keyCoverageIdentities,
      keyRuntimeUnknownArm,
    ]),
    specifier: unique([
    ...
    // The generic residual arm of each axis, named separately because it is the
    // one arm that is open by construction: any expression kind can reach it, so
    // it is never partitioned by a committed row the way a closed arm is.
    residual: Object.freeze({ key: keyRuntimeUnknownArm, specifier: runtimeUnknownArm }),
  };
}
 ...
 * carry exactly one row each. Each axis also has one generic residual arm that
 * is open by construction -- any expression kind can reach it -- so it is not
 * partitioned by a committed row. Whether the matrix carries a census row for a
 * residual arm is therefore declared once in `residualArms`, outside the row
 * list, and asserted here in both directions: the declared arm must be the arm
 * the analyzer actually routes to, and the declared witness row must be exactly
 * the set of rows carrying it. Deleting the declared witness row is a coverage
 * omission exactly like deleting any other row, and no row may quietly take a
 * residual arm to hide behind its openness.
 */
export function assertConsentAuthorizationCensusArmPartition(coverage, derivedArms) {
  ...
  const differences = [];
  if (typeof arms?.residual?.key !== "string" || typeof arms?.residual?.specifier !== "string") {
    throw guardFailure(
      "consent-authorization-coverage-partition-partial",
      "coverage.arm-partition",
      "the derived census arm authority names no generic residual arm for each axis",
      { differences: [{ axis: "residual-arm", unwitnessed: ["key", "specifier"], underived: [] }] },
    );
  }
  for (const axis of ["key", "specifier"]) {
    const residualArm = arms.residual[axis];
    const witnessed = rows.filter((row) => row.axis === axis && row.census !== "silent").map((row) => row.arm);
    const unwitnessed = arms[axis].filter((arm) => arm !== residualArm && !witnessed.includes(arm));
    ...
    const declared = coverage?.residualArms?.[axis] ?? null;
    const declaredWitnesses = typeof declared?.witnessRow === "string" ? [declared.witnessRow] : [];
    const carriers = rows
      .filter((row) => row.axis === axis && row.census !== "silent" && row.arm === residualArm)
      .map((row) => row.rowId);
    const missingWitnesses = declaredWitnesses.filter((rowId) => !carriers.includes(rowId));
    const undeclaredCarriers = carriers.filter((rowId) => !declaredWitnesses.includes(rowId));
    if (declared?.arm !== residualArm || missingWitnesses.length > 0 || undeclaredCarriers.length > 0) {
      differences.push({
        axis: `${axis}-residual-arm`,
        unwitnessed: declared?.arm === residualArm ? missingWitnesses : [residualArm],
        underived: undeclaredCarriers,
      });
    }
  }
...
export function analyzeConsentAuthorizationSites(options = {}) {
  ...
  // The row-granular arm authority is consulted before the coarser
  // expression-kind authority, so a coordinated row-plus-fixture omission is
  // always reported at the finest clause that catches it rather than at
  // whichever coarser clause happens to trip on the same row.
