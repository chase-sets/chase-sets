...
const designSystemRequire = createRequire(
  path.resolve(import.meta.dirname, "../../packages/design-system/package.json"),
);
const fontPackage = designSystemRequire.resolve("@fontsource/space-grotesk/package.json");
const digest = (value: string) => createHash("sha256").update(value).digest("hex");

type RuntimeEvent = { kind: string; monotonicMs: number; digest: string };
let runtimeState: null | { events: RuntimeEvent[]; observe: (kind: string) => void } = null;
...
const test = base.extend<{ authorityTeardown: void }>({
  authorityTeardown: [
    async ({}, use) => {
      ...
      const receipt = {
        schema: "runtime-witness/v1",
        capturedAt: new Date().toISOString(),
        retry: 1,
        events: runtimeState.events,
      };
      await writeFile(path.join(artifactRoot, "runtime-receipt.json"), `${JSON.stringify(receipt)}\n`);
      runtimeState = null;
    },
    { auto: true },
  ],
...
test("retry-1 retained evidence corpus", async ({ page, context, request }, testInfo) => {
  ...
  let registeredValue = "";
  const woff2Bytes = await readFile(path.join(path.dirname(fontPackage), "files/space-grotesk-latin-400-normal.woff2"));
  ...
  const server = createServer((incoming, response) => {
    ...
    if (url.pathname === "/font.woff2") {
      ...
      response.end(
        `<style>@font-face{font-family:p;src:url('/font.woff2')}body{font-family:p}</style><main>${registeredValue}</main>`,
      );
      observe("response");
    }
  });
  ...
  await page.evaluate((value) => {
    ...
    console.log("authority-console-observation");
    return fetch("/api", {
      method: "POST",
  ...
  await testInfo.attach("registered-shapes", {
    body: Buffer.from(
      JSON.stringify({
        exact: registeredValue,
        url: encodeURIComponent(registeredValue),
        base64: Buffer.from(registeredValue).toString("base64"),
      }),
    ),
    contentType: "application/json",
  });
  await testInfo.attach("opaque-attachment", {
    body: Buffer.concat([Buffer.from([0, 1, 2, 255]), Buffer.from(registeredValue)]),
    contentType: "application/octet-stream",
  });
  await page.locator("main").screenshot({ path: testInfo.outputPath("rendered-value.png") });
  process.stdout.write("authority-corpus-stdout\n");
  process.stderr.write("authority-corpus-stderr\n");
  await new Promise<void>((resolve) => server.close(() => resolve()));

  runtimeState = { events, observe };
  await mkdir(artifactRoot, { recursive: true });
