...
function zipEntries(bytes, limit) {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    if (
      ...
      throw new Error("ZIP_REFUSED");
    const start = local + 30 + bytes.readUInt16LE(local + 26) + bytes.readUInt16LE(local + 28);
    const compressed = bytes.subarray(start, start + compressedSize),
      value = method === 0 ? compressed : inflateRawSync(compressed);
...
function stringsFrom(value, state, depth = 0) {
  ...
  const strings = Array.isArray(value)
    ? value.flatMap((entry) => stringsFrom(entry, state, depth + 1))
    : Object.values(value).flatMap((entry) => stringsFrom(entry, state, depth + 1));
  if (!Array.isArray(value) && Array.isArray(value.chunks))
    strings.push(value.chunks.map(String).join(value.separator === undefined ? "" : String(value.separator)));
  return strings;
}
...
function decodedStrings(text, state) {
  ...
  for (const token of tokens) {
    try {
      ...
      /* invalid token is not authority */
    }
  }
  let parsed;
  try {
  ...
  if (lines.length > 1) {
    const lineStrings = lines.flatMap((line) => {
      try {
        return stringsFrom(JSON.parse(line), state);
      } catch {
        return [line];
      }
    });
...
export function recoverRegisteredValue({ registeredValue, payloads, limits = {} }) {
  ...
  if (typeof registeredValue !== "string" || registeredValue.length < 16 || !Array.isArray(payloads))
    return { status: "INDETERMINATE", code: "INPUT_INVALID", observations: state };
  const targets = targetRepresentations(registeredValue),
    queue = payloads.map(Buffer.from),
    seenBytes = new Set(),
    seenStrings = new Set();
  try {
    while (queue.length) {
      ...
      if (
        state.decodedValues++ > bounded.maxDecodedValues ||
        state.decodedBytes > bounded.maxBytes ||
        queue.length > bounded.maxPayloads
      )
        throw new Error("BOUND_LIMIT");
      ...
      if (targets.some((target) => target.length && bytes.includes(target)))
        return { status: "HIT", code: "RECOVERABLE", observations: state };
      const zip = bytes.length >= 4 && bytes.readUInt32LE(0) === 0x04034b50,
        gzip = bytes.length >= 2 && bytes[0] === 0x1f && bytes[1] === 0x8b;
      if (zip) {
        state.archives += 1;
        queue.push(...zipEntries(bytes, bounded.maxPayloads));
      }
      ...
      if (
        ...
        state.unsupportedBinary += 1;
      const text = bytes.toString("utf8");
      for (const decoded of decodedStrings(text, state))
        if (!seenStrings.has(decoded)) {
          seenStrings.add(decoded);
          queue.push(Buffer.from(decoded));
        }
    }
    return state.unsupportedBinary
      ? { status: "INDETERMINATE", code: "UNSUPPORTED_BINARY_OR_RENDERED_VALUE", observations: state }
      : { status: "CLEAR", code: "NOT_RECOVERED_WITHIN_BOUNDS", observations: state };
  } catch (error) {
    return {
...
export function scanTrackedConsumers(root = path.resolve(import.meta.dirname, "../..")) {
  ...
  const candidates = tracked.filter(
    (file) =>
      /\.(?:[cm]?[jt]sx?)$/.test(file) &&
      !/(?:^|\/)(?:fixtures|tests?|__tests__|test-support)(?:\/|$)/.test(file) &&
  ...
  const grep = spawnSync(
    ...
    [
      ...
      "-z",
      "-E",
      "recovery-oracle\\.mjs|recoverRegisteredValue|targetRepresentations|decodedStrings|zipEntries|stringRepresentations|base64url.*utf16|utf16.*base64url",
      "--",
      "*.mjs",
      "*.mts",
      "*.ts",
  ...
  const violations = scanned
    .filter((file) => suspicious.has(file))
    .flatMap((file) =>
      checkConsumerIndependence(readFileSync(path.join(root, file), "utf8")).violations.map((code) => ({ file, code })),
    );
...
async function main(argv) {
  ...
  const valueFile = option("--registered-value-file"),
    payloadRoot = option("--payload-root");
  if (!valueFile || !payloadRoot) throw new Error("ORACLE_ARGUMENTS_REQUIRED");
  const result = recoverRegisteredValue({
    registeredValue: readFileSync(valueFile, "utf8").trim(),
    payloads: files(payloadRoot).map((file) => readFileSync(file)),
  });
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href)
  main(process.argv.slice(2)).catch(() => {
    process.stderr.write("recovery-oracle failed (bounded).\n");
    process.exitCode = 2;
  });
