...
export function Tabs({ items, value, defaultValue = items[0]?.value, onValueChange }: TabsProps) {
  return (
    <TabsPrimitive.Root value={value} defaultValue={defaultValue} onValueChange={onValueChange} className="grid gap-4">
      ...
      {items.map((item) => (
        <TabsPrimitive.Panel key={item.value} value={item.value} className="outline-none">
          {item.content}
        </TabsPrimitive.Panel>
      ))}
    </TabsPrimitive.Root>
