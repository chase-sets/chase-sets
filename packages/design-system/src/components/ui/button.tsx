...
const variantClasses: Record<ButtonVariant, string> = {
  ...
  secondary:
    "border border-[var(--border)] bg-[var(--secondary)] text-[var(--secondary-foreground)] shadow-[var(--shadow-sm)] hover:border-[var(--border-strong)] hover:bg-[var(--surface-3)]",
  outline:
    "border border-[var(--muted)] bg-[color-mix(in_srgb,var(--card)_72%,transparent)] text-[var(--foreground)] shadow-[var(--shadow-sm)] hover:border-[var(--primary)] hover:bg-[var(--secondary)]",
  ghost: "bg-transparent text-[var(--foreground)] hover:bg-[var(--secondary)]",
  destructive:
    "bg-[var(--destructive)] text-[var(--destructive-foreground)] shadow-[var(--shadow-sm)] hover:bg-[color-mix(in_srgb,var(--destructive)_82%,black)]",
};
...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(function Button(
  { className, variant = "default", size = "md", type = "button", loading = false, disabled, children, ...props },
  ref,
) {
  return (
    <button
