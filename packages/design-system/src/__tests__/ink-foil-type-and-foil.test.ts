...
describe("Ink & Foil display type and brand foil values", () => {
  ...
  it("imports all three Fontsource families at identical per-weight latin entrypoints backed by declared dependencies", () => {
    // Constructed from a string rather than written as a regex literal: the
    // repo-wide importer scan tokenizes source without regular-expression
    // context, so a literal whose body carries `@import` followed by a quoted
    // run would read there as a phantom module specifier.
    const fontsourceImportPattern = new RegExp('^@import "(@fontsource/[\\w-]+)/([\\w.-]+)";$', "gm");
    const imports = [...stylesSource.matchAll(fontsourceImportPattern)].map((match) => ({
      pkg: match[1]!,
      entry: match[2]!,
    }));
