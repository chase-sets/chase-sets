...
- Two probes are mandatory in that pass when they apply, because both defect classes have shipped through green CI:
  ...
  - **Added validation to a command handler?** Enumerate every seeding path (seed, bootstrap, import, reconciliation) and run each against the new rules. PR-lane CI runs the `DB Profile Tests` job on every PR, and that hosted job is the DB proof; a focused local `test:db` against a disposable Postgres is a useful diagnostic before you push, never a gate.
