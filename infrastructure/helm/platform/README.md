...
`platform-bootstrap` renders as a Helm `pre-install,pre-upgrade` hook. Before running bootstrap it pauses each worker's KEDA ScaledObject at zero through the in-image `bootstrap-quiesce.mjs` wrapper, scales the worker Deployment targets to zero, waits for those pods to drain, then runs the existing bootstrap command. The wrapper always removes the KEDA pause annotation after bootstrap so normal autoscaling resumes. If bootstrap fails or exceeds `CHASE_SETS_BOOTSTRAP_COMMAND_TIMEOUT_SECONDS`, deployments without a ScaledObject are restored to their previous replica counts before the wrapper returns the failing exit code and Helm aborts before new pods roll. The hook Job uses `activeDeadlineSeconds` below Helm's rollout timeout so Kubernetes reports a Job failure before Helm reaches its generic condition timeout. During first install, missing worker Deployments are skipped because there is no outgoing version to quiesce yet.
