...
/**
 * The loader's own closed vocabulary of production missing-config messages. `productionMissingConfigError`
 * was previously declared `string`, which made it an unconstrained external raw-value source echoed
 * verbatim through `new Error(...)` before any classification. Both members are byte-identical to the
 * literals the two production callers already pass inline, so neither caller file moves.
 *
 * A TypeScript annotation alone is erased at runtime, so the loader also refuses a non-member as its
 * first statement, before either key acquisition, with a fixed internal message that names no input,
 * no key, and no parameter value.
 */
export const PRODUCTION_MISSING_STRIPE_CONFIG_ERRORS = [
...
export type PlatformStripeProductionMissingConfigError = (typeof PRODUCTION_MISSING_STRIPE_CONFIG_ERRORS)[number];

/** The fixed internal refusal. It names no input, no key, and no parameter value. */
export const PRODUCTION_MISSING_STRIPE_CONFIG_VOCABULARY_REFUSAL =
  "loadStripeProviderConfig refused a production missing-config message outside its declared vocabulary.";

export function loadStripeProviderConfig(input: {
  productionLike: boolean;
  deploymentEnvironment?: DeploymentEnvironment;
  productionMissingConfigError: PlatformStripeProductionMissingConfigError;
}): PlatformStripeProviderConfig {
  if (
    input.productionMissingConfigError !== PRODUCTION_MISSING_STRIPE_CONFIG_ERRORS[0] &&
    input.productionMissingConfigError !== PRODUCTION_MISSING_STRIPE_CONFIG_ERRORS[1]
  ) {
    throw new Error(PRODUCTION_MISSING_STRIPE_CONFIG_VOCABULARY_REFUSAL);
  }
