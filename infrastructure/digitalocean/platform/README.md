...
- DigitalOcean App Platform domain attachments for App Platform hosts. Stable staging mail/delegation/assets and environment-scoped DOKS shadow DNS live in the sibling `environment-dns` Terraform root. In production, App Platform owns live CNAMEs while attached and Terraform owns live A records while DOKS serves. DOKS mode replaces the live attachment set with `app-platform.chasesets.com` and authority-qualifies every warm fallback route to it; each A record depends on that app update, and rollback destroys the A records before restoring the live attachments.
...
Production cutover defaults are deliberately inert: `production_app_serving=app-platform`, `production_serving_dns_phase=steady`, `production_doks_certificate_ready=false`, and an empty `doks_ingress_target`. The DOKS switch fails closed until the dedicated production target is set, the DNS-01 certificate covering every applicable live and shadow host is recorded ready, and the separate `prepare-doks` TTL-lowering phase has aged past the previously served TTL. `prepare-app-platform` applies the same state-backed wait before rollback. The TTL phase temporarily imports App Platform-owned CNAMEs; the flip releases them from state so App Platform removes them with the live domain attachments before dependent A records can create. Marketplace host records remain conditional on the existing public-exposure gate.
