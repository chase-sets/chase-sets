...
Production cutover defaults are deliberately inert: `production_app_serving=app-platform`, `production_doks_certificate_ready=false`, and an empty `doks_ingress_target`. The DOKS switch fails closed until the dedicated production target is set and the DNS-01 certificate covering every applicable live and shadow host is recorded ready. Marketplace host records remain conditional on the existing public-exposure gate.
