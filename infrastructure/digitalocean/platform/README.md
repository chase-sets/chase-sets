...
- DigitalOcean App Platform domain attachments for App Platform hosts. Stable staging mail/delegation/assets and environment-scoped DOKS shadow DNS live in the sibling `environment-dns` Terraform root. In production, App Platform owns live CNAMEs while attached and Terraform owns live A records while DOKS serves. DOKS mode replaces the live attachment set with `app-platform.chasesets.com`; each A record depends on that app update, and rollback destroys the A records before restoring the live attachments.
