locals {
  ...
  worker_max_concurrent_runners        = local.is_staging ? "6" : "5"
  worker_projection_concurrency        = "2"
  worker_default_job_concurrency       = local.is_staging ? 3 : 1
  ...
  source_observation_bulk_job_lanes    = local.is_staging ? "3" : "1"
  source_observation_bulk_workflow_cap = local.is_staging ? "3" : "1"
