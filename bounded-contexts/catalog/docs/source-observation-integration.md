...
Reapply and replay previews use the shared [Catalog Integration Impact Analysis](./catalog-integration-impact-analysis.md) model. The preview reports matched, eligible, blocked, impacted Catalog Item, external-reference, active-job, and bounded sample evidence from committed Catalog state before the confirm action re-resolves the scope server-side.
