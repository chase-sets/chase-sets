...
| J13 | Retirement and deprecation | destructive recovery | Deprecate or retire transitional profile versions only when reference counts and lifecycle rules allow it, with blocked-action explanations. | `primary-workbench-admin-contracts.test.ts` and `catalog-integration-legacy-cleanup.test.ts` prove complete deletion semantics. |
