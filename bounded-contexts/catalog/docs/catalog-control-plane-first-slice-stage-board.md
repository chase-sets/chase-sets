...
| #1038 | Stage 2 | PR #1201 | Complete; scoped import readiness, durable job monitoring, grouped failure evidence, modeled retry/resume/cancel availability, active-job conflict blocking, and Source Observation handoff links now live inside the rebuilt #1056 workbench. Executable retry/resume/cancel command routes remain #1063-owned and must not appear as clickable controls until backend support ships. |
...
| #1063 | Stage 3 | Issue comment required before implementation PR | Waiting on #1060 durable-job fields and Stage 2 workflow behavior; owns launch-ready retry/resume/cancel command routes for provider import jobs plus durable-job edge-case acceptance. |
