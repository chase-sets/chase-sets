...
This board operationalizes the first-slice delivery sequence for the Catalog Control Plane UX rebuild. It turns the milestone stage labels into entry gates, exit evidence, owner records, and follow-up rules so implementation can proceed without preserving retired admin structure as a migration target.
...
- migrate retired route layout or module boundaries;
...
| #1056 | Stage 2 | Owner comment, PR #1169, and current bounded-context delivery segment | In implementation; PR #1169 shipped the clean default `/integrations` workbench shell, and the current segment wires primary command forms plus the `/catalog/integrations` route-action bridge for provider import, promotion preview, promotion execution, reject, and reapply without reviving retired admin structure. |
...
| #1040 | Stage 2 | Owner comment and current bounded-context delivery segment | In implementation; completes exact selected-ID promotion preview freshness, durable defer jobs that keep observations in review, current-active-profile reapply, original-source-profile replay, and final promote/reject/defer/reapply/replay acceptance without preserving retired admin patterns. |
