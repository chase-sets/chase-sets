...
Catalog owns external product references because they map third-party product identity to Chase Sets Product identity.

An external product reference links one provider-owned identifier to one Catalog Item plus the selected Options needed to resolve the Product. Downstream contexts consume these references as published Catalog facts; they do not decide provider product identity themselves.
...
1. Provider variant or SKU identifiers that already encode condition/printing/finish.
2. Provider product identifiers paired with selected Options.
...
Provider sync jobs may add or refresh Source Observations, provider integration profile data, and external product references. They must not write Inventory Items or Marketplace Listings directly.

Inventory import and sync adapters consume the Catalog projection of external product references. If a provider export row carries multiple identifiers, Inventory may try each candidate in adapter order and use the first Catalog-owned reference that resolves.
