...
Provider-specific branches are launch blockers unless they are reviewed extension points named by provider adapter, profile section definition, fixture, section registry entry, or engine semantic primitive. Retired compatibility branches must be deleted completely from runtime code, API routes, admin UI, tests, fixtures, screenshots, docs, runbooks, and operator instructions; they may not remain as hidden flags, support-only tools, migration shims, redirects, or aliases.
