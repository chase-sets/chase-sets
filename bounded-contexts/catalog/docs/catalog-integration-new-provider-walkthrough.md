...
It is fixture-backed and has no live provider dependency. The thin real-provider proof follows later in #800.
...
After the reference slice passes, #800 should add one thin real-provider ingestion unit. The real-provider slice must keep the same architecture shape and should treat live transport, provider-data policy, fixture retention, and legal/policy gates as explicit readiness inputs rather than hidden implementation details.
