...
The rebuilt primary workbench composes this query inventory through the typed API/read-model contract documented in [Catalog Primary Workbench Admin Contract](./primary-workbench-admin-contract.md). Stage 2 workbench consumers must use that primary workbench contract for provider import, Source Observation review, promotion preview/result, blocker categories, route context, deploy-skew behavior, and instrumentation. They must not preserve retired admin module structure, add provider-specific admin branches, or recover missing fields through raw JSON reads.

The Health triage support workspace consumes the primary workbench `healthTriage` read model. That model composes `integration-health-summary`, `provider-transport-readiness-summary`, `import-job-progress-summary`, and `audit-evidence-timeline` into one operator dashboard with rollout-control state. It must show Catalog semantic readiness separately from provider adapter transport readiness and must keep freshness, owner metrics, affected provider/unit, and next action on degraded rows. UI modules must not rebuild the dashboard by calling lower-level storage, parsing provider profile JSON, or preserving retired admin support routes.

Performance, pagination, indexing, and stale-state expectations for this query inventory are documented in [Admin Control Plane Read-Model SLOs](./admin-control-plane-read-model-slos.md). Provider option query cache, cursor, stale, and degraded response behavior is documented in [Catalog Integration Provider Option Query Controls](./catalog-integration-provider-option-query-controls.md). Schema versioning and compatibility policy for Admin read models, job DTOs, and profile pointers is documented in [Catalog Integration Schema Compatibility](./catalog-integration-schema-compatibility.md). Provider-data visibility, redaction, retention, export, and approval policy is documented in [Catalog Integration Data Governance](./catalog-integration-data-governance.md). Credential ownership, validation states, and Admin readiness behavior are documented in [Catalog Integration Credential Readiness](./catalog-integration-credential-readiness.md). Canonical lifecycle audit/evidence records are documented in [Catalog Integration Audit Evidence](./catalog-integration-audit-evidence.md). Activation, rollback, retirement, replay, and reapply impact behavior is documented in [Catalog Integration Impact Analysis](./catalog-integration-impact-analysis.md). The authoritative typed SLO surface lives in:
