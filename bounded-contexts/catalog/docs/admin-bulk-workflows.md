...
High-volume list surfaces should use the shared manual reload prompt instead of automatic loader revalidation. The prompt counts unique projected entity IDs from realtime patches, shows the count in the list action bar, and lets the operator reload the current list when they are ready. Use this pattern when one operator action can emit many projection patches, such as promoting Source Observations into draft Catalog Items. Low-volume pages may keep automatic revalidation.
