...
| --- | --- | --- |
| Dense scanning | Workflow modules use compact headings, status pills, summary facts, tables, and scoped actions without marketing-style hero/card bloat. | Visual evidence and `primary-workbench-page.test.tsx` verify the rebuilt primary workbench by accessible headings and dense state coverage. |
...
| Keyboard flow | Grouped navigation, table actions, dialog actions, section editors, and destructive confirmations are reachable by keyboard and preserve focus context. | Accessibility evidence covers keyboard completion, focus trap/return, mobile navigation semantics, and dynamic progress announcements. |
...
| Responsive behavior | Supported admin viewports keep workflow sections readable; action bars wrap instead of overlapping tables or status labels. | Visual evidence must include desktop and mobile screenshots/review links for primary states, dense tables, evidence drawers, and mobile navigation. |
...
- the generated no-confusion UX acceptance packet when the change affects first-slice launch acceptance;
