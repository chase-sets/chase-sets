...
describe("findLatestConsent", () => {
  it("returns null without querying when no subject is given", async () => {
    ...
    const result = await findLatestConsent(db, { policyKey: "terms-of-service" });
  ...
  it("queries by policy key and subject, ordered to the most recent fact", async () => {
    ...
    const result = await findLatestConsent(db, {
      userId: "usr_1",
      accountId: "acc_1",
  ...
  it("returns null when no matching fact exists", async () => {
    ...
    const result = await findLatestConsent(db, { userId: "usr_new", policyKey: "terms-of-service" });
