...
- **Publication and activation are both required to RECORD, not only to require.** A Consent for a member whose artifact is not consent-activatable is rejected before its authority is read or written — activating a key whose document is still a counsel-review placeholder does not make it agreeable, and no seed, bootstrap or fixture profile authors such a fact.
- **The subject is declared by the bundle, never chosen by the caller.** A registration-bundle Consent is recorded for an exact user; a seller-onboarding Consent is recorded for an exact account and captures the acting account member. The account carried alongside a user-scoped Consent is recorded context, not the subject.
- **Satisfaction is read for one exact subject.** Acceptance is resolved on `(subject type, subject id, policy key)`, so another member of the same account can neither satisfy nor be satisfied by a user's bundle, and an account bundle is satisfied only by that exact account's Consents.
