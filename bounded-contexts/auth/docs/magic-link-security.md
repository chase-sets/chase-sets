...
Magic-link request responses must not expose the raw magic-link secret to browser clients. The request endpoint stores only the hashed token for consumption and stores the short-lived delivery token in mutable Auth token storage for the transactional email projector. The `auth.magic-link.requested` event publishes request facts such as token ID, user ID, email, and expiry; it must not publish token secrets, token hashes, or delivery tokens.
...
The `/api/auth/magic-link/consume` endpoint remains available for email-delivered token completion. It must continue to enforce token hashing, expiry, single-use consumption, and normal membership/account-selection checks before a session is created.
