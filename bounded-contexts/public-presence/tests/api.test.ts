...
describe("public presence API", () => {
  ...
  it("protects the sole referral-link join with session, permission, CSRF, and no-store", async () => {
  ...
  it("rejects nested unknowns and invalid creator tuples before provisioning", async () => {
    const provisionReferralLink = vi.fn(createServices().provisionReferralLink);
  ...
  it("keeps stale provisioning failures private and out of logs", async () => {
