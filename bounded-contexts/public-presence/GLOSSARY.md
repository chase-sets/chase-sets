...
- Each artifact section carries a review manifest (scope note, decision refs, product truth refs, open questions, assumptions) that feeds the counsel review packet; apart from the scope note shown as the counsel-pending placeholder, manifest fields are never rendered on the public page.
