...
4. Change the artifact publication status to `published`, regenerate the `@chase-sets/public-docs` metadata contract, and activate the same version through Identity's `identity.terms-of-service-active-version` policy.
