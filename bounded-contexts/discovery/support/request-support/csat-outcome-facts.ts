...
export const DISCOVERY_CSAT_OUTCOME_FACT_EVENT_TYPE = "discovery.csat-outcome-fact.published" as const;
...
function stableStreamId(idempotencyKey: string) {
  return `discovery.csat-outcome-fact-${createHash("sha256").update(idempotencyKey).digest("hex")}`;
}

export async function publishDiscoveryCsatOutcomeFact(
  ...
  if (sourceContextForOutcomeCode(outcomeCode) !== "discovery") {
    throw new Error(`CSAT workflow outcome code '${outcomeCode}' is not owned by discovery.`);
  }
