...
describe("item detail market offers", () => {
  it("projects submitted and accepted offers into the public discovery market table", async () => {
    ...
    expect(calls[0].sql).toContain("INSERT INTO discovery_market_offers");
  ...
  it("returns public market offers on item detail payloads", async () => {
    const db = {
      query: async (sql: string) => {
        ...
        if (sql.includes("FROM discovery_market_offers")) {
          return {
            rows: [
              {
    ...
    expect(item?.market_offers).toEqual([
