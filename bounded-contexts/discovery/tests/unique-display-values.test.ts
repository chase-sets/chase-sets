...
describe("uniqueDisplayValues", () => {
  it("removes duplicate and blank labels while preserving order", () => {
    expect(uniqueDisplayValues(["Pokemon TCG", "Singles", "Pokemon TCG", "  ", " Near Mint ", "Near Mint"])).toEqual([
      "Pokemon TCG",
      "Singles",
      "Near Mint",
    ]);
  ...
  it("removes duplicate strings while preserving order", () => {
    expect(uniqueStrings(["ctg_seed_pokemon_tcg", "ctg_seed_singles", "ctg_seed_pokemon_tcg"])).toEqual([
