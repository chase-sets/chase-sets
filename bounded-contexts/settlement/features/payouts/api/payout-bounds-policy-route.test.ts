...
import { createPayoutBoundsPolicyRoutes } from "./payout-bounds-policy-route";
...
function createApp(policies: Partial<PolicyRuntime>, permissions: readonly string[]) {
  ...
  app.route("/", createPayoutBoundsPolicyRoutes(policies as PolicyRuntime));
  return app;
}
...
describe("settlement payout-bounds policy routes", () => {
  it("returns the resolved current policy for viewers", async () => {
    ...
    const getPolicyDocument = vi.fn(async () => ({
      document_id: "pol_1",
      policy_key: "settlement.payout-bounds",
  ...
  it("returns a command snapshot when creating the payout-bounds policy", async () => {
    ...
    expect(createPolicyDocument).toHaveBeenCalledWith(
      expect.objectContaining({ policyKey: "settlement.payout-bounds" }),
  ...
  it("revises the payout-bounds policy through the admin API", async () => {
    ...
    expect(revisePolicyDocument).toHaveBeenCalledWith(
      ...
      "pol_1",
      expect.objectContaining({
        value: { currencyCode: "usd", minimumAmount: "10.00", maximumAmount: "5000.00" },
        actorUserId: "usr_admin",
      }),
      context,
    );
