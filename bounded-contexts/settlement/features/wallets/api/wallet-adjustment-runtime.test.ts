...
function createInMemoryEventStore() {
  ...
  const eventStore: EventStore = {
    ...
    // `fromVersion` is the store's INCLUSIVE 1-based stream version, not an
    // array offset, and an omitted `limit` means the enforced page maximum --
    // a fake that models either differently answers questions the real store
    // would not.
