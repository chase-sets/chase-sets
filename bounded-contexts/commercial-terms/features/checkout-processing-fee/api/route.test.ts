...
const samplePolicyValue = {
  ...
  methodAdjustments: [{ paymentMethodCategory: "card", percentageBpsDelta: 0, fixedAmountDelta: "0.00" }],
};

describe("commercial terms checkout processing-fee routes", () => {
  it("returns the resolved current policy for viewers", async () => {
    ...
    const getPolicyDocument = vi.fn(async () => ({
      document_id: "pol_1",
      policy_key: "commercial-terms.checkout-processing-fee",
  ...
  it("returns a command snapshot when creating the checkout processing-fee policy", async () => {
    ...
    expect(createPolicyDocument).toHaveBeenCalledWith(
      expect.objectContaining({ policyKey: "commercial-terms.checkout-processing-fee" }),
      expect.objectContaining({
        value: expect.objectContaining({ base: { percentageBps: 290, fixedAmount: "0.30" } }),
        status: "active",
  ...
  it("revises the checkout processing-fee policy through the admin API", async () => {
    ...
    expect(revisePolicyDocument).toHaveBeenCalledWith(
      ...
      "pol_1",
      expect.objectContaining({
        value: expect.objectContaining({ base: { percentageBps: 350, fixedAmount: "0.35" } }),
        actorUserId: "usr_admin",
      }),
      context,
    );
