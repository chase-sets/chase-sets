...
async function resolveTerms(
  ...
  const paymentFeeAmount = applyFeeFormula(amount, {
    percentageBps:
      agreement?.payment_fee_percentage_bps ?? schedule?.payment_fee_percentage_bps ?? 0,
    fixedAmount:
      agreement?.payment_fee_fixed_amount ?? schedule?.payment_fee_fixed_amount ?? "0.00",
  });
  ...
  return {
    ...
    paymentFeeAmount,
    sellerNetAmount: subtractMoneyAmounts(amount, marketplaceFeeAmount, paymentFeeAmount),
