...
The private `tcgplayer-market-capture-v1` receipt combines field-presence/type summaries captured before decoding with post-reduction counts, retained after the immutable capture commits. Its first optional nested member, `responseSummary.endpointDiagnostics`, is absent as a whole when diagnostics were not retained; absence never means success. When present, the typed object has exactly `sales`, `listings`, and `history`, each with exactly `failurePhase` and `httpStatusClass`; the producer emits all three. `failurePhase` is `transport` for rejection before parsed return, `response-processing` for failure after return during summarizing, decoding, or page processing, and null on success. It resets before every page request, independently for each endpoint and capture pass. These are code locations, not causes: transport failures cannot distinguish HTTP, request, configuration, throttle, cancellation, or JSON errors, and neither phase nor class diagnoses authentication, timeout, or schema causes. `httpStatusClass` copies the observation's `none`, `4xx`, `5xx`, or `other` class; null means no observation in direct sanitizer use (configuration-invalid captures exit before the sink). Diagnostics retain no exception text, body, cookie, unexpected field name, provider identity, or URL. A numeric `count` shape supplies no live count or completeness claim; null history remains a decode failure, distinct from a valid empty array. Existing receipt bytes are not read, defaulted, or backfilled.
