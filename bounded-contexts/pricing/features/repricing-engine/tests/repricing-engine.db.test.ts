...
describeDb("pricing signal-reactive repricing engine (#4331)", () => {
  ...
  it.each(["pause", "halt"] as const)(
    "BEFORE selection %s omits assignments and inputs with no command or fact",
    async (mode) => {
      const pool = pools.pricing;
  ...
  it.each(["pause", "halt"] as const)(
    "AFTER captured load %s records policy-precondition-failed for every listing without a command",
    async (mode) => {
      const pool = pools.pricing;
      ...
      for (const listingId of listingIds)
        expect(data.listings).toContainEqual(
          expect.objectContaining({ listingId, outcome: "skipped", skipReason: "policy-precondition-failed" }),
        );
    },
  );
