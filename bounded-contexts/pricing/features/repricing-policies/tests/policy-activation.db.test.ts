...
import { createPostgresEventStore, type PgTransactionalPool, type PostgresEventStore } from "@chase-sets/event-core-postgres";
...
describeDb("policy first activation", () => {
  ...
  function services(eventStore = createPostgresEventStore({ pool: pools.pricing }), pool = pools.pricing) {
    return { ...createRepricingPolicyRuntime({ eventStore, db: pool }),
      ...createRepricingPolicyActivationServices({ eventStore, pool }) };
  }
  async function completedRun() {
    const engine = createRepricingEngineRuntime({ eventStore: createPostgresEventStore({ pool: pools.pricing }), db: pools.pricing });
  ...
  async function createdEvents() {
    return (await createPostgresEventStore({ pool: pools.pricing }).readAll()).filter((event) => event.eventType === "pricing.repricing-policy.created");
  }
  ...
  it("creates from the exact stored body indefinitely and owns commands before projection catchup", async () => {
    ...
    await pools.pricing.query("UPDATE pricing_repricing_dry_runs SET completed_at = '2000-01-01' WHERE dry_run_id = $1 AND consumed_at IS NULL", [dryRunId]);
    ...
    expect(await controls.executeOwnedRepricingPolicy({ accountId: "acc_foreign", policyId, command: { type: "DeleteRepricingPolicy", deletedAt: new Date().toISOString() }, context: dryRunContext })).toBeNull();
    expect(await controls.executeOwnedRepricingPolicy({ accountId, policyId, command: { type: "PauseRepricingPolicy", pausedAt: new Date().toISOString() }, context: dryRunContext })).toMatchObject({ status: "paused" });
    const projected = buildRepricingPolicyProjectionHandlers(pools.pricing);
    for (const event of await createPostgresEventStore({ pool: pools.pricing }).readAll()) await projected[event.eventType]?.(toTransportEvent(event));
    expect(await controls.getAccountRepricingPolicy({ accountId, policyId })).toMatchObject({ policyId, status: "paused" });
    ...
    await expect(controls.activateRepricingPolicy(activateInput(dryRunId), dryRunContext)).rejects.toBeInstanceOf(DryRunRequiredError);
  });

  it.each(["queued", "failed", "consumed", "mismatched"])("rejects %s runs without an event or consumption", async (invalid) => {
    const dryRunId = await completedRun();
    if (invalid === "mismatched") await pools.pricing.query("UPDATE pricing_repricing_dry_runs SET body_hash = 'synthetic_wrong_hash' WHERE dry_run_id = $1 AND consumed_at IS NULL", [dryRunId]);
    else if (invalid === "consumed") await pools.pricing.query("UPDATE pricing_repricing_dry_runs SET consumed_at = now() WHERE dry_run_id = $1 AND consumed_at IS NULL", [dryRunId]);
    else if (invalid === "queued") await pools.pricing.query("UPDATE pricing_repricing_dry_runs SET status = 'queued' WHERE dry_run_id = $1 AND status = 'completed'", [dryRunId]);
    else await pools.pricing.query("UPDATE pricing_repricing_dry_run_jobs SET status = 'failed' WHERE job_id = $1 AND status = 'completed'", [dryRunId]);
    await expect(services().activateRepricingPolicy(activateInput(dryRunId), dryRunContext)).rejects.toBeInstanceOf(DryRunRequiredError);
    expect(await createdEvents()).toHaveLength(0);
    const row = (await pools.pricing.query<{ consumed: boolean }>("SELECT consumed_at IS NOT NULL AS consumed FROM pricing_repricing_dry_runs WHERE dry_run_id = $1", [dryRunId])).rows[0];
  ...
  it("rejects a genuinely cancelled dry-run worker without consuming its request", async () => {
    const engine = createRepricingEngineRuntime({ eventStore: createPostgresEventStore({ pool: pools.pricing }), db: pools.pricing });
    const run = await engine.enqueueDryRun({ sellerAccountId: accountId, body: dryRunBody }, dryRunContext);
    await expect(engine.processNextDryRunJob({ claimOwnerId: "synthetic_cancelled_7911", claimTtlMs: 30_000, signal: AbortSignal.abort() })).rejects.toThrow("cancelled");
    await expect(services().activateRepricingPolicy(activateInput(run!.dryRunId), dryRunContext)).rejects.toBeInstanceOf(DryRunRequiredError);
    expect(await createdEvents()).toHaveLength(0);
  ...
  it("foreign and absent dry runs are indistinguishable and do not consume", async () => {
    const dryRunId = await completedRun();
    expect(await services().activateRepricingPolicy({ ...activateInput(dryRunId), accountId: "acc_foreign" }, dryRunContext)).toBeNull();
  ...
  it("holds concurrent consumption until winner commit and permits exactly one activation", async () => {
    ...
    let reached!: () => void;
    let release!: () => void;
    const atAppend = new Promise<void>((resolve) => { reached = resolve; });
    const barrier = new Promise<void>((resolve) => { release = resolve; });
    const heldStore: PostgresEventStore = { ...store, appendToStreamInTransaction: async (client, input) => {
      reached(); await barrier; return store.appendToStreamInTransaction(client, input);
    } };
    ...
    try {
      ...
      const atConsumption = new Promise<void>((resolve) => { reachedConsumption = resolve; });
      ...
      expect(await createdEvents()).toHaveLength(1);
    } finally { release(); await Promise.allSettled([winner, ...(loser ? [loser] : [])]); }
  });

  it("rolls back consumption and an appended event on transaction failure, then permits retry", async () => {
    const dryRunId = await completedRun();
    const store = createPostgresEventStore({ pool: pools.pricing });
    const failing: PostgresEventStore = { ...store, appendToStreamInTransaction: async (client, input) => {
      await store.appendToStreamInTransaction(client, input);
      throw new Error("synthetic failure after append before commit");
    } };
    await expect(services(failing).activateRepricingPolicy(activateInput(dryRunId), dryRunContext)).rejects.toThrow("synthetic failure");
    expect(await createdEvents()).toHaveLength(0);
    expect((await pools.pricing.query<{ consumed_at: string | null }>("SELECT consumed_at FROM pricing_repricing_dry_runs WHERE dry_run_id = $1", [dryRunId])).rows[0]?.consumed_at).toBeNull();
    expect(await services().activateRepricingPolicy(activateInput(dryRunId), dryRunContext)).toMatchObject({ status: "active" });
    expect(await createdEvents()).toHaveLength(1);
  });
});
