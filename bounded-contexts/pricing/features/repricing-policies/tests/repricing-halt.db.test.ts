...
describeDb("Repricing Halt steady-state lifecycle", () => {
  ...
  it("engage excludes, repeat emits nothing, release rejoins the next drift sweep without resuming an intended pause", async () => {
    ...
    for (const [policyId, listingId, paused] of [
      ...
      await controls.commandHandler({
        streamId: controls.streamIdForPolicy(policyId),
        context: dryRunContext,
        command: {
          ...dryRunBody,
          scope: { kind: "listing-set", listingIds: [listingId] },
    ...
    const project = async () => {
      for (const event of await eventStore.readAll()) await handlers[event.eventType]?.(toTransportEvent(event));
    };
    await project();
    ...
    await project();
    expect(await controls.listRepricingPolicyAssignments({ accountId: "acc_7910" })).toEqual([
      expect.objectContaining({ policyId: "rpp_synthetic_active" }),
    ]);
    ...
    const apply = vi.fn<RepricingMarketplaceGateway["applyBulkListingPriceUpdates"]>(async ({ updates }) => ({
      items: updates.map(({ listingId }) => ({ listingId, outcome: "applied" })),
    }));
    const gateway: RepricingMarketplaceGateway = {
      applyBulkListingPriceUpdates: apply,
      pauseListing: vi.fn(),
      publishListing: vi.fn(),
    };
    expect(
      await engine.processNextEvaluationJob({
        claimOwnerId: "synthetic_7911",
        claimTtlMs: 30_000,
    ...
    const haltEvents = (await eventStore.readAll()).filter((event) =>
      event.eventType.startsWith("pricing.repricing-halt."),
    );
    expect(haltEvents.map(({ eventType }) => eventType)).toEqual([
      "pricing.repricing-halt.engaged",
      "pricing.repricing-halt.released",
    ]);
    expect(
      haltEvents.every(
        ({ performedByUserId, forAccountId }) =>
          performedByUserId === dryRunContext.audit.performedByUserId && forAccountId === "acc_7910",
      ),
    ).toBe(true);
    ...
    expect(
      (
        await db.query<{ engaged: boolean }>(
          "SELECT engaged FROM pricing_repricing_halts WHERE seller_account_id = 'acc_7910'",
        )
      ).rows,
