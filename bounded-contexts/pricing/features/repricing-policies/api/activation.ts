...
import { hashRepricingDryRunBody, validateRepricingDryRunBody, type RepricingDryRunBody } from "../../repricing-engine/api/dry-run";
import { decideRepricingPolicy, evolveRepricingPolicy, initialRepricingPolicyState, type RepricingPolicyEvent } from "../domain/domain";
import { repricingPolicyStreamId } from "../read-model/projection";

export class DryRunRequiredError extends Error {
  constructor() { super("dry_run_required"); }
}

export function createRepricingPolicyActivationServices(deps: Readonly<{
  ...
  return {
    activateRepricingPolicy: async (input: Readonly<{ accountId: string; dryRunId: string; name: string }>, context: EventStoreContext) =>
      withPgTransaction(deps.pool, async (client) => {
        const run = (await client.query<{
          body: RepricingDryRunBody; body_hash: string; status: string; consumed_at: string | null; job_status: string;
        }>(
        ...
        if (!run) return null;
        if (run.status !== "completed" || run.job_status === "failed" || run.job_status === "cancelled" || run.consumed_at !== null) throw new DryRunRequiredError();
        try {
          if (hashRepricingDryRunBody(run.body) !== run.body_hash ||
              hashRepricingDryRunBody(validateRepricingDryRunBody(run.body, input.accountId)) !== run.body_hash)
            throw new DryRunRequiredError();
        } catch { throw new DryRunRequiredError(); }
        const policyId = createId("rpp");
        const createdAt = new Date().toISOString();
        const events = decideRepricingPolicy(initialRepricingPolicyState, {
          ...run.body, type: "CreateRepricingPolicy", policyId,
          accountId: parseTypedId(input.accountId, "acc"), name: input.name, createdAt,
        });
        ...
        await deps.eventStore.appendToStreamInTransaction(client, {
          streamId: repricingPolicyStreamId(policyId), expectedVersion: "no_stream", context,
          wakeSourceContextName: "pricing", events: events.map(codec.encode),
        });
