...
export async function getRepricingBudget(db: PgQueryable, accountId: string, day: string) {
  const row = (await db.query<{ changes_reserved: number }>(
    `SELECT changes_reserved FROM pricing_repricing_daily_change_budgets
     WHERE seller_account_id = $1 AND budget_day = $2::date`, [accountId, day],
  )).rows[0];
...
export async function listRepricingCategories(db: PgQueryable, accountId: string) {
  return (await db.query<{ id: string; name: string; status: string; listingCount: number }>(
    `SELECT category.category_id AS id, category.name, category.status,
     ...
     GROUP BY category.category_id ORDER BY category.name, category.category_id`, [accountId],
  )).rows;
}

export type RepricingScopePreviewInput = Readonly<{
  accountId: string; scope: RepricingPolicyScope; excludedListingIds?: readonly string[]; replacingPolicyId?: string;
}>;
type PolicyCount = Readonly<{ policyId: string; name: string; count: number }>;
export type RepricingScopePreview = Readonly<{
  matching: number; governed: number; shadowedBy: readonly PolicyCount[]; takenFrom: readonly PolicyCount[];
}>;

export async function previewRepricingScope(db: PgQueryable, input: RepricingScopePreviewInput): Promise<RepricingScopePreview> {
  const result = await db.query<RepricingScopePreview>(
    ...
    [input.accountId, input.replacingPolicyId ?? null, JSON.stringify({
      sellerAccountId: input.accountId, replacingPolicyId: input.replacingPolicyId,
