...
| `checkout-concept` | `/checkout/concept` | Internal reference only until the real implementation exists | None; delete or hard-disable before launch |
...
- legacy checkout paths are removed, hard-disabled, or internal-only
...
4. Remove or hard-disable legacy route modules and update route composition tests.
