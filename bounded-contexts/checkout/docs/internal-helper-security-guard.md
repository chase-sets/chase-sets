...
Milestone #17 keeps customer checkout Shopify-simple. Internal diagnostics, support lookup helpers, provider test
utilities, and webhook replay must stay outside customer checkout and fail closed unless the owning operator surface
grants the required permission.

This guard covers retained helper families that can influence checkout launch posture. It does not approve new customer
surfaces. Customer-visible checkout may show only support-safe status and recovery copy.

## Requirements

- Customer routes must not expose diagnostic, provider payload, webhook replay, or deferred-payment helper copy.
...
- Allowed internal use must emit audit or observability with redacted fields only: no provider payloads, PII, card or bank
  details, cookies, session ids, raw emails, addresses, full URLs, or provider secrets.
...
| Helper | Owner | Runtime surface | Gate | Customer reachable | Guard |
| --- | --- | --- | --- | --- | --- |
...
Checkout no longer exposes a deferred-payment confirmation helper. Confirmation requires the normal payment quote path
before order creation or payment handoff.

## Removal

Checkout owns no retained internal helper in this inventory. Other helper families remain in their owning contexts and
are not customer checkout capabilities.
