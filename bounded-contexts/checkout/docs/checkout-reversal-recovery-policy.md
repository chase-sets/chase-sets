...
| Fresh-state reversal cleanup | Platform | pre-confirmation, operator-recovery, owner-rule | no-side-effect, payment-void, payment-refund, order-cancel, label-cancel, payout-reversal, settlement-adjustment | supported, disabled, deferred, unsupported, no-side-effect | Fresh-state scans show cancellation, refund, void, reversal, and adjustment paths cannot succeed through old routes, old payload adapters, old sell execution ids, old receipt rows, migrations, hidden repair, stale fixtures/read models, provider-dashboard-only recovery, or dense checkout fallback. |
