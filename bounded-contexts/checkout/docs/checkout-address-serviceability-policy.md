...
| Fresh-state address cleanup | Platform | checkout-session-create, active-session-return, final-confirmation, operator-support | none | internal-only | operations-status | Fresh-state scans show address handling cannot succeed through old payloads, hidden normalization, migration/backfill helpers, stale read models, stale fixtures, or dense checkout fallback. |
