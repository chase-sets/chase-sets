...
- Runtime evidence that trusted UI and UCP confirmation call the same
  ...
  address telemetry.
- Release or staging evidence for guest and signed-in buyer delivery
...
- #1114 observability proving category-only telemetry, support-safe references,
  freshness status, launch decision, and no full address or raw provider
  payload exposure.
