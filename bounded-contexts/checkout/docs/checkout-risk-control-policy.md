...
| Fresh-state risk cleanup | Platform | cart-list-readiness, checkout-session-create, final-confirmation, operator-support | buy-checkout-session-create, sell-checkout-session-create, buy-final-confirmation, sell-final-confirmation, support-escalation | enabled | operations-status | Fresh-state scans show risk handling cannot succeed through old routes, payload adapters, migration/backfill helpers, dual writes, hidden repair, stale fixtures, or dense checkout fallback. |
