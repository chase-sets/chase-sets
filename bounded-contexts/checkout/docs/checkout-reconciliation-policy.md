...
| Unassigned fulfillment readiness reconciliation | Checkout | before-checkout | cart-list-readiness, conditional-pre-checkout, launch-decision | checkout-confirmation, payment, order, inventory-reservation, label, payout, settlement | Items without fulfillment assignment stay in cart/list readiness or a conditional pre-checkout step; checkout can reconcile the rejection as no-side-effect evidence and must not assign fulfillment, create labels, or start payment/order work. |
...
| Operator recovery reconciliation | Support | operator-support | operator-recovery, launch-decision | support, payment, order, label, payout, settlement, notification, account-history, refund, void, reversal, adjustment | Operator recovery starts from support-safe references, shows owner and next action, audits every recovery command, and does not instruct support to edit data or infer completion. |
...
- Release or staging evidence for duplicate submit, reload, redirect
...
- Launch decision rows for disabled, deferred, unsupported, provider-outage,
...
- #1114 observability proving owner/status/idempotency telemetry, support-safe
  references, redaction, launch-decision outcomes, and no raw provider payload,
