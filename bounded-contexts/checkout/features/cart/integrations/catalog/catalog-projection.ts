...
type CheckoutCatalogItemRow = Readonly<{
...
type CheckoutBlueprintRow = Readonly<{
  blueprint_id: string;
  name: string;
...
type CheckoutChoiceRow = Readonly<{
  option_id: string;
  code: string;
  labels: unknown;
}>;
...
export function buildCheckoutCatalogProjectionHandlers(
  ...
  return {
    "catalog.catalog-item.created": async (event) => {
      ...
      await db.query(
        `INSERT INTO checkout_catalog_items (
           catalog_item_id,
           language_code,
           title,
           subtitle,
      ...
      await refreshCheckoutCatalogItem(db, itemId);
    },
    ...
    "catalog.blueprint.created": async (event) => {
      ...
      await db.query(
        `INSERT INTO checkout_catalog_blueprints (
           blueprint_id,
           name,
           status,
           updated_at
    ...
    "catalog.blueprint.revised": async (event) => {
      ...
      await db.query(
        `INSERT INTO checkout_catalog_blueprints (
           ...
           $1,
           $2,
           COALESCE((SELECT status FROM checkout_catalog_blueprints WHERE blueprint_id = $1), 'draft'),
           $3
         )
         ON CONFLICT (blueprint_id) DO UPDATE SET
           name = EXCLUDED.name,
    ...
    "catalog.blueprint.dimensions-set": async (event) => {
      ...
      await db.query(
        `UPDATE checkout_catalog_blueprints
    ...
    "catalog.dimension.created": async (event) => {
      ...
      await db.query(
        `INSERT INTO checkout_catalog_dimensions (
           dimension_id,
           name,
           updated_at
         ) VALUES ($1, $2, $3)
      ...
      await refreshAllCheckoutCatalogItems(db);
    },
    ...
    "catalog.dimension.option-added": async (event) => {
      ...
      await db.query(
        `INSERT INTO checkout_catalog_dimension_options (
           option_id,
           dimension_id,
           code,
           labels,
