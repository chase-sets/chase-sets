...
export function buildSavedListPickerProjectionHandlers(db: PgQueryable): ProjectorHandlerMap {
  return {
    "collections.saved-list.created": async (event) => {
      ...
      await db.query(
        `INSERT INTO collections_saved_list_picker (
           list_id,
           owner_account_id,
           title,
           lifecycle,
    ...
    "collections.saved-list.details-changed": async (event) => {
      ...
      await db.query(
        `UPDATE collections_saved_list_picker
