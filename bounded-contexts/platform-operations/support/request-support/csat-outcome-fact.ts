...
export const supportCsatOutcomeFactEventType = "platform-operations.csat-outcome-fact.v1" as const;
...
function normalizeIsoTimestamp(value: string, message: string) {
...
export function createSupportCsatOutcomeFact(
  input: Readonly<{
    outcomeCode: string;
  ...
  if (sourceContextForOutcomeCode(input.outcomeCode) !== "platform-operations") {
    throw new Error(`Platform Operations cannot publish CSAT outcome code: ${input.outcomeCode}`);
  }
  ...
  return {
    ...
    outcomeOccurredAt: normalizeIsoTimestamp(input.outcomeOccurredAt, "CSAT outcome requires an occurrence timestamp."),
