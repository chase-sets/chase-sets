...
import type { FulfillmentPackingSlip, FulfillmentPackingSlipFormat } from "../../support/request-support/api-client";
...
const packingSlipFormats = new Set<FulfillmentPackingSlipFormat>(["letter", "thermal-4x6"]);

function parseShipmentIds(url: URL) {
  return Array.from(
    new Set(
      [...url.searchParams.getAll("shipmentIds"), url.searchParams.get("shipmentIds") ?? ""]
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  if (shipmentIds.length === 0) {
    throw new Response(t("fulfillment.routes.marketplace.accountSaleShipmentPackingSlips.no.shipments.selected"), {
      status: 400,
    });
  }
...
export const meta: MetaFunction = () =>
  buildOpenGraphMeta({
    title: t("fulfillment.routes.marketplace.accountSaleShipmentPackingSlips.packing.slips.marketplace"),
  });
