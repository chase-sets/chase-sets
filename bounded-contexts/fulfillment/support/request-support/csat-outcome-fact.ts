...
export const fulfillmentCsatOutcomeFactEventType = "fulfillment.csat-outcome-fact.v1" as const;
...
export function createFulfillmentCsatOutcomeFact(
  input: Readonly<{
    outcomeCode: string;
  ...
  if (sourceContextForOutcomeCode(input.outcomeCode) !== "fulfillment") {
    throw new Error(`Fulfillment cannot publish CSAT outcome code: ${input.outcomeCode}`);
  }
