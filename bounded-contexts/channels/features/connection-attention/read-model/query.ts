...
export async function readConnectionAttention(
  ...
  const keys = await db.query<{ connection_id: string }>(
    ...
    eligible AS (SELECT connection_id, observed_at::timestamptz AS opened_at FROM manual
      UNION ALL SELECT connection_id, opened_at FROM health)
    SELECT connection_id FROM eligible GROUP BY connection_id ORDER BY min(opened_at), connection_id LIMIT 100`
      : `SELECT connection_id FROM channel_connections WHERE account_id=$1 AND connection_id=$2`,
    connectionId === null ? [accountId] : [accountId, connectionId],
  );
