...
export async function readChannelDriftDetail(
  ...
  try {
    return await withPgTransaction(pool, async (db): Promise<ChannelDriftDetail> => {
      ...
      const result = await db.query<{
        ...
        `WITH rows AS (
            SELECT 'listing'::text AS row_kind,
                   encode(sha256(convert_to(item.channel_listing_id,'UTF8')),'hex') AS row_identity,
            ...
            FROM channel_reconciliation_items AS item WHERE item.connection_id=$1
            UNION ALL
            SELECT 'finding',encode(sha256(convert_to(finding_id,'UTF8')),'hex'),NULL,NULL,run_generation,NULL,NULL
            FROM channel_reconciliation_findings WHERE connection_id=$1 AND open
          ), page AS (
            SELECT * FROM rows WHERE ($2::text IS NULL OR (row_kind,row_identity)>($2,$3))
            ORDER BY row_kind,row_identity LIMIT 51
          )
          SELECT page.*,decision.revision,decision.accepted_observed_fingerprint,
          ...
          FROM page LEFT JOIN channel_drift_decisions AS decision
            ON decision.connection_id=$1 AND decision.channel_listing_id=page.channel_listing_id
          ORDER BY page.row_kind,page.row_identity`,
        [input.connectionId, cursor?.kind ?? null, cursor?.identity ?? null],
      );
      const rows = result.rows.slice(0, 50).map((row): ChannelDriftDetailRow => {
      ...
      const hasMore = result.rows.length > 50 ? 1 : 0;
